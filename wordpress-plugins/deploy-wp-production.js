/**
 * Deploy neo-pulse-wp zip to all production clients in the SFTP CSV.
 *
 * Run: npm run deploy:wp-production
 */
...
async function deploySite(site) {
  ...
  try {
    ...
    process.stdout.write("\n");
    console.log(err(site.label + " — " + (e && e.message ? e.message : String(e))));
    return false;
  }
}

async function main() {
  header("NEO Pulse WP Production Deploy");
  ...
  if (okCount === sites.length) {
    console.log(ok(`${okCount} production upload${sites.length === 1 ? "" : "s"}`));
  } else {
