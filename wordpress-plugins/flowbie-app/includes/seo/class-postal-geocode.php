...
class Flowbie_App_Postal_Geocode {
	...
	private static function data_dir(): ?string {
		if ( defined( 'FLOWBIE_APP_GEONAMES_DIR' ) && is_dir( FLOWBIE_APP_GEONAMES_DIR ) ) {
			return rtrim( (string) FLOWBIE_APP_GEONAMES_DIR, '/\\' );
		}
		$plugin_data = FLOWBIE_APP_PLUGIN_DIR . 'data/geonames';
		if ( is_dir( $plugin_data ) ) {
			return $plugin_data;
		}
		$repo = dirname( FLOWBIE_APP_PLUGIN_DIR, 2 ) . '/server/data/geonames';
