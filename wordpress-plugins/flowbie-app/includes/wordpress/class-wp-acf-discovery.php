<?php
/**
 * ACF field group discovery (Flowbie export, ACF REST, sample scan).
 *
 * @package Flowbie_App
 */
...
class Flowbie_App_Wp_Acf_Discovery {
	...
	public static function discover_acf_field_groups( $body ) {
		...
		$ping = Flowbie_App_Wp_Flowbie_Tools::ping_public( $normalized );
		if ( ! empty( $ping['ok'] ) ) {
			$export = Flowbie_App_Wp_Flowbie_Tools::execute_tool( $normalized, $username, $app_password, 'wp_fields_export_json', array() );
			$json   = isset( $export['json'] ) ? $export['json'] : ( isset( $export['data']['json'] ) ? $export['data']['json'] : ( is_string( $export['data'] ?? null ) ? $export['data'] : null ) );
			$parsed = self::parse_flowbie_fields_export_json( is_string( $json ) ? $json : '' );
		...
		$groups_rs  = Flowbie_App_Wp_Rest_Client::request( 'GET', $groups_url, $username, $app_password, array( 'timeout' => 30 ) );
		...
		$posts_url = $normalized . '/wp-json/wp/v2/' . rawurlencode( $collection ) . '?per_page=' . max( 1, min( 100, $sample_size ) ) . '&context=edit&_fields=id,title,acf,flowbie_fields';
		$scan_rs   = Flowbie_App_Wp_Rest_Client::request( 'GET', $posts_url, $username, $app_password, array( 'timeout' => 30 ) );
		if ( ! $scan_rs['is_wp_error'] && (int) $scan_rs['status'] === 200 && is_array( $scan_rs['body'] ) ) {
			$field_map = array();
			foreach ( $scan_rs['body'] as $post ) {
				$acf = Flowbie_App_Wp_Url_Normalize::rest_acf_from_post( is_array( $post ) ? $post : null );
				if ( ! is_array( $acf ) ) {
					continue;
				}
	...
	/**
	 * @param string $json_string Flowbie fields export JSON.
	 * @return array{fieldGroups:array<int,array<string,mixed>>,fields:array<int,array<string,mixed>>}|null
	 */
	public static function parse_flowbie_fields_export_json( $json_string ) {
