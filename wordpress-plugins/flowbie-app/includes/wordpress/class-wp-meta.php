...
class Flowbie_App_Wp_Meta {
	...
	public static function get_post_meta( $body ) {
		...
		$resp = Flowbie_App_Wp_Rest_Client::request( 'GET', $url, $username, $app_password, array( 'timeout' => 30 ) );
		if ( $resp['is_wp_error'] ) {
	...
	public static function update_post_meta( $body ) {
		...
		$current_post = null;
		$get_resp     = Flowbie_App_Wp_Rest_Client::request(
			'GET',
		...
		if ( $current_post ) {
			if ( $has_title ) {
				...
				$update_data['title'] = Flowbie_App_Wp_Url_Normalize::rendered_text( $current_post['title'] ?? '' );
			}
			if ( $has_excerpt ) {
		...
		$verified_focus = null;
		$verify         = Flowbie_App_Wp_Rest_Client::request(
			'GET',
			$base_url . '?context=edit',
			$username,
			$app_password,
	...
	public static function update_acf_field( $body ) {
		...
		$validation = Flowbie_App_Wp_Acf_Protocol::validate_acf_setup( $site_url, $username, $app_password, $post_type, $endpoint, $post_id );
		...
		$serialized = Flowbie_App_Wp_Acf_Protocol::serialize_acf_value( $field_value );
		...
		if ( ! $verify['is_wp_error'] && (int) $verify['status'] === 200 ) {
			$acf_payload = Flowbie_App_Wp_Url_Normalize::rest_acf_from_post( is_array( $verify['body'] ) ? $verify['body'] : array() );
