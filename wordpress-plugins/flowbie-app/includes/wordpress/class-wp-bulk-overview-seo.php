...
class Flowbie_App_Wp_Bulk_Overview_Seo {

	const FLOWBIE_BULK_ITEMS_MAX = 500;
	...
	public static function update_overview_seo_item( $body ) {
		...
		$post_id = Flowbie_App_Wp_Url_Normalize::normalize_post_id( isset( $item['postId'] ) ? $item['postId'] : null );
		if ( $post_id === null ) {
	...
	public static function bulk_update_overview_seo( $body ) {
		...
		foreach ( $items as $index => $raw ) {
			$post_id = Flowbie_App_Wp_Url_Normalize::normalize_post_id( isset( $raw['postId'] ) ? $raw['postId'] : null );
			if ( $post_id === null ) {
	...
	public static function bulk_write_items( $normalized, $username, $app_password, $to_run, $on_batch = null ) {
		...
		foreach ( $to_run as $item ) {
			if ( Flowbie_App_Wp_Overview_Seo_Item::build_core_put_body( $item ) || Flowbie_App_Wp_Overview_Seo_Item::has_acf_payload( $item ) ) {
				++$writable;
			}
		}
		$cumulative = 0;
		...
		for ( $i = 0; $i < count( $to_run ); $i += self::WP_REST_BATCH_V1_MAX ) {
			...
			foreach ( $slice as $item ) {
				$core = Flowbie_App_Wp_Overview_Seo_Item::build_core_put_body( $item );
				$acf  = Flowbie_App_Wp_Overview_Seo_Item::has_acf_payload( $item );
				...
				if ( ! $core && ! $acf ) {
					...
					$acf_result = Flowbie_App_Wp_Overview_Seo_Item::write_acf_via_post( $normalized, $username, $app_password, $item );
					$row        = array(
						'postId'     => $item['postId'],
						'index'      => $item['index'],
					...
					$core_row = Flowbie_App_Wp_Overview_Seo_Item::write_core_via_direct_put( $normalized, $username, $app_password, $item );
					$row      = Flowbie_App_Wp_Overview_Seo_Item::finalize_with_optional_acf( $normalized, $username, $app_password, $item, $core_row );
				}
