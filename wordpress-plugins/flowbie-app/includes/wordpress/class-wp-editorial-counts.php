...
class Flowbie_App_Wp_Editorial_Counts {
	...
	public static function get_quarter_editorial_counts( $body ) {
		...
		$secondary        = Flowbie_App_Wp_Url_Normalize::resolve_secondary_rest_collection(
	...
	private static function fetch_collection_status_total( $ctx, $collection, $status ) {
		...
		if ( $resp['is_wp_error'] ) {
			$msg = Flowbie_App_Wp_Rest_Client::transport_error_message( $resp );
		...
		if ( Flowbie_App_Wp_Url_Normalize::rest_looks_like_siteground_captcha( $status_code, $resp['body'] ) ) {
			return array(
				'ok'         => false,
				'error'      => Flowbie_App_Wp_Url_Normalize::SITEGROUND_REST_BLOCKED_MESSAGE,
				'siteground' => true,
			);
		}
