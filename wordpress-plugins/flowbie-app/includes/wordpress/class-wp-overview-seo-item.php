...
class Flowbie_App_Wp_Overview_Seo_Item {
	...
	public static function build_core_put_body( $item ) {
		...
		if ( ! empty( $item['postTitle'] ) && is_string( $item['postTitle'] ) && trim( $item['postTitle'] ) !== '' ) {
			$put['title'] = Flowbie_App_Wp_Url_Normalize::clean_placeholders_and_markdown( trim( $item['postTitle'] ) );
		}
		if ( ! empty( $item['postExcerpt'] ) && is_string( $item['postExcerpt'] ) && trim( $item['postExcerpt'] ) !== '' ) {
			$put['excerpt'] = Flowbie_App_Wp_Url_Normalize::clean_placeholders_and_markdown( trim( $item['postExcerpt'] ) );
		}
	...
	public static function wp_put_error_message( $resp ) {
		...
		if ( Flowbie_App_Wp_Url_Normalize::rest_looks_like_cloudflare_challenge( $data ) ) {
			return Flowbie_App_Wp_Url_Normalize::CLOUDFLARE_REST_BLOCKED_MESSAGE;
		}
		if ( is_string( $data ) ) {
			$trim = trim( $data );
