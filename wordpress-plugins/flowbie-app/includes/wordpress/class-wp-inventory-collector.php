...
class Flowbie_App_Wp_Inventory_Collector {
	...
	public static function collect( $normalized, $username, $app_password, $rest_collection, $include_content, $include_raw_acf, $include_page_heading, $wp_status, $include_ids, $max_rows ) {
		...
		while ( $has_more ) {
			...
			if ( Flowbie_App_Wp_Url_Normalize::rest_looks_like_siteground_captcha( (int) $resp['status'], $resp['body'] ) ) {
				return array( 'ok' => false, 'error' => Flowbie_App_Wp_Url_Normalize::SITEGROUND_REST_BLOCKED_MESSAGE, 'siteground' => true );
			}
	...
	private static function map_inventory_row( $post, $normalized, $include_content, $include_raw_acf, $include_page_heading ) {
		...
		$fields = array(
			'title'   => trim( wp_strip_all_tags( Flowbie_App_Wp_Url_Normalize::rendered_text( $post['title'] ?? '' ) ) ),
			'meta'    => self::meta_from_acf( $acf ),
			'keyword' => isset( $acf['keyword_focus'] ) ? trim( (string) $acf['keyword_focus'] ) : '',
		);
		$excerpt = trim( wp_strip_all_tags( Flowbie_App_Wp_Url_Normalize::rendered_text( $post['excerpt'] ?? '' ) ) );
		if ( $excerpt !== '' ) {
