...
class Flowbie_App_Wp_Media {
	...
	public static function upload_media( $body ) {
		...
		$headers = Flowbie_App_Wp_Rest_Client::auth_headers(
			$username,
			$app_password,
			array(
		...
		$data   = Flowbie_App_Wp_Rest_Client::parse_json_body( $raw );
		...
		if ( $alt !== '' && $media_id > 0 ) {
			Flowbie_App_Wp_Rest_Client::request(
				'POST',
		...
		$media_title = Flowbie_App_Wp_Url_Normalize::rendered_text( $data['title'] ?? '' );
		if ( $media_title === '' ) {
	...
	public static function list_media( $body ) {
		...
		while ( count( $catalog ) < $max_items ) {
			...
			foreach ( $resp['body'] as $item ) {
				...
				$item_title   = Flowbie_App_Wp_Url_Normalize::rendered_text( $item['title'] ?? '' );
				$item_caption = Flowbie_App_Wp_Url_Normalize::rendered_text( $item['caption'] ?? '' );
				$catalog[]    = array(
					'id'        => (int) ( $item['id'] ?? 0 ),
