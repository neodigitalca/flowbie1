<?php
/**
 * Flowbie WP plugin tools proxy (flowbie-wp-tools-routes.js).
 *
 * @package Flowbie_App
 */
...
class Flowbie_App_Wp_Flowbie_Tools {
	 ...
	 */
	public static function flowbie_wp_tool( $body ) {
		...
		$url        = $normalized . '/wp-json/flowbie/v1/tools/execute';
	 ...
	 * @return array{0:int,1:array<string,mixed>}
	 */
	public static function flowbie_wp_tools_list( $body ) {
		$site_url     = isset( $body['siteUrl'] ) ? (string) $body['siteUrl'] : '';
		$username     = isset( $body['username'] ) ? (string) $body['username'] : '';
		$app_password = isset( $body['appPassword'] ) ? (string) $body['appPassword'] : '';
		...
		$normalized = Flowbie_App_Wp_Url_Normalize::normalize_url( $site_url );
		$url        = $normalized . '/wp-json/flowbie/v1/tools/list';
		$resp       = Flowbie_App_Wp_Rest_Client::request(
			'POST',
			$url,
			$username,
	...
	public static function execute_tool( $normalized, $username, $app_password, $tool, $params = array() ) {
		...
		list( , $data ) = self::flowbie_wp_tool( $body );
		return $data;
	}
	...
	public static function ping_public( $normalized ) {
		$url  = Flowbie_App_Wp_Url_Normalize::normalize_url( $normalized ) . '/wp-json/flowbie/v1/ping';
		$resp = wp_remote_get(
			$url,
			array(
				'timeout' => 8,
	...
	private static function format_tools_http_error( $status, $data ) {
		...
		if ( ! empty( $data['error'] ) && is_string( $data['error'] ) ) {
			...
			$raw = 'Flowbie WP tool failed (HTTP ' . (int) $status . ')';
		}
		if ( (int) $status === 404 && ( $code === 'rest_no_route' || strpos( $raw, 'No route was found' ) !== false ) ) {
			return 'Flowbie WP plugin is not installed or not active on this site.';
		}
		return $raw;
	}
}
