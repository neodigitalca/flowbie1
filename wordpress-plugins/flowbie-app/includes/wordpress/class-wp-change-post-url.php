...
class Flowbie_App_Wp_Change_Post_Url {
	...
	public static function change( $body ) {
		...
		$flowbie_url = $normalized . '/wp-json/flowbie/v1/ai/change-url';

		$flowbie_res = Flowbie_App_Wp_Rest_Client::request(
			'POST',
			$flowbie_url,
			$username,
			$app_password,
		...
		if ( ! $flowbie_res['is_wp_error'] ) {
			$st = (int) $flowbie_res['status'];
			if ( $st >= 200 && $st < 300 ) {
				$data = is_array( $flowbie_res['body'] ) ? $flowbie_res['body'] : array();
				return array( 200, array_merge( array( 'ok' => true, 'method' => 'flowbie' ), $data ) );
			}
			if ( $st !== 404 ) {
				$err_data = is_array( $flowbie_res['body'] ) ? $flowbie_res['body'] : array();
				$err_msg  = '';
				if ( ! empty( $err_data['message'] ) && is_string( $err_data['message'] ) ) {
					...
					$err_msg = 'Flowbie change-url failed (HTTP ' . $st . ')';
				}
				return array(
					$st,
					array(
		...
		$core_res = Flowbie_App_Wp_Rest_Client::request(
			'PUT',
			$api_url,
			$username,
			$app_password,
