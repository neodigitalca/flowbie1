...
class Flowbie_App_Wp_Connection {
	...
	public static function test_connection( $body ) {
		...
		$response = Flowbie_App_Wp_Rest_Client::request(
			'GET',
			$api_url,
			$username,
			$app_password,
		...
		$transport = Flowbie_App_Wp_Rest_Client::transport_error_message( $response );
		if ( $transport ) {
		...
		if ( $status === 200 ) {
			...
			$site_resp = Flowbie_App_Wp_Rest_Client::request(
				'GET',
				$normalized . '/wp-json/',
				$username,
				$app_password,
	...
	private static function probe_capabilities( $normalized_url, $username, $app_password, $cookie ) {
		$ping = Flowbie_App_Wp_Rest_Client::request(
			'GET',
			$normalized_url . '/wp-json/flowbie/v1/ping',
			$username,
			$app_password,
			array(
				'timeout' => 8,
		...
		$flowbie_ok = ! $ping['is_wp_error'] && (int) $ping['status'] >= 200 && (int) $ping['status'] < 300;

		$types_resp = Flowbie_App_Wp_Rest_Client::request(
			'GET',
