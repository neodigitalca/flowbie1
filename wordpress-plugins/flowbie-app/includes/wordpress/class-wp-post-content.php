...
class Flowbie_App_Wp_Post_Content {
	...
	public static function get_post_content( $body ) {
		...
		$use_plugin   = ! empty( $body['useFlowbiePluginEndpoint'] ) &&
			in_array( $body['useFlowbiePluginEndpoint'], array( true, 'true', 1, '1' ), true );
		...
		$hint_sm     = Flowbie_App_Wp_Url_Normalize::endpoint_from_entity_sitemap_url( $entity_sm );
	...
	private static function fetch_via_plugin( $normalized, $username, $app_password, $resolved, $post_ids, $slugs ) {
		...
		if ( $slugs && ! $queue ) {
			return array( 400, array( 'error' => 'useFlowbiePluginEndpoint requires post IDs, not slugs only.' ) );
		}
		if ( ! $queue ) {
			return array( 400, array( 'error' => 'useFlowbiePluginEndpoint requires at least one post ID.' ) );
		}
		...
		foreach ( $queue as $obj ) {
			...
			$resp = Flowbie_App_Wp_Rest_Client::request(
				'GET',
				$normalized . '/wp-json/flowbie/v1/post-content/' . $id,
				$username,
				$app_password,
				array(
					'timeout'      => 25,
			...
			if ( $st === 404 ) {
				$errors[] = array( 'resolvedObject' => $obj, 'error' => 'Post not found at Flowbie plugin REST route (ID ' . $id . ').' );
				continue;
			}
			...
			$posts[] = array(
				'id'               => (int) $d['id'],
				'slug'             => (string) ( $d['slug'] ?? '' ),
				'title'            => is_string( $d['title'] ?? null ) ? $d['title'] : Flowbie_App_Wp_Url_Normalize::rendered_text( $d['title'] ?? '' ),
				'content'          => is_string( $d['content'] ?? null ) ? $d['content'] : Flowbie_App_Wp_Url_Normalize::rendered_text( $d['content'] ?? '' ),
				'excerpt'          => is_string( $d['excerpt'] ?? null ) ? $d['excerpt'] : Flowbie_App_Wp_Url_Normalize::rendered_text( $d['excerpt'] ?? '' ),
				...
				'postTypeEndpoint' => (string) ( $d['postTypeEndpoint'] ?? 'flowbie_plugin' ),
	...
	private static function map_post( $post, $endpoint, $normalized ) {
		...
		return array(
			...
			'content'          => Flowbie_App_Wp_Url_Normalize::rendered_text( $post['content'] ?? '' ),
