...
class Flowbie_App_Wp_Acf_By_Url {
	...
	public static function get_acf_fields_by_url_batch( $body ) {
		...
		$batch_max    = Flowbie_App_Wp_Acf_Protocol::BATCH_MAX;
	...
	public static function fetch( $normalized, $username, $app_password, $url, $endpoint_hint, $include_snapshot ) {
		...
		foreach ( $endpoints as $endpoint ) {
			...
			$search_rs  = Flowbie_App_Wp_Rest_Client::request( 'GET', $search_url, $username, $app_password, array( 'timeout' => 30 ) );
		...
		$resp    = Flowbie_App_Wp_Rest_Client::request( 'GET', $get_url, $username, $app_password, array( 'timeout' => 30 ) );
		$fields  = array();
		$out     = array(
			'success'              => true,
			'fields'               => array(),
		...
		if ( ! $resp['is_wp_error'] && (int) $resp['status'] === 200 && is_array( $resp['body'] ) ) {
			...
			if ( $include_snapshot ) {
				...
				$out['postSnapshot'] = array(
					...
					'slug'             => (string) ( $d['slug'] ?? '' ),
					'title'            => Flowbie_App_Wp_Url_Normalize::rendered_text( $d['title'] ?? null ),
					'content'          => Flowbie_App_Wp_Url_Normalize::rendered_text( $d['content'] ?? null ),
					'excerpt'          => Flowbie_App_Wp_Url_Normalize::rendered_text( $d['excerpt'] ?? null ),
