...
class Flowbie_App_Teams_Store {
	...
	public static function install_tables(): void {
		...
		$users   = $wpdb->prefix . 'flowbie_users';
		$teams   = $wpdb->prefix . 'flowbie_teams';
		$members = $wpdb->prefix . 'flowbie_team_members';
		$invites = $wpdb->prefix . 'flowbie_team_invites';
		$presets = $wpdb->prefix . 'flowbie_team_job_title_presets';
	...
	public static function get_user_by_id( int $user_id ): ?array {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT id, email, display_name, avatar_url, created_at FROM ' . $wpdb->prefix . 'flowbie_users WHERE id = %d',
				$user_id
			),
			ARRAY_A
		);
	...
	public static function get_user_by_email( string $email ): ?array {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT * FROM ' . $wpdb->prefix . 'flowbie_users WHERE email = %s',
	...
	private static function unique_slug( string $base ): string {
		...
		$slug = $base;
		$i    = 2;
		while ( $wpdb->get_var( $wpdb->prepare( 'SELECT id FROM ' . $wpdb->prefix . 'flowbie_teams WHERE slug = %s', $slug ) ) ) {
			$slug = $base . '-' . $i;
			++$i;
		}
		return $slug;
	...
	public static function list_teams_for_user( int $user_id ): array {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				...
				FROM ' . $wpdb->prefix . 'flowbie_teams t
				INNER JOIN ' . $wpdb->prefix . 'flowbie_team_members m ON m.team_id = t.id
	...
	public static function get_team( int $team_id ): ?array {
		global $wpdb;
		$row = $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'flowbie_teams WHERE id = %d', $team_id ),
			ARRAY_A
		);
		return is_array( $row ) ? $row : null;
	}
	...
	public static function count_active_members( int $team_id ): int {
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				'SELECT COUNT(*) FROM ' . $wpdb->prefix . 'flowbie_team_members WHERE team_id = %d AND status = %s',
				$team_id,
				'active'
			)
		);
	...
	public static function get_membership( int $team_id, int $user_id ): ?array {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT * FROM ' . $wpdb->prefix . 'flowbie_team_members WHERE team_id = %d AND user_id = %d AND status = %s',
				$team_id,
				$user_id,
				'active'
			),
	...
	private static function seed_job_title_presets( int $team_id ): void {
		...
		foreach ( $titles as $i => $title ) {
			$wpdb->insert(
				$wpdb->prefix . 'flowbie_team_job_title_presets',
				array(
					'team_id'    => $team_id,
					'title'      => $title,
					'sort_order' => $i,
	...
	public static function ensure_job_title_presets( int $team_id ): void {
		...
		$table = $wpdb->prefix . 'flowbie_team_job_title_presets';
		foreach ( $titles as $i => $title ) {
			$exists = $wpdb->get_var(
				$wpdb->prepare(
	...
	/**
	 * Add an existing Flowbie user to a team, or reactivate a removed member.
	 *
	 * @return array<string,mixed>
	 */
	public static function add_member( int $team_id, int $user_id, string $access_role, string $job_title ): array {
		global $wpdb;
		$table = $wpdb->prefix . 'flowbie_team_members';
	...
	/**
	 * Create or reuse a Flowbie user and add them to a team (no email).
	 *
	...
	public static function list_members( int $team_id ): array {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				...
				FROM ' . $wpdb->prefix . 'flowbie_team_members m
				INNER JOIN ' . $wpdb->prefix . 'flowbie_users u ON u.id = m.user_id
		...
		foreach ( $rows as $row ) {
			...
			$out[]   = array(
				...
				'isBot'       => class_exists( 'Flowbie_App_Chat_Flo' ) && Flowbie_App_Chat_Flo::is_flo( (int) $row['user_id'] ),
			);
		}
		return $out;
	}
	...
	public static function team_workspace_path( int $team_id ): string {
		return Flowbie_App_Data_Paths::subdir( 'teams/' . (string) $team_id ) . '/workspace.json';
	}
}
