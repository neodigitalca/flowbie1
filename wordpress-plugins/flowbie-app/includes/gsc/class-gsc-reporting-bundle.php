...
class Flowbie_App_Gsc_Reporting_Bundle {
	...
	public static function fetch_reporting_bundle( array $body ): array {
		...
		$date_validation = Flowbie_App_Gsc_Service_Account::validate_dates( $body['startDate'], $body['endDate'] );
		...
		if ( $has_compare ) {
			$cmp_val = Flowbie_App_Gsc_Service_Account::validate_dates(
				trim( (string) $body['compareStartDate'] ),
				trim( (string) $body['compareEndDate'] )
			);
		...
		foreach ( $candidates as $i => $property ) {
			$probe = Flowbie_App_Gsc_Service_Account::search_analytics_query(
				$property,
				array(
					'startDate'  => $start_str,
					'endDate'    => $end_str,
		...
		$q_res = Flowbie_App_Gsc_Service_Account::search_analytics_query(
			$successful_property,
			array(
				'startDate'  => $start_str,
				'endDate'    => $end_str,
		...
		$pages       = array();
		$pages_error = null;
		$p_res       = Flowbie_App_Gsc_Service_Account::search_analytics_query(
			$successful_property,
			array(
				'startDate'  => $start_str,
				'endDate'    => $end_str,
		...
		if ( $has_compare && $compare_start && $compare_end ) {
			$cq_res = Flowbie_App_Gsc_Service_Account::search_analytics_query(
				$successful_property,
				array(
			...
			$cp_res = Flowbie_App_Gsc_Service_Account::search_analytics_query(
				$successful_property,
				array(
					'startDate'  => $compare_start,
					'endDate'    => $compare_end,
		...
		$sitemaps       = null;
		$sitemaps_error = null;
		$sm_res         = Flowbie_App_Gsc_Service_Account::list_sitemaps( $successful_property );
