...
class Flowbie_App_Gsc_Service_Account {
	...
	public static function get_credentials() {
		...
		$raw = Flowbie_App_Secrets::gsc_service_account_json();
		if ( $raw === '' ) {
			return new WP_Error(
				'flowbie_gsc_missing',
		...
		if ( ! is_array( $data ) ) {
			return new WP_Error( 'flowbie_gsc_bad_json', 'GSC service account JSON is invalid.' );
		}
	...
	public static function get_access_token( bool $require_write = false ) {
		...
		$cache_key = 'flowbie_app_gsc_token_' . md5( $creds['client_email'] . '|' . $scope . '|' . FLOWBIE_APP_VERSION );
	...
	private static function build_jwt( string $client_email, string $private_key, string $scope ) {
		...
		if ( false === $key ) {
			return new WP_Error( 'flowbie_gsc_key', 'Could not read GSC service account private key.' );
		}
		...
		if ( ! $signed ) {
			return new WP_Error( 'flowbie_gsc_sign', 'Could not sign GSC JWT.' );
		}
	...
	public static function find_matching_property( string $site_url ): array {
		...
		$cache_key = 'flowbie_app_gsc_prop_' . md5( $requested );
		$cached    = get_transient( $cache_key );
		if ( is_array( $cached ) ) {
			return $cached;
		}
	...
	public static function list_sites() {
		...
		if ( $code < 200 || $code >= 300 ) {
			$msg = is_array( $data ) && ! empty( $data['error']['message'] )
				...
				: ( $raw !== '' ? $raw : 'HTTP ' . $code );
			return new WP_Error( 'flowbie_gsc_sites', $msg, array( 'status' => $code ) );
		}
	...
	public static function search_analytics_query( string $property, array $body ) {
		...
		if ( $code < 200 || $code >= 300 ) {
			$msg = is_array( $data ) && ! empty( $data['error']['message'] )
				? (string) $data['error']['message']
				: ( $raw !== '' ? $raw : 'HTTP ' . $code );
			return new WP_Error( 'flowbie_gsc_query', $msg, array( 'status' => $code ) );
		}
	...
	public static function list_sitemaps( string $property ) {
		...
		if ( $code < 200 || $code >= 300 ) {
			$msg = is_array( $data ) && ! empty( $data['error']['message'] )
				? (string) $data['error']['message']
				: ( $raw !== '' ? $raw : 'HTTP ' . $code );
			return new WP_Error( 'flowbie_gsc_sitemaps', $msg, array( 'status' => $code ) );
		}
	...
	public static function write_report_date_range( array $range ): void {
		$path = Flowbie_App_Data_Paths::file( 'gsc', 'report-date-range.json' );
		Flowbie_App_Json_File_Store::write( $path, $range );
	}
