...
class Flowbie_App_Chekkit_Webhook {
	...
	private static function handle_inbound_message( array $body ): void {
		$dir  = Flowbie_App_Data_Paths::subdir( 'chekkit-messages' );
	...
	private static function handle_contact_submission( array $body ): void {
		...
		$form_email = trim( Flowbie_App_Secrets::chekkit_form_email() );
		if ( $form_email === '' ) {
			self::send_json(
				array(
					'success' => false,
		...
		register_shutdown_function(
			static function () use ( $form_email, $subject, $body_text ) {
				Flowbie_App_Chekkit_Webhook::send_chekkit_form_email( $form_email, $subject, $body_text );
			}
		);
	...
	private static function send_json( array $data, int $status ): void {
		...
		if ( defined( 'FLOWBIE_APP_VERSION' ) ) {
			header( 'X-Flowbie-Webhook-Version: ' . FLOWBIE_APP_VERSION );
		}
