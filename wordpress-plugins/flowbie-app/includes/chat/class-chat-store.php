...
class Flowbie_App_Chat_Store {
	...
	public static function install_tables(): void {
		...
		$channels = $wpdb->prefix . 'flowbie_chat_channels';
		$members  = $wpdb->prefix . 'flowbie_chat_channel_members';
		$messages = $wpdb->prefix . 'flowbie_chat_messages';
		$read     = $wpdb->prefix . 'flowbie_chat_read_state';
		...
		$assets = $wpdb->prefix . 'flowbie_chat_assets';
		$previews = $wpdb->prefix . 'flowbie_chat_link_previews';
		$activity = $wpdb->prefix . 'flowbie_chat_activity_log';
		...
		$thread_read = $wpdb->prefix . 'flowbie_chat_thread_read_state';
		dbDelta(
		...
		Flowbie_App_Chat_Typing::install_table();
		Flowbie_App_Chat_Calls::install_tables();
		Flowbie_App_Chat_Mentions::install_tables();
		Flowbie_App_Chat_Preferences::install_tables();
	}

	public static function ensure_default_channels( int $team_id, int $creator_user_id ): void {
		global $wpdb;
		$table = $wpdb->prefix . 'flowbie_chat_channels';
	...
	public static function get_channel( int $channel_id ): ?array {
		global $wpdb;
		$row = $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'flowbie_chat_channels WHERE id = %d', $channel_id ),
			ARRAY_A
		);
	...
	public static function user_can_access_channel( int $channel_id, int $team_id, int $user_id ): bool {
		...
		if ( $type === 'public' ) {
			return Flowbie_App_Teams_Store::get_membership( $team_id, $user_id ) !== null;
		}
		global $wpdb;
		$exists = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT id FROM ' . $wpdb->prefix . 'flowbie_chat_channel_members WHERE channel_id = %d AND user_id = %d',
				$channel_id,
				$user_id
			)
		);
	...
	public static function get_or_create_dm( int $team_id, int $user_id, int $other_user_id ): ?array {
		...
		if ( ! Flowbie_App_Teams_Store::get_membership( $team_id, $other_user_id ) ) {
			return null;
		}
	...
	public static function format_channel_payload( array $row, int $user_id, int $team_id ): array {
		...
		if ( $type === 'dm' ) {
			...
			if ( $other ) {
				...
				$user       = Flowbie_App_Teams_Store::get_user_by_id( $dm_user_id );
	...
	private static function get_last_message( int $channel_id ): ?array {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT * FROM ' . $wpdb->prefix . 'flowbie_chat_messages WHERE channel_id = %d AND deleted_at IS NULL ORDER BY id DESC LIMIT 1',
				$channel_id
			),
			ARRAY_A
		);
	...
	public static function count_unread( int $channel_id, int $user_id ): int {
		global $wpdb;
		$read_table = $wpdb->prefix . 'flowbie_chat_read_state';
		$msg_table  = $wpdb->prefix . 'flowbie_chat_messages';
	...
	public static function mark_read( int $channel_id, int $user_id, int $message_id ): void {
		global $wpdb;
		$table = $wpdb->prefix . 'flowbie_chat_read_state';
	...
	public static function list_messages(
		...
		global $wpdb;
		$table = $wpdb->prefix . 'flowbie_chat_messages';
		...
		$attachments = $team_id > 0 ? Flowbie_App_Chat_Assets::get_for_messages( $message_ids, $team_id ) : array();
		$previews    = Flowbie_App_Chat_Link_Unfurl::get_for_messages( $message_ids );
	...
	public static function create_message(
		...
		global $wpdb;
		$body_html  = Flowbie_App_Chat_Mentions::sanitize_chat_body_html( $body_html );
		...
		$wpdb->insert(
			$wpdb->prefix . 'flowbie_chat_messages',
			$data,
			$formats
		);
		...
		$row = $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'flowbie_chat_messages WHERE id = %d', $id ),
			ARRAY_A
		);
		if ( ! is_array( $row ) ) {
			return null;
		...
		$channel = self::get_channel( $channel_id );
		if ( $channel ) {
			Flowbie_App_Chat_Mentions::sync_for_message(
				$id,
				$channel_id,
				(int) $channel['team_id'],
				$user_id,
	...
	private static function enrich_message_payload(
		...
		if ( count( $attachment_asset_ids ) > 0 ) {
			Flowbie_App_Chat_Assets::link_to_message( $team_id, $channel_id, $message_id, $user_id, $attachment_asset_ids );
		}
		if ( $is_new || ! empty( $payload['bodyHtml'] ) ) {
			Flowbie_App_Chat_Link_Unfurl::process_message( $message_id, (string) $payload['bodyHtml'], $channel_id, $team_id, $user_id );
		}

		$attachments = Flowbie_App_Chat_Assets::get_for_messages( array( $message_id ), $team_id );
		$previews    = Flowbie_App_Chat_Link_Unfurl::get_for_messages( array( $message_id ) );
	...
	public static function get_message( int $message_id ): ?array {
		global $wpdb;
		$row = $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'flowbie_chat_messages WHERE id = %d', $message_id ),
			ARRAY_A
		);
		return is_array( $row ) ? $row : null;
	}
	...
	public static function edit_message(
		...
		$body_plain = self::html_to_plain( $body_html );
		if ( $body_plain === '' && count( $attachment_asset_ids ) === 0 ) {
			$existing = Flowbie_App_Chat_Assets::get_for_messages( array( $message_id ), (int) ( self::get_channel( $channel_id )['team_id'] ?? 0 ) );
		...
		Flowbie_App_Chat_Link_Unfurl::clear_for_message( $message_id );
	...
	public static function delete_message( int $message_id, int $user_id, bool $is_admin ): bool {
		...
		$channel_id = (int) $row['channel_id'];
		if ( $team_id > 0 ) {
			Flowbie_App_Chat_Assets::soft_delete_for_message( $team_id, $channel_id, $message_id, (int) $row['user_id'] );
			Flowbie_App_Chat_Activity_Log::purge_shared_for_message( $team_id, $channel_id, $message_id, (int) $row['user_id'] );
			Flowbie_App_Chat_Link_Unfurl::clear_for_message( $message_id );
			Flowbie_App_Chat_Mentions::purge_for_message( $message_id );
		}
		global $wpdb;
	...
	public static function add_channel_members( int $channel_id, array $member_user_ids ): void {
		global $wpdb;
		$table = $wpdb->prefix . 'flowbie_chat_channel_members';
	...
	public static function patch_channel( int $channel_id, array $updates ): bool {
		...
		return (bool) $wpdb->update(
			$wpdb->prefix . 'flowbie_chat_channels',
			$allowed,
			array( 'id' => $channel_id ),
			$formats,
			array( '%d' )
	...
	public static function mark_thread_read( int $channel_id, int $user_id, int $thread_root_id, int $message_id ): void {
		global $wpdb;
		$table = $wpdb->prefix . 'flowbie_chat_thread_read_state';
		$existing = $wpdb->get_var(
			$wpdb->prepare(
	...
	public static function threads_with_unread( int $channel_id, int $user_id ): array {
		global $wpdb;
		$msg_table   = $wpdb->prefix . 'flowbie_chat_messages';
		$read_table  = $wpdb->prefix . 'flowbie_chat_thread_read_state';
