...
class Flowbie_App_Chat_Mentions {
	...
	public static function install_tables(): void {
		...
		$table   = $wpdb->prefix . 'flowbie_chat_mentions';
	...
	public static function sync_for_message(
		...
		foreach ( $ids as $uid ) {
			...
			if ( ! Flowbie_App_Teams_Store::get_membership( $team_id, $uid ) ) {
				continue;
			}
			if ( ! Flowbie_App_Chat_Store::user_can_access_channel( $channel_id, $team_id, $uid ) ) {
				continue;
			}
			$row_data = array(
				'team_id'           => $team_id,
	...
	public static function purge_for_message( int $message_id ): void {
		global $wpdb;
		$wpdb->delete(
			$wpdb->prefix . 'flowbie_chat_mentions',
	...
	public static function count_unread( int $team_id, int $user_id ): int {
		...
		$count = $wpdb->get_var(
			$wpdb->prepare(
				...
				INNER JOIN {$wpdb->prefix}flowbie_chat_messages msg ON msg.id = m.message_id AND msg.deleted_at IS NULL
	...
	public static function list_for_user( int $team_id, int $user_id, int $limit = 50, bool $unread_only = false ): array {
		...
		foreach ( $rows as $row ) {
			...
			$author = Flowbie_App_Teams_Store::get_user_by_id( (int) $row['author_user_id'] );
	...
	private static function channel_label( array $channel, int $user_id, int $team_id ): string {
		$formatted = Flowbie_App_Chat_Store::format_channel_payload( $channel, $user_id, $team_id );
