<?php
/**
 * Team chat file assets (flowbie-data storage).
 *
 * @package Flowbie_App
 */
...
class Flowbie_App_Chat_Assets {
	...
	public static function upload(
		...
		$dir = Flowbie_App_Data_Paths::subdir( 'chat/teams/' . $team_id );
		...
		global $wpdb;
		$table = $wpdb->prefix . 'flowbie_chat_assets';
		$wpdb->insert(
			$table,
			array(
				'team_id'       => $team_id,
	...
	public static function link_to_message(
		...
		foreach ( $asset_ids as $asset_id ) {
			...
			Flowbie_App_Chat_Activity_Log::append(
				$team_id,
				$channel_id,
				array(
					'kind'                => 'file_shared',
					...
					'bytes'               => (int) $row['bytes'],
					'threadRootMessageId' => Flowbie_App_Chat_Store::activity_thread_root( $message_id ),
				)
			);
		}
	}
	...
	public static function delete_asset( int $team_id, int $channel_id, int $asset_id, int $user_id, bool $is_admin ): ?array {
		...
		Flowbie_App_Chat_Activity_Log::append(
			...
			array(
				...
				'mime'                => (string) $row['mime'],
				'bytes'               => (int) $row['bytes'],
				'threadRootMessageId' => $message_id > 0 ? Flowbie_App_Chat_Store::activity_thread_root( $message_id ) : 0,
			)
		);
		return array(
			'assetId'  => $asset_id,
	...
	public static function get_row( int $asset_id ): ?array {
		global $wpdb;
		$row = $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'flowbie_chat_assets WHERE id = %d', $asset_id ),
			ARRAY_A
		);
	...
	public static function serve_download( int $team_id, int $channel_id, int $asset_id, int $user_id ): ?array {
		...
		$path = Flowbie_App_Data_Paths::root() . '/' . ltrim( (string) $row['storage_path'], '/' );
		if ( ! is_readable( $path ) ) {
			return null;
		}
		return array(
	...
	public static function download_url( int $team_id, int $channel_id, int $asset_id ): string {
		return rest_url( 'flowbie-app/v1/teams/' . $team_id . '/chat/channels/' . $channel_id . '/files/' . $asset_id );
	}
