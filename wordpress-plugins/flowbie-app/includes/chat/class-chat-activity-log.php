...
class Flowbie_App_Chat_Activity_Log {
	...
	public static function append( int $team_id, int $channel_id, array $payload ): ?int {
		...
		$table = $wpdb->prefix . 'flowbie_chat_activity_log';
		$wpdb->insert(
			$table,
			array(
				'team_id'      => $team_id,
	...
	public static function search_shared( int $team_id, int $user_id, array $filters ): array {
		...
		if ( $channel > 0 ) {
			if ( ! Flowbie_App_Chat_Store::user_can_access_channel( $channel, $team_id, $user_id ) ) {
				return array();
			}
		...
		foreach ( $rows as $row ) {
			...
			if ( $channel <= 0 && ! Flowbie_App_Chat_Store::user_can_access_channel( $channel_id, $team_id, $user_id ) ) {
				continue;
			}
			$entry = self::format_row( $row );
			if ( ! $entry ) {
			...
			$channel_row = Flowbie_App_Chat_Store::get_channel( $channel_id );
			if ( $channel_row ) {
				$formatted = Flowbie_App_Chat_Store::format_channel_payload( $channel_row, $user_id, $team_id );
				$entry['channelName'] = (string) ( $formatted['slug'] ?? $formatted['name'] ?? '' );
			}
			$sharer = Flowbie_App_Teams_Store::get_user_by_id( (int) ( $entry['userId'] ?? 0 ) );
			$entry['sharerDisplayName'] = $sharer ? (string) $sharer['display_name'] : 'Unknown';
			if ( $thread_id > 0 ) {
				$root_msg = Flowbie_App_Chat_Store::get_message( $thread_id );
				if ( $root_msg ) {
	...
	private static function should_include_shared_entry( array $entry ): bool {
		...
		if ( $kind === 'file_shared' ) {
			...
			if ( $asset_id > 0 ) {
				$asset = Flowbie_App_Chat_Assets::get_row( $asset_id );
