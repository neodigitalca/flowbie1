...
class Flowbie_App_Proposal_Route_Handlers {
	...
	public static function run_site_audit( array $body ): array {
		...
		if ( count( $urls ) > Flowbie_App_Proposal_Site_Audit::MAX_URLS ) {
			return array(
				'status' => 400,
				'data'   => array(
					'success' => false,
					'error'   => 'Maximum ' . Flowbie_App_Proposal_Site_Audit::MAX_URLS . ' URLs per audit',
				),
			);
		}

		try {
			$result = Flowbie_App_Proposal_Site_Audit::run( $urls );
			return array(
				'status' => 200,
