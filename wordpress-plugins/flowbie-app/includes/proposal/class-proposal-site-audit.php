...
class Flowbie_App_Proposal_Site_Audit {
	...
	public static function run( array $urls ): array {
		...
		foreach ( $pages as $page ) {
			...
			foreach ( array( 'desktop', 'mobile' ) as $device ) {
				...
				$metrics = Flowbie_App_Proposal_Lighthouse_Parse::parse_page_metrics( $res, $url, $device );
				if ( $metrics === null ) {
					$errors[] = array(
						'url'     => $url,
						'step'    => 'lighthouse_' . $device,
		...
		$performance = Flowbie_App_Proposal_Lighthouse_Parse::build_performance_summary( $desktop_metrics, $mobile_metrics );
		...
		if ( $has_excerpt ) {
			try {
				$faq = Flowbie_App_Proposal_Faq_Inventory::run( $faq_inputs );
			} catch ( Exception $e ) {
				$errors[] = array(
					'url'     => '(all)',
					'step'    => 'faq_inventory',
