...
class Flowbie_App_Vertical_Benchmark_Gsc_Export {
	...
	public static function export_rows( array $sites, array $opts = array() ): array {
		...
		Flowbie_App_Json_File_Store::write(
			Flowbie_App_Data_Paths::vertical_benchmark_export_path(),
			array(
				'rows'          => $rows,
	...
	private static function collect_site_rows( array $site, array $opts ): array {
		...
		$custom = Flowbie_App_Vertical_Benchmark_Client_Tag::resolve_custom( $site );
		$client_tag_label = $opts['clientTagLabel'] ?: ( $custom['clientTagLabel'] ?? Flowbie_App_Vertical_Benchmark_Taxonomy::label( (string) ( $site['industryVertical'] ?? 'uncategorized' ) ) );
		$client_tag       = $opts['clientTag'] ?: ( $custom['clientTagLabel'] ?? $client_tag_label );

		$gsc = Flowbie_App_Gsc_Performance_Batch::top_pages(
			array(
				'siteUrl'   => $site_url,
		...
		$sets      = Flowbie_App_Vertical_Benchmark_Sitemap_Urls::resolve_url_sets( $site );
		...
		foreach ( $gsc_pages as $p ) {
			...
			if ( Flowbie_App_Vertical_Benchmark_Sitemap_Urls::classify_url_by_sitemap( $url, $sets['postPaths'], $sets['entityPaths'] ) === 'unknown' ) {
...
post = blog/article content. entity = service area / location landing pages. exclude = homepage, category archives, contact, shop, or non-content URLs.';
			...
			$parsed = Flowbie_App_Vertical_Benchmark_Openrouter::json_completion(
				array(
	...
	private static function bucket_gsc_pages( array $gsc_pages, string $kind, array $post_paths, array $entity_paths, array $gemini_kinds, int $max_n ): array {
		...
		foreach ( $gsc_pages as $p ) {
			...
			$label = Flowbie_App_Vertical_Benchmark_Sitemap_Urls::classify_url_by_sitemap( $url, $post_paths, $entity_paths );
			if ( $label === 'unknown' ) {
