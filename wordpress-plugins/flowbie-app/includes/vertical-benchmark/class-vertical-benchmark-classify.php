...
class Flowbie_App_Vertical_Benchmark_Classify {
	...
	public static function classify_client_tags_batch( array $sites, array $opts = array() ): array {
		...
		$system = 'You assign each WordPress client to exactly one industry vertical id from this list (reply JSON only):' . "\n"
			. Flowbie_App_Vertical_Benchmark_Taxonomy::list_for_prompt() . "\n\n"
			. Flowbie_App_Vertical_Benchmark_Taxonomy::classify_rules() . "\n\n"
		...
		foreach ( $need_gemini as $site ) {
			$id  = (string) ( $site['id'] ?? '' );
			$hit = $by_id[ $id ] ?? null;
			$resolved = Flowbie_App_Vertical_Benchmark_Client_Tag::resolve_taxonomy( (string) ( $hit['industryVertical'] ?? 'uncategorized' ) );
			$results[] = array(
				'siteId'         => $id,
