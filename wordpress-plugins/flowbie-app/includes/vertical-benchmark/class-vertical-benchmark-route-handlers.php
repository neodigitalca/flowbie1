...
class Flowbie_App_Vertical_Benchmark_Route_Handlers {
	...
	public static function dispatch_http( string $subpath, string $method, array $body ): void {
		...
		if ( $subpath === 'taxonomy' && $method === 'GET' ) {
			Flowbie_App_Api_Dispatcher::send_json(
				array(
					'ok'       => true,
					'taxonomy' => Flowbie_App_Vertical_Benchmark_Taxonomy::TAXONOMY,
				)
			);
			return;
		}
	...
	private static function classify_clients( array $body ): void {
		try {
			$api_key = Flowbie_App_Vertical_Benchmark_Openrouter::resolve_key( (string) ( $body['apiKey'] ?? '' ) );
			if ( $api_key === '' ) {
				Flowbie_App_Api_Dispatcher::send_json( array( 'ok' => false, 'error' => 'OpenRouter API key required for Gemini client tags' ), 400 );
				return;
			}

			$body_sites = isset( $body['sites'] ) && is_array( $body['sites'] ) ? $body['sites'] : null;
			$sites      = Flowbie_App_Vertical_Benchmark_Sites::resolve_for_job( $body_sites );
			$site_ids   = isset( $body['siteIds'] ) && is_array( $body['siteIds'] ) ? $body['siteIds'] : null;
			$sites      = Flowbie_App_Vertical_Benchmark_Sites::filter_by_ids( $sites, $site_ids );

			if ( empty( $sites ) ) {
				Flowbie_App_Api_Dispatcher::send_json( array( 'ok' => false, 'error' => 'No sites to classify' ), 400 );
				return;
			}

			$tags = Flowbie_App_Vertical_Benchmark_Classify::classify_client_tags_batch(
				$sites,
				array(
					'apiKey' => $api_key,
			...
			foreach ( $tags as $t ) {
				$clients[] = array(
					...
					'clientTagLabel' => $t['clientTagLabel'] ?? Flowbie_App_Vertical_Benchmark_Taxonomy::label( (string) $t['clientTag'] ),
					'source'         => $t['source'] ?? 'taxonomy',
				);
			}

			Flowbie_App_Vertical_Benchmark_Sites::save_classification_snapshot( $clients );
			Flowbie_App_Api_Dispatcher::send_json( array( 'ok' => true, 'clients' => $clients ) );
		} catch ( Exception $e ) {
			Flowbie_App_Api_Dispatcher::send_json( array( 'ok' => false, 'error' => $e->getMessage() ), 500 );
		}
	}
	...
	private static function export_gsc_csv( array $body ): void {
		$api_key = Flowbie_App_Vertical_Benchmark_Openrouter::resolve_key( (string) ( $body['apiKey'] ?? '' ) );
		if ( $api_key === '' ) {
			Flowbie_App_Api_Dispatcher::send_json( array( 'ok' => false, 'error' => 'OpenRouter API key required for Gemini URL labeling' ), 400 );
			return;
		}
		...
		if ( empty( $sites ) ) {
			Flowbie_App_Api_Dispatcher::send_json( array( 'ok' => false, 'error' => 'No sites selected for export' ), 400 );
			return;
		}
		...
		try {
			$out = Flowbie_App_Vertical_Benchmark_Gsc_Export::export_rows(
				$sites,
				array(
					'apiKey'                 => $api_key,
					'model'                  => isset( $body['model'] ) ? (string) $body['model'] : '',
