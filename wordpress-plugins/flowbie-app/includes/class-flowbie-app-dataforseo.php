<?php
/**
 * DataForSEO SERP fetch for SEO research briefs (Flowbie App facade).
 *
 * @package Flowbie_App
 */
...
class Flowbie_App_Dataforseo {
	...
	public static function post_live( string $endpoint, array $tasks, int $timeout = 120 ) {
		return Flowbie_App_Dataforseo_Client::post(
			$endpoint,
			$tasks,
	...
	public static function fetch_serp_organic_live_advanced( string $keyword, array $options = array() ) {
		...
		if ( $keyword === '' ) {
			return new WP_Error( 'flowbie_dfs_keyword', 'Focus keyword is required for SERP lookup.' );
		}

		$result = Flowbie_App_Dataforseo_Mcp_Router::dispatch(
	...
	public static function fetch_business_listings_search( string $title, array $location, int $limit = 25 ) {
		$title = trim( $title );
		if ( $title === '' ) {
			return new WP_Error( 'flowbie_dfs_title', 'Business title is required for listings search.' );
		}
	...
	public static function fetch_my_business_info_attempt( string $keyword, array $location ): array {
		...
		if ( $keyword === '' ) {
			return array(
				'item'  => null,
				'error' => new WP_Error( 'flowbie_dfs_keyword', 'Keyword is required for Google Business lookup.' ),
			);
		}
