...
class Flowbie_App_Bulk_Validate_Links {
	...
	public static function stream( array $body ): void {
		try {
			...
			if ( empty( $urls ) ) {
				Flowbie_App_Api_Dispatcher::send_json( array( 'results' => array(), 'allOk' => true ) );
				return;
			}
	...
	private static function ping_url( string $url ): array {
		$args = array(
			...
			'redirection' => 5,
			'headers'     => array( 'User-Agent' => 'FlowbieLinkValidator/1.0' ),
		);
