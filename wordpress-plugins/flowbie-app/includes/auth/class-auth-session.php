<?php
/**
 * HTTP session for Flowbie app users (signed cookie, no PHP session files).
 *
 * @package Flowbie_App
 */
...
class Flowbie_App_Auth_Session {

	const COOKIE_NAME = 'flowbie_session';
	...
	private static function secret(): string {
		if ( defined( 'FLOWBIE_APP_SESSION_SECRET' ) && FLOWBIE_APP_SESSION_SECRET !== '' ) {
			return (string) FLOWBIE_APP_SESSION_SECRET;
		}
		if ( function_exists( 'wp_salt' ) ) {
			return wp_salt( 'flowbie_app_session' );
		}
		if ( defined( 'AUTH_KEY' ) && AUTH_KEY !== '' ) {
			return (string) AUTH_KEY;
		}
		return 'flowbie-app-session';
	}
	...
	public static function require_user(): ?array {
		...
		return Flowbie_App_Teams_Store::get_user_by_id( $user_id );
	}
}
