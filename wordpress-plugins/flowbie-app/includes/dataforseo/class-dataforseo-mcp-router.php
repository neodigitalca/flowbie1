...
class Flowbie_App_Dataforseo_Mcp_Router {
	...
	public static function dispatch( string $tool, array $body ) {
		if ( ! Flowbie_App_Dataforseo_Client::has_credentials() ) {
			return new WP_Error( 'flowbie_dfs_missing', 'DataForSEO credentials are not configured.', array( 'status' => 400 ) );
		}

		if ( $tool === 'DataForSEO_serp_google_ai_mode' ) {
			return Flowbie_App_Dataforseo_Ai_Mode::run( $body );
		}

		if ( ! isset( self::TOOL_ENDPOINTS[ $tool ] ) ) {
			return new WP_Error( 'flowbie_dfs_tool', 'Unknown DataForSEO MCP tool.', array( 'status' => 404 ) );
		}
		...
		$result  = Flowbie_App_Dataforseo_Client::post( $endpoint, $built['tasks'], array( 'timeout' => $timeout ) );
		if ( is_wp_error( $result ) ) {
			return $result;
		}

		$allow_empty = strpos( $tool, 'serp_' ) !== false;
		$check       = Flowbie_App_Dataforseo_Client::assert_task_ok( $result, $allow_empty );
		if ( is_wp_error( $check ) ) {
			if ( $tool === 'DataForSEO_business_data_google_my_business_info_live' ) {
				$task = $result['tasks'][0] ?? array();
				return new WP_Error(
					'flowbie_dfs_gmb_task',
		...
		if ( $tool === 'DataForSEO_serp_organic_live_advanced' ) {
			$stored = Flowbie_App_Dataforseo_Serp_Dumps::write(
	...
	private static function build_tasks( string $tool, array $body ) {
		...
		switch ( $tool ) {
			case 'DataForSEO_dataforseo_labs_google_keyword_overview':
				if ( empty( $body['keywords'] ) || ! is_array( $body['keywords'] ) ) {
					return new WP_Error( 'flowbie_dfs_validate', 'keywords array is required', array( 'status' => 400 ) );
				}
				return array(
					'tasks' => array(
						array(
							'keywords'       => array_values( $body['keywords'] ),
							'location_code'  => Flowbie_App_Dataforseo_Client::location_code_from_name( $body['location_name'] ?? null ),
							'language_code'  => Flowbie_App_Dataforseo_Client::ensure_language_code( $body['language_code'] ?? 'en' ),
						),
					),
				);
			...
			case 'DataForSEO_kw_data_dfs_trends_explore':
				if ( empty( $body['keywords'] ) || ! is_array( $body['keywords'] ) ) {
					return new WP_Error( 'flowbie_dfs_validate', 'keywords array is required (max 5)', array( 'status' => 400 ) );
				}
				$kw = array();
				...
				if ( $kw === array() ) {
					return new WP_Error( 'flowbie_dfs_validate', 'keywords must contain at least one non-empty string', array( 'status' => 400 ) );
				}
				$task = array(
					'keywords' => $kw,
				...
				if ( isset( $body['location_code'] ) && is_numeric( $body['location_code'] ) ) {
					...
					$task['location_code'] = Flowbie_App_Dataforseo_Client::location_code_from_name( $body['location_name'] ?? 'United States' );
				}
			...
			case 'DataForSEO_serp_organic_live_advanced':
				if ( empty( $body['keyword'] ) ) {
					return new WP_Error( 'flowbie_dfs_validate', 'keyword is required', array( 'status' => 400 ) );
				}
				$task = array(
					'keyword'       => (string) $body['keyword'],
					'location_code' => Flowbie_App_Dataforseo_Client::location_code_from_name( $body['location_name'] ?? null ),
			...
			case 'DataForSEO_serp_google_maps_live_advanced':
				if ( empty( $body['keyword'] ) || empty( $body['location_coordinate'] ) ) {
					return new WP_Error( 'flowbie_dfs_validate', 'keyword and location_coordinate are required', array( 'status' => 400 ) );
				}
				$task = array(
			...
			case 'DataForSEO_on_page_lighthouse':
				if ( empty( $body['url'] ) ) {
					return new WP_Error( 'flowbie_dfs_validate', 'url is required', array( 'status' => 400 ) );
				}
				$core = array(
			...
			case 'DataForSEO_dataforseo_labs_google_page_intersection':
				if ( empty( $body['pages'] ) || ! is_array( $body['pages'] ) ) {
					return new WP_Error( 'flowbie_dfs_validate', 'pages array is required', array( 'status' => 400 ) );
				}
			...
			case 'DataForSEO_dataforseo_labs_google_competitors_domain':
				...
				$target = rtrim( $target, '/' );
				if ( $target === '' ) {
					return new WP_Error( 'flowbie_dfs_validate', 'target is required', array( 'status' => 400 ) );
				}
				$task = array(
					'target'        => $target,
					'language_code' => Flowbie_App_Dataforseo_Client::ensure_language_code( $body['language_code'] ?? 'en' ),
				...
				if ( isset( $body['location_code'] ) && is_numeric( $body['location_code'] ) ) {
					...
					$task['location_code'] = Flowbie_App_Dataforseo_Client::location_code_from_name( $body['location_name'] ?? null );
				}
			...
			case 'DataForSEO_business_data_business_listings_search':
				if ( empty( $body['location_coordinate'] ) ) {
					return new WP_Error( 'flowbie_dfs_validate', 'location_coordinate is required', array( 'status' => 400 ) );
				}
				$task = array(
					'location_coordinate' => trim( (string) $body['location_coordinate'] ),
		...
		return new WP_Error( 'flowbie_dfs_tool', 'Unhandled DataForSEO tool.', array( 'status' => 500 ) );
	}
