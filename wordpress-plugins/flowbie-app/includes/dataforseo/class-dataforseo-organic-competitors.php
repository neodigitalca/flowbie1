...
class Flowbie_App_Dataforseo_Organic_Competitors {
	...
	public static function run( array $opts ): array {
		...
		$blocked        = Flowbie_App_Semrush_Competitor_Shared::sanitize_portfolio_blocked_hosts( $opts['portfolioBlockedHosts'] ?? null );
		$blocked_list   = is_array( $blocked ) ? $blocked : array();

		$empty  = self::empty_payload();
		$domain = Flowbie_App_Semrush_Competitor_Shared::domain_from_site_url( $site_url );
		if ( $domain === '' ) {
		...
		$seed_key      = Flowbie_App_Semrush_Competitor_Shared::clean_domain_key( $domain );
		...
		} elseif ( Flowbie_App_Dataforseo_Organic_Competitors_Parse::assert_task_ok( $comp_res, 'competitors_domain', $errors ) ) {
			foreach ( Flowbie_App_Dataforseo_Organic_Competitors_Parse::extract_competitors_items( $comp_res ) as $it ) {
				if ( ! is_array( $it ) ) {
					continue;
				}
				$row = Flowbie_App_Dataforseo_Organic_Competitors_Parse::map_competitor_item_to_row( $it );
				if ( $row === null ) {
					continue;
				}
				if ( Flowbie_App_Semrush_Competitor_Shared::clean_domain_key( $row['domain'] ) === $seed_key ) {
					continue;
				}
				if ( Flowbie_App_Semrush_Competitor_Shared::is_portfolio_blocked_domain( $row['domain'], $blocked_list ) ) {
					continue;
				}
				$rows[] = $row;
			}
		...
		} elseif ( Flowbie_App_Dataforseo_Organic_Competitors_Parse::assert_task_ok( $overview_res, 'domain_rank_overview:seed', $errors ) ) {
			$seed_metrics = Flowbie_App_Dataforseo_Organic_Competitors_Parse::seed_metrics_from_domain_rank_overview( $overview_res );
		}
		...
		} elseif ( Flowbie_App_Dataforseo_Organic_Competitors_Parse::assert_task_ok( $seed_kw_res, 'ranked_keywords:seed', $errors ) ) {
			$seed_top_keywords = Flowbie_App_Dataforseo_Organic_Competitors_Parse::top_keywords_from_ranked_response( $seed_kw_res, self::SEED_DOMAIN_ORGANIC_LIMIT );
		}

		$enrichment_by_domain = array();
		$domain_organic_csv   = array();
		$to_enrich            = Flowbie_App_Semrush_Competitor_Shared::domains_for_enrichment( $rows, $enrichment_cap );
		...
		foreach ( $to_enrich as $d ) {
			...
			$key = Flowbie_App_Semrush_Competitor_Shared::clean_domain_key( $d );
			$enrichment_by_domain[ $key ] = array( 'topKeywords' => $top_kw );
			$domain_organic_csv[ $key ]   = Flowbie_App_Semrush_Competitor_Shared::build_domain_organic_csv( $top_kw, self::DOMAIN_ORGANIC_CSV_TOP_ROWS );
		}
		...
		return array(
			...
			'seedOverview'             => null,
			'seedDomainOrganicCsv'     => Flowbie_App_Semrush_Competitor_Shared::build_domain_organic_csv( $seed_top_keywords, self::DOMAIN_ORGANIC_CSV_TOP_ROWS ),
	...
	public static function run_seed_ranked_keywords_only( string $site_url ): array {
		...
		$target_norm       = Flowbie_App_Semrush_Competitor_Shared::clean_domain_key( $domain );
		$seed_top_keywords = array();
		$seed_kw_res       = self::post(
			'/dataforseo_labs/google/ranked_keywords/live',
			array(
	...
	public static function run_manual_domain( array $opts ): array {
		...
		} elseif ( Flowbie_App_Dataforseo_Organic_Competitors_Parse::assert_task_ok( $overview_res, 'domain_rank_overview:manual', $errors ) ) {
			$seed_metrics = Flowbie_App_Dataforseo_Organic_Competitors_Parse::seed_metrics_from_domain_rank_overview( $overview_res );
		}
		...
		return array(
			...
			'domainOrganicCsv' => Flowbie_App_Semrush_Competitor_Shared::build_domain_organic_csv( $top_keywords, self::DOMAIN_ORGANIC_CSV_TOP_ROWS ),
			'errors'           => $errors,
		);
	}
	...
	private static function post( string $endpoint, array $tasks, int $timeout_sec ) {
		return Flowbie_App_Dataforseo_Client::post( $endpoint, $tasks, array( 'timeout' => $timeout_sec * 1000 ) );
	}
	...
	private static function fetch_ranked_keywords_for_domain( string $target_domain, int $location_code, string $language_code, array &$errors ): array {
		$norm = Flowbie_App_Semrush_Competitor_Shared::clean_domain_key( $target_domain );
		$res  = self::post(
			'/dataforseo_labs/google/ranked_keywords/live',
			array(
				array(
		...
		if ( ! Flowbie_App_Dataforseo_Organic_Competitors_Parse::assert_task_ok( $res, 'ranked_keywords:' . $norm, $errors ) ) {
			return array();
		}
		return Flowbie_App_Dataforseo_Organic_Competitors_Parse::top_keywords_from_ranked_response( $res, self::ENRICHMENT_DOMAIN_ORGANIC_LIMIT );
	}
