...
class Flowbie_App_Dataforseo_Route_Handlers {
	...
	public static function handle_mcp_post( WP_REST_Request $request ) {
		...
		if ( ! Flowbie_App_Dataforseo_Mcp_Router::is_supported( $tool ) ) {
			return new WP_Error( 'flowbie_dfs_tool', 'Unknown DataForSEO MCP tool.', array( 'status' => 404 ) );
		}

		$result = Flowbie_App_Dataforseo_Mcp_Router::dispatch( $tool, $body );
	...
	public static function handle_serp_dump_download( WP_REST_Request $request ) {
		...
		if ( ! Flowbie_App_Dataforseo_Serp_Dumps::is_safe_filename( $filename ) ) {
			return new WP_Error( 'flowbie_dfs_dump', 'Invalid filename', array( 'status' => 400 ) );
		}

		$data = Flowbie_App_Dataforseo_Serp_Dumps::read( $filename );
		if ( $data === null ) {
			return new WP_Error( 'flowbie_dfs_dump', 'File not found', array( 'status' => 404 ) );
		}
	...
	public static function handle_google_images( WP_REST_Request $request ) {
		...
		$result = Flowbie_App_Dataforseo_Google_Images::fetch( $keyword, $options );
		if ( is_wp_error( $result ) ) {
			return new WP_REST_Response(
	...
	public static function send_serp_dump_file( string $filename ): void {
		if ( ! Flowbie_App_Dataforseo_Serp_Dumps::is_safe_filename( $filename ) ) {
			Flowbie_App_Api_Dispatcher::send_json( array( 'error' => 'Invalid filename' ), 400 );
			return;
		}
		$data = Flowbie_App_Dataforseo_Serp_Dumps::read( $filename );
		if ( $data === null ) {
	...
	public static function handle_google_images_body( array $body ): array {
		...
		$result  = Flowbie_App_Dataforseo_Google_Images::fetch( $keyword, $body );
		if ( is_wp_error( $result ) ) {
	...
	public static function run_competitor_research( array $body ): array {
		...
		try {
			...
			return array(
				'status' => 200,
				'data'   => Flowbie_App_Dataforseo_Organic_Competitors::run( $opts ),
			);
		} catch ( Exception $e ) {
	...
	public static function run_competitor_manual_domain( array $body ): array {
		...
		try {
			$result = Flowbie_App_Dataforseo_Organic_Competitors::run_manual_domain(
				array(
	...
	public static function run_seed_ranked_keywords( array $body ): array {
		...
		try {
			return array(
				'status' => 200,
				'data'   => Flowbie_App_Dataforseo_Organic_Competitors::run_seed_ranked_keywords_only( $site_url ),
			);
		} catch ( Exception $e ) {
			return array( 'status' => 502, 'data' => array( 'error' => $e->getMessage() ) );
		}
	...
	public static function dispatch_http( string $subpath, string $method, array $body ): void {
		...
		if ( $subpath === 'google-images' && $method === 'POST' ) {
			Flowbie_App_Api_Dispatcher::send_json( self::handle_google_images_body( $body ), 200 );
			return;
		}
