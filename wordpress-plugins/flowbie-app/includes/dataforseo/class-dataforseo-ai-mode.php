...
class Flowbie_App_Dataforseo_Ai_Mode {
	...
	public static function run( array $body ) {
		...
		$lang          = Flowbie_App_Dataforseo_Client::ensure_language_code( $body['language_code'] ?? 'en' );
		$min_dr        = isset( $body['min_dr'] ) ? (int) $body['min_dr'] : 60;

		$post = Flowbie_App_Dataforseo_Client::post(
		...
		if ( ! $task_id ) {
			return new WP_Error( 'flowbie_dfs_ai_mode', 'No task created from ai_mode/task_post' );
		}
		...
		while ( time() < $deadline ) {
			sleep( 3 );
			$ready = Flowbie_App_Dataforseo_Client::get( 'serp/google/ai_mode/tasks_ready' );
			...
			if ( $found ) {
				$ai_result = Flowbie_App_Dataforseo_Client::get(
		...
		if ( ! is_array( $ai_result ) ) {
			return new WP_Error( 'flowbie_dfs_ai_mode', 'AI Mode task timed out after 120s' );
		}

		$check = Flowbie_App_Dataforseo_Client::assert_task_ok( $ai_result, false );
		...
		$domain_ranks = array();
		if ( $candidates !== array() ) {
			$bulk = Flowbie_App_Dataforseo_Client::post(
