<?php
/**
 * DataForSEO REST client (credentials via Flowbie_App_Secrets).
 *
 * @package Flowbie_App
 */
...
class Flowbie_App_Dataforseo_Client {
	...
	public static function credentials(): array {
		return Flowbie_App_Secrets::dataforseo();
	}
	...
	public static function post( string $endpoint, array $tasks, array $options = array() ) {
		...
		if ( self::contains_key_deep( $payload, 'language_name' ) ) {
			return new WP_Error( 'flowbie_dfs_payload', 'language_name is not allowed in DataForSEO payload.' );
		}
		...
		if ( ! is_array( $data ) ) {
			return new WP_Error( 'flowbie_dfs_bad_json', 'DataForSEO returned an unexpected response.' );
		}
	...
	public static function get( string $endpoint, array $options = array() ) {
		$creds = self::credentials();
		if ( $creds['login'] === '' || $creds['password'] === '' ) {
			return new WP_Error( 'flowbie_dfs_missing', 'DataForSEO credentials are not configured.' );
		}
		...
		return is_array( $data ) ? $data : new WP_Error( 'flowbie_dfs_bad_json', 'DataForSEO returned an unexpected response.' );
	}
	...
	public static function assert_task_ok( array $result, bool $allow_benign_empty = false ) {
		...
		return new WP_Error( 'flowbie_dfs_task', $msg, array( 'status_code' => (int) $task['status_code'] ) );
	}
}
