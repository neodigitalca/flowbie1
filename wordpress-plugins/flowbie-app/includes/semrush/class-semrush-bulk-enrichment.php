...
class Flowbie_App_Semrush_Bulk_Enrichment {
	...
	public static function run( array $opts ): array {
		...
		$url = Flowbie_App_Semrush_Client::normalize_url( $page_url );
		if ( $url === '' ) {
		...
		$url_organic = Flowbie_App_Semrush_Client::request_report(
			array(
				'type'           => 'url_organic',
				'database'       => $database,
				'url'            => $url,
				'display_limit'  => Flowbie_App_Semrush_Client::DISPLAY_LIMIT,
				'export_columns' => 'Ph',
			)
		);
		...
		$url_organic_keywords = is_string( $url_organic ) ? Flowbie_App_Semrush_Client::keywords_from_csv( $url_organic ) : array();
		$url_organic_urls   = is_string( $url_organic ) ? Flowbie_App_Semrush_Client::urls_from_csv( $url_organic ) : array();
		...
		if ( $phrase !== '' ) {
			...
			$phrase_related = Flowbie_App_Semrush_Client::request_report(
			...
			if ( is_wp_error( $phrase_related ) ) {
				...
				$phrase_related_keywords = Flowbie_App_Semrush_Client::keywords_from_csv( $phrase_related );
				$phrase_related_urls     = Flowbie_App_Semrush_Client::urls_from_csv( $phrase_related );
			}

			$phrase_organic = Flowbie_App_Semrush_Client::request_report(
			...
			if ( is_wp_error( $phrase_organic ) ) {
				...
				$phrase_organic_urls = Flowbie_App_Semrush_Client::urls_from_csv( $phrase_organic );
			}
			...
			$phrase_this        = Flowbie_App_Semrush_Client::request_report( $phrase_this_params );
			if ( is_wp_error( $phrase_this ) ) {
				$errors[] = array( 'step' => 'phrase_this', 'message' => $phrase_this->get_error_message() );
			}

			$phrase_kdi = Flowbie_App_Semrush_Client::request_report(
	...
	private static function filter_reference_urls(
		...
		$page_host = '';
		$norm      = Flowbie_App_Semrush_Client::normalize_url( $page_url );
		if ( $norm !== '' ) {
