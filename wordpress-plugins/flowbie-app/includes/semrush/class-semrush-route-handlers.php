...
class Flowbie_App_Semrush_Route_Handlers {

	const REST_NAMESPACE = 'flowbie-app/v1';
	...
	public static function run_bulk_enrichment( array $body ): array {
		...
		if ( ! Flowbie_App_Semrush_Client::has_api_key() ) {
			$payload = array(
				'skipped' => true,
				'reason'  => 'no_api_key',
			);
			...
			try {
				$payload = Flowbie_App_Semrush_Bulk_Enrichment::run(
					array(
						'pageUrl'               => $page_url,
		...
		$stored                = Flowbie_App_Semrush_Overview_Json::write_overview( $page_url, $seed_keyword, $payload );
	...
	public static function handle_overview_json( WP_REST_Request $request ) {
		...
		if ( ! Flowbie_App_Semrush_Overview_Json::is_safe_overview_filename( $filename ) ) {
			return new WP_Error( 'flowbie_semrush_file', 'Invalid filename', array( 'status' => 400 ) );
		}

		$data = Flowbie_App_Semrush_Overview_Json::read( $filename );
		if ( $data === null ) {
			return new WP_Error( 'flowbie_semrush_file', 'File not found', array( 'status' => 404 ) );
		}
	...
	public static function handle_audit_json( WP_REST_Request $request ) {
		$filename = (string) $request->get_param( 'filename' );
		if ( ! Flowbie_App_Semrush_Overview_Json::is_safe_audit_filename( $filename ) ) {
			return new WP_Error( 'flowbie_semrush_file', 'Invalid filename', array( 'status' => 400 ) );
		}
	...
	public static function handle_audit_ai_context( WP_REST_Request $request ) {
		...
		if ( $filename === '' || ! Flowbie_App_Semrush_Overview_Json::is_safe_audit_filename( $filename ) ) {
			return new WP_Error( 'flowbie_semrush_file', 'Invalid or missing filename', array( 'status' => 400 ) );
		}

		$doc = Flowbie_App_Semrush_Overview_Json::read( $filename );
		if ( $doc === null ) {
			return new WP_Error( 'flowbie_semrush_file', 'File not found', array( 'status' => 404 ) );
		}

		return new WP_REST_Response(
			Flowbie_App_Semrush_Overview_Json::compact_audit_context( $doc, $max ),
			200
		);
	}
	...
	public static function run_match_project_for_site( array $body ): array {
		...
		$projects = Flowbie_App_Semrush_Projects_Api::list_projects( array( 'filter' => 'all' ) );
		if ( is_wp_error( $projects ) ) {
			return array( 'status' => 502, 'data' => array( 'error' => $projects->get_error_message() ) );
		}

		$match_result = Flowbie_App_Semrush_Projects_Api::match_project_for_site_url( $site_url, $projects );
	...
	public static function run_competitor_research( array $body ): array {
		...
		try {
			...
			return array(
				'status' => 200,
				'data'   => Flowbie_App_Semrush_Organic_Competitors::run( $opts ),
			);
		} catch ( Exception $e ) {
	...
	public static function run_competitor_manual_domain( array $body ): array {
		...
		try {
			$result = Flowbie_App_Semrush_Organic_Competitors::run_manual_domain(
				array(
					'siteUrl'  => $site_url,
					'database' => $body['database'] ?? null,
					'domain'   => $domain,
	...
	public static function run_site_audit_url( array $body ): array {
		...
		$stored                = Flowbie_App_Semrush_Overview_Json::write_audit( $page_url, $project_id, $payload );
		$payload['storedFile'] = $stored;
		return $payload;
	}
	...
	public static function dispatch_http( string $subpath, string $method, array $body ): void {
		...
		if ( $subpath === 'bulk-enrichment' && $method === 'POST' ) {
			Flowbie_App_Api_Dispatcher::send_json( self::run_bulk_enrichment( $body ), 200 );
			return;
		}
		if ( preg_match( '#^overview-json/([a-zA-Z0-9._-]+)$#', $subpath, $m ) && $method === 'GET' ) {
			...
			if ( is_wp_error( $response ) ) {
				Flowbie_App_Api_Dispatcher::send_json( array( 'error' => $response->get_error_message() ), 404 );
				return;
			}
		...
		if ( $subpath === 'audit-ai-context' && $method === 'POST' ) {
			...
			$max      = isset( $body['maxChars'] ) && is_numeric( $body['maxChars'] )
				...
				: 120000;
			if ( $filename === '' || ! Flowbie_App_Semrush_Overview_Json::is_safe_audit_filename( $filename ) ) {
				Flowbie_App_Api_Dispatcher::send_json( array( 'error' => 'Invalid or missing filename' ), 400 );
				return;
			}
			$doc = Flowbie_App_Semrush_Overview_Json::read( $filename );
			if ( $doc === null ) {
				Flowbie_App_Api_Dispatcher::send_json( array( 'error' => 'File not found' ), 404 );
				return;
			}
			Flowbie_App_Api_Dispatcher::send_json( Flowbie_App_Semrush_Overview_Json::compact_audit_context( $doc, $max ), 200 );
			return;
		}
		...
		if ( $subpath === 'match-project-for-site' && $method === 'POST' ) {
			$result = self::run_match_project_for_site( $body );
			Flowbie_App_Api_Dispatcher::send_json( $result['data'], $result['status'] );
			return;
		}
		...
		if ( $subpath === 'site-audit-url' && $method === 'POST' ) {
			Flowbie_App_Api_Dispatcher::send_json( self::run_site_audit_url( $body ), 200 );
			return;
		}

		Flowbie_App_Api_Dispatcher::send_json( array( 'error' => 'Not found', 'path' => 'semrush/' . $subpath ), 404 );
	}
}
