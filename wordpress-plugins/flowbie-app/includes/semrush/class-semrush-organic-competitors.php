...
class Flowbie_App_Semrush_Organic_Competitors {
	...
	public static function run( array $opts ): array {
		...
		$database = Flowbie_App_Semrush_Competitor_Shared::resolve_database(
			$opts['database'] ?? null,
			$site_url,
			'us'
		);
		$display_limit = self::int_limit( $opts['displayLimit'] ?? null, Flowbie_App_Semrush_Competitor_Shared::DEFAULT_DISPLAY_LIMIT, 1000 );
		$enrichment_cap  = self::int_limit( $opts['enrichmentCap'] ?? null, Flowbie_App_Semrush_Competitor_Shared::DEFAULT_ENRICHMENT_CAP, 50 );
		...
		$rank_csv = self::request( 'domain_rank', $database, $domain, array(
			'display_limit'  => 1,
			'export_columns' => Flowbie_App_Semrush_Competitor_Shared::EXPORT_SEED_RANK,
		), $errors, 'seed:domain_rank' );

		$seed_kw_csv = self::request( 'domain_organic', $database, $domain, array(
			'display_limit'  => Flowbie_App_Semrush_Competitor_Shared::SEED_DOMAIN_ORGANIC_LIMIT,
			'display_sort'   => 'tr_desc',
			'export_columns' => Flowbie_App_Semrush_Competitor_Shared::EXPORT_DOMAIN_ORGANIC_KW,
		), $errors, 'seed:domain_organic' );

		$comp_csv = self::request( 'domain_organic_organic', $database, $domain, array(
			'display_limit'  => $display_limit,
			'display_sort'   => 'np_desc',
			'export_columns' => Flowbie_App_Semrush_Competitor_Shared::EXPORT_COMPETITORS,
		...
		$seed_top_keywords = is_string( $seed_kw_csv )
			? Flowbie_App_Semrush_Competitor_Shared::extract_top_keywords_from_csv(
				$seed_kw_csv,
				Flowbie_App_Semrush_Competitor_Shared::SEED_DOMAIN_ORGANIC_LIMIT
			)
			: array();

		$rows = is_string( $comp_csv ) ? Flowbie_App_Semrush_Competitor_Shared::extract_competitor_rows( $comp_csv ) : array();
		$rows = array_values(
			array_filter(
				$rows,
				static function ( $row ) use ( $seed_key, $blocked_list ) {
					...
					return ! Flowbie_App_Semrush_Competitor_Shared::is_portfolio_blocked_domain( $row['domain'], $blocked_list );
				}
			)
		);
		usort(
		...
		$deadline             = time() + Flowbie_App_Semrush_Competitor_Shared::ENRICHMENT_DEADLINE_SEC;
		...
		foreach ( $to_enrich as $d ) {
			...
			$kw_csv = self::request( 'domain_organic', $database, $d, array(
				'display_limit'  => Flowbie_App_Semrush_Competitor_Shared::ENRICHMENT_DOMAIN_ORGANIC_LIMIT,
				'display_sort'   => 'tr_desc',
				'export_columns' => Flowbie_App_Semrush_Competitor_Shared::EXPORT_DOMAIN_ORGANIC_KW,
			), $errors, 'enrich:' . $d . ':domain_organic' );

			$top_kw = is_string( $kw_csv )
				? Flowbie_App_Semrush_Competitor_Shared::extract_top_keywords_from_csv( $kw_csv )
				: array();
			$key = Flowbie_App_Semrush_Competitor_Shared::clean_domain_key( $d );
			$enrichment_by_domain[ $key ] = array( 'topKeywords' => $top_kw );
			$domain_organic_csv[ $key ]   = Flowbie_App_Semrush_Competitor_Shared::build_domain_organic_csv( $top_kw );
		}
		...
		return array(
			...
			'seedOverview'            => null,
			'seedDomainOrganicCsv'    => Flowbie_App_Semrush_Competitor_Shared::build_domain_organic_csv( $seed_top_keywords ),
	...
	public static function run_manual_domain( array $opts ): array {
		...
		$dk         = Flowbie_App_Semrush_Competitor_Shared::hostname_from_domain_field( $raw_domain );
		$errors     = array();
		...
		return array(
			...
			'domainOrganicCsv' => Flowbie_App_Semrush_Competitor_Shared::build_domain_organic_csv( $top_kw ),
			'errors'           => $errors,
		);
	}
	...
	private static function request( string $type, string $database, string $domain, array $extra, array &$errors, string $step ): ?string {
		...
		$result = Flowbie_App_Semrush_Client::request_report( $params );
	...
	private static function pick_seed_rank_row( string $csv, string $seed_domain ): ?array {
		$rows = Flowbie_App_Semrush_Competitor_Shared::extract_competitor_rows( $csv );
		$sd   = Flowbie_App_Semrush_Competitor_Shared::clean_domain_key( $seed_domain );
		foreach ( $rows as $row ) {
			if ( Flowbie_App_Semrush_Competitor_Shared::clean_domain_key( $row['domain'] ) === $sd ) {
				return $row;
			}
		}
		return $rows[0] ?? null;
