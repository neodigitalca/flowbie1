...
class Flowbie_App_Semrush_Projects_Api {
	...
	public static function list_projects( array $options = array() ) {
		$key = Flowbie_App_Semrush_Client::api_key();
		if ( $key === '' ) {
		...
		if ( $code < 200 || $code >= 300 ) {
			...
			return new WP_Error( 'flowbie_semrush_projects', $msg );
		}

		if ( ! is_array( $data ) ) {
			return new WP_Error( 'flowbie_semrush_projects', 'Semrush projects response was not a JSON array' );
		}
