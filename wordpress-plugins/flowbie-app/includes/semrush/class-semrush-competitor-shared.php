...
class Flowbie_App_Semrush_Competitor_Shared {
	...
	public static function normalize_competitor_row( array $r ): ?array {
		$lower = Flowbie_App_Semrush_Table_Parse::record_keys_lower( $r );
		...
		return array(
			'domain'           => $dn,
			'competitionLevel' => Flowbie_App_Semrush_Table_Parse::num( $pick( array( 'cr', 'competition', 'competition_level' ) ) ),
			'commonKeywords'   => Flowbie_App_Semrush_Table_Parse::num( $pick( array( 'np', 'common_keywords' ) ) ),
			'organicTraffic'   => Flowbie_App_Semrush_Table_Parse::num( $pick( array( 'or', 'organic_traffic', 'traffic' ) ) ),
			'trafficCost'      => Flowbie_App_Semrush_Table_Parse::num( $pick( array( 'ot', 'traffic_cost', 'traffic_value' ) ) ),
			'organicKeywords'  => Flowbie_App_Semrush_Table_Parse::num( $pick( array( 'oc', 'organic_keywords' ) ) ),
			'adsKeywords'      => Flowbie_App_Semrush_Table_Parse::num( $pick( array( 'ad', 'paid_keywords', 'ads_keywords' ) ) ),
		);
	}
	...
	public static function extract_competitor_rows( string $csv ): array {
		$out  = array();
		$rows = Flowbie_App_Semrush_Table_Parse::rows_from_csv_text( $csv );
		foreach ( $rows as $row ) {
	...
	public static function extract_top_keywords_from_csv( string $csv, int $limit = self::ENRICHMENT_DOMAIN_ORGANIC_LIMIT ): array {
		...
		$out  = array();
		foreach ( $rows as $row ) {
			$m = self::record_keys_lower( Flowbie_App_Semrush_Table_Parse::record_keys_lower( $row ) );
			...
			$out[] = array(
				...
				'position' => Flowbie_App_Semrush_Table_Parse::num( $m['po'] ?? $m['position'] ?? null ),
			);
			if ( count( $out ) >= $limit ) {
				break;
			}
	...
	private static function volume_from_row( array $m ): ?float {
		foreach ( array( 'nq', 'volume', 'search volume', 'sv' ) as $k ) {
			if ( isset( $m[ $k ] ) ) {
				$n = Flowbie_App_Semrush_Table_Parse::num( $m[ $k ] );
				if ( $n !== null ) {
					return $n;
				}
			}
	...
	private static function record_keys_lower( array $m ): array {
		return Flowbie_App_Semrush_Table_Parse::record_keys_lower( $m );
	}
	...
	public static function resolve_database( $body_database, string $site_url, string $default = 'us' ): string {
		...
		$from_site = Flowbie_App_Semrush_Client::database_from_site_url( $site_url );
