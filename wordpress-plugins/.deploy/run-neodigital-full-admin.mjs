/**
 * Load real wp-admin NEO Pulse page on neodigital.ca.
 */
...
writeFileSync(join(outDir, "full-admin-token.txt"), token, "utf8");
...
console.log(cut === -1 ? body : body.slice(0, Math.min(cut, 5000)));
if (cut !== -1) {
  writeFileSync(join(outDir, "full-admin.html"), body.slice(cut), "utf8");
