/**
 * Parallel SFTP download of remaining In The Shade plugins, themes, and uploads.
 */
import { mkdirSync, existsSync, readdirSync, appendFileSync } from "fs";
import { join } from "path";
import SftpClient from "ssh2-sftp-client";

const phase = process.argv[2] || "content";
...
try {
  if (phase === "content" || phase === "all") {
    ...
    for (const e of plugins.filter((x) => x.type === "d")) {
      if (SKIP_PLUGINS.has(e.name) || DONE_PLUGINS.has(e.name)) continue;
      jobs.push({
