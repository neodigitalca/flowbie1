...
for (const p of [
  "./wp-content/plugins/phoenix-restore-mysql.sql.gz",
  "./wp-content/plugins/phoenix-restore-mysql.sql",
  "./wp-content/plugins/phoenix-restore-once.php",
]) {
  try {
    const st = await sftp.stat(p);
    console.log("ok", p, st.size);
  } catch (e) {
    console.log("missing", p, e.message);
  }
}
await sftp.end();
