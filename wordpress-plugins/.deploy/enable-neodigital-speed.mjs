...
const local = join(tmpDir, "nd-speed-on.php");
...
await sftp.put(local, "./wp-content/plugins/neo-pulse-wp/nd-speed-on.php");
await sftp.end();
const res = await fetch(`https://neodigital.ca/wp-content/plugins/neo-pulse-wp/nd-speed-on.php?key=${token}`, { cache: "no-store" });
