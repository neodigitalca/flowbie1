(function () {
  ...
  var TAB_KEY = 'flowbie_ai_sidebar_tab';
  var merged = false;
  var shell = null;
  var activeTab = 'chat';
  ...
  function findSearchPanelBody(searchWrap) {
    ...
    for (var i = 0; i < portaledPanels.length; i++) {
      ...
      if (!panel.querySelector('.flowbie-search-wrap--panel-inner, .fbs--sidebar-inner')) continue;
      return panel.querySelector('.fai-sidebar-panel__body');
    }
    var inners = document.querySelectorAll('.flowbie-search-wrap--panel-inner');
  ...
  function findSearchPanel(searchWrap) {
    ...
    for (var i = 0; i < portaledPanels.length; i++) {
      var panel = portaledPanels[i];
      if (panel.querySelector('.flowbie-search-wrap--panel-inner, .fbs--sidebar-inner')) {
        return panel;
      }
    }
    return null;
  ...
  function tryMerge() {
    ...
    var chatRoot = document.getElementById('flowbie-chat-widget-root');
    ...
    if (window.FlowbieAiSidebarShell) {
      shell = window.FlowbieAiSidebarShell.init(chatRoot, {
    ...
    merged = true;
    document.dispatchEvent(new CustomEvent('flowbie-ai-sidebar-merged'));
    return true;
  }

  window.FlowbieAiSidebarUnify = {
    tryMerge: tryMerge,
    isMerged: function () {
      return merged;
    },
