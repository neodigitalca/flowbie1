(function ($) {
	...
	var cfg = window.flowbieWpImageSeoField || {};
	...
	function panelValues($panel) {
		var values = {};
		$panel.find('.flowbie-image-seo-input').each(function () {
			var $input = $(this);
	...
	function buildPanelHtml(attachmentId) {
		...
		var html = '<div class="flowbie-image-seo-panel" data-attachment-id="' + attachmentId + '" data-context="gallery">';
		fields.forEach(function (f) {
			html += '<div class="flowbie-image-seo-field flowbie-image-seo-field--' + f.key + '">';
			html += '<label>' + f.label + '</label><div class="flowbie-image-seo-field__row">';
			if (f.type === 'textarea') {
				html += '<textarea class="flowbie-image-seo-input" data-field="' + f.key + '" rows="2"></textarea>';
			} else {
				html += '<input type="text" class="flowbie-image-seo-input" data-field="' + f.key + '" value="" />';
			}
			html += '<button type="button" class="button flowbie-image-seo-wand" data-field="' + f.key + '" title="Optimize" aria-label="Optimize">✦</button>';
			html += '</div></div>';
		});
		html += '<button type="button" class="button flowbie-image-seo-optimize-all">Optimize all</button>';
		html += '<button type="button" class="button flowbie-image-seo-save">Save metadata</button></div>';
		return html;
	}

	window.flowbieImageSeoField = {
		renderGalleryItemHtml: function (attachmentId) {
			return '<li class="flowbie-gallery-item" data-id="' + attachmentId + '"><div class="flowbie-gallery-item__loading">Loading…</div></li>';
		},
		hydrateGalleryItem: function ($item, attachmentId, postId) {
			fetchAttachmentRow(attachmentId).then(function (row) {
				...
				var html = '<div class="flowbie-gallery-item__thumb">' + thumb +
					'<button type="button" class="flowbie-gallery-item__remove" aria-label="Remove">&times;</button></div>';
				html += buildPanelHtml(attachmentId);
				$item.html(html);
				var $panel = $item.find('.flowbie-image-seo-panel');
		...
		renderImageSeoPanel: function ($wrap, attachmentId) {
			var $seo = $wrap.find('.flowbie-media-seo-wrap');
			$seo.empty();
			if (!attachmentId) {
				return;
			}
			...
			fetchAttachmentRow(attachmentId).then(function (row) {
				var $panel = $seo.find('.flowbie-image-seo-panel');
				populatePanelFromRow($panel, row);
			});
		}
	};
	...
	$(function () {
		...
		$(document).on('click', '.flowbie-image-seo-wand', function () {
			var $btn = $(this);
			var field = $btn.data('field');
			var $panel = $btn.closest('.flowbie-image-seo-panel');
		...
		$(document).on('click', '.flowbie-image-seo-optimize-all', function () {
			var $btn = $(this);
			var $panel = $btn.closest('.flowbie-image-seo-panel');
			$btn.prop('disabled', true);
		...
		$(document).on('click', '.flowbie-image-seo-save', function () {
			var $btn = $(this);
			var $panel = $btn.closest('.flowbie-image-seo-panel');
			$btn.prop('disabled', true);
			savePanel($panel)
		...
		$(document).on('click', '.flowbie-gallery-item__remove', function (e) {
			e.preventDefault();
			var $item = $(this).closest('.flowbie-gallery-item');
			var $field = $item.closest('.flowbie-gallery-field');
			var id = String($item.data('id'));
			var ids = ($field.find('.flowbie-gallery-ids').val() || '').split(',').filter(Boolean);
			ids = ids.filter(function (x) {
				return x !== id;
			});
			$field.find('.flowbie-gallery-ids').val(ids.join(','));
			$item.remove();
		});
	});
})(jQuery);
