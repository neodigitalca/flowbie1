...
(function ($) {
	...
	api.manifestDownloadFilename = function() {
		var title = String($('#flowbie-agent-hub-field-title').val() || 'seo-block')
			.toLowerCase()
			.replace(/[^\w-]+/g, '-')
			.replace(/^-+|-+$/g, '');
	...
	api.agentScrollDown = function() {
		var el = document.getElementById('flowbie-builder-agent-messages');
	...
	api.appendAgentUser = function(text) {
		var $msgs = $('#flowbie-builder-agent-messages');
		$('#flowbie-builder-agent-center').hide();
		$msgs.append('<div class="flowbie-builder-agent-assist__user">' + api.esc(text) + '</div>');
		api.agentScrollDown();
	}
	...
	api.buildAgentWorkflowStepsList = function(steps) {
		var $ul = $('<ul class="flowbie-builder-agent-assist__workflow-steps"></ul>');
		(steps || []).forEach(function (step, idx) {
			...
			var $li = $('<li class="flowbie-builder-agent-assist__workflow-step flowbie-builder-agent-assist__workflow-step--' + api.esc(st) + (kind ? ' flowbie-builder-agent-assist__workflow-step--' + api.esc(kind) : '') + '" data-step-index="' + idx + '"></li>');
			$li.append('<span class="flowbie-builder-agent-assist__workflow-step-icon">' + api.esc(api.agentWorkflowStepIcon(st)) + '</span>');
			$li.append('<span class="flowbie-builder-agent-assist__workflow-step-label">' + api.esc(step.label || ('Step ' + (idx + 1))) + '</span>');
			$ul.append($li);
		});
		return $ul;
	}
	...
	api.appendAgentWorkflowCard = function(card) {
		...
		var $root = $('<div class="flowbie-builder-agent-assist__card flowbie-builder-agent-assist__card--' + api.esc(type) + ' flowbie-builder-agent-assist__card--workflow"></div>');
		$root.append('<div class="flowbie-builder-agent-assist__card-badge">' + api.esc(type) + '</div>');
		$root.append('<div class="flowbie-builder-agent-assist__card-title">' + api.esc(card.title || '') + '</div>');
		var $body = $('<div class="flowbie-builder-agent-assist__card-body"></div>');
		...
		$msgs.append($root);
		api.agentScrollDown();
		return { root: $root[0], titleEl: $root.find('.flowbie-builder-agent-assist__card-title')[0], bodyEl: $body[0], stepsList: $steps[0] };
	}
	...
	api.setAgentWorkflowStepStatus = function(shell, idx, status) {
		...
		$li.removeClass(function (i, c) {
			return (c.match(/flowbie-builder-agent-assist__workflow-step--(?:pending|running|done|error)/g) || []).join(' ');
		}).addClass('flowbie-builder-agent-assist__workflow-step--' + status);
		$li.find('.flowbie-builder-agent-assist__workflow-step-icon').text(api.agentWorkflowStepIcon(status));
	}

	api.setAgentWorkflowCardActive = function(shell, active) {
		if (!shell || !shell.root) return;
		$(shell.root).toggleClass('flowbie-builder-agent-assist__card--workflow-active', !!active);
	}
	...
	api.attachManifestActions = function($card, card) {
		...
		$card.find('.flowbie-builder-agent-assist__card-actions').remove();
		$card.find('.flowbie-builder-agent-assist__card-body').append(
			'<div class="flowbie-builder-agent-assist__card-actions">' +
			'<button type="button" class="button button-primary flowbie-builder-agent-apply-manifest">' + api.esc(R.cfg.i18n.applyManifest || 'Apply to R.builder') + '</button></div>'
		);
	...
	api.appendAgentCard = function(card) {
		...
		var $card = $('<div class="flowbie-builder-agent-assist__card flowbie-builder-agent-assist__card--' + api.esc(type) + '">' +
			'<div class="flowbie-builder-agent-assist__card-badge">' + api.esc(type) + '</div>' +
			'<div class="flowbie-builder-agent-assist__card-title">' + api.esc(card.title || '') + '</div>' +
			'<div class="flowbie-builder-agent-assist__card-body">' + api.renderMd(card.body || '') + '</div></div>');
	...
	api.runBackendAssistAgent = function(text) {
		...
		$('#flowbie-builder-agent-send, #flowbie-builder-agent-input').prop('disabled', true);
		...
		var thinkingShell = window.FlowbieThinkingCard
			? FlowbieThinkingCard.createThinkingCard(host, { includeVoice: false, title: 'Working on your block…' })
			: null;
		...
		return api.ensurePrimaryPageContext(payload.primary_post_id || 0, payload.id || 0).then(function (ctx) {
			...
			if (!plan.workflow) {
				if (thinkingShell && window.FlowbieThinkingCard) {
			...
			$('#flowbie-builder-agent-send, #flowbie-builder-agent-input').prop('disabled', false);
			$('#flowbie-builder-agent-input').focus();
		});
	}

	api.sendAgentMessage = function () {
		var text = ($('#flowbie-builder-agent-input').val() || '').trim();
		if (!text || R.agent.loading) {
			return;
		}
		api.appendAgentUser(text);
		R.agent.history.push({ role: 'user', content: text, ts: Math.floor(Date.now() / 1000) });
		$('#flowbie-builder-agent-input').val('');
		api.runBackendAssistAgent(text);
	};

	api.initAgentTab = function () {
		$('#flowbie-builder-agent-send').on('click', api.sendAgentMessage);
		$('#flowbie-builder-agent-input').on('keydown', function (e) {
			if (e.key === 'Enter' && !e.shiftKey) {
				e.preventDefault();
				api.sendAgentMessage();
			}
		});
		$('#flowbie-builder-agent-suggestions .flowbie-builder-agent-assist__chip').on('click', function () {
			var prompt = $(this).data('prompt') || $(this).attr('data-prompt') || '';
			if (prompt) { $('#flowbie-builder-agent-input').val(prompt); api.sendAgentMessage(); }
		});
		$('#flowbie-agent-hub-modal-download-json').on('click', api.downloadManifestJson);
		R.dom.$modal.on('click', '.flowbie-builder-agent-apply-manifest', function () {
			var $card = $(this).closest('.flowbie-builder-agent-assist__card');
