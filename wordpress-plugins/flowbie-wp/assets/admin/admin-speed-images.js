(function () {
	...
	var cfg = window.flowbieWpSpeedImages;
	if (!cfg || !cfg.root) {
		return;
	}

	var startBtn = document.getElementById('flowbie-speed-images-bulk-start');
	var forceEl = document.getElementById('flowbie-speed-images-bulk-force');
	var box = document.getElementById('flowbie-speed-images-bulk');
	if (!startBtn || !box) {
		return;
	}

	var statusEl = box.querySelector('.flowbie-speed-images-bulk__status');
	var bar = box.querySelector('.flowbie-speed-images-bulk__bar');
	var running = false;
	var page = 1;
