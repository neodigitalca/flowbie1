(function () {
  ...
  if (typeof flowbieSuperMigrate === 'undefined') {
    return;
  }

  var cfg = flowbieSuperMigrate;
  var jobId = null;
  var pollTimer = null;
  var running = false;
  ...
  var elImport = document.getElementById('flowbie-sm-import');
  var elProgressWrap = document.getElementById('flowbie-sm-progress-wrap');
  var elProgressFill = document.getElementById('flowbie-sm-progress-fill');
  var elProgressBar = document.getElementById('flowbie-sm-progress-bar');
  var elStatus = document.getElementById('flowbie-sm-status');
  var elHudHeadline = document.getElementById('flowbie-sm-hud-headline');
  var elMacroRow = document.getElementById('flowbie-sm-macro-row');
  var elOverallPct = document.getElementById('flowbie-sm-overall-pct');
  var elMicroGrid = document.getElementById('flowbie-sm-micro-grid');
  var elConflicts = document.getElementById('flowbie-sm-conflicts');
  var elConflictList = document.getElementById('flowbie-sm-conflict-list');
  var elDeactivate = document.getElementById('flowbie-sm-deactivate');
  var elSkipConflicts = document.getElementById('flowbie-sm-skip-conflicts');
  var elRestore = document.getElementById('flowbie-sm-restore');
  var elRestoreList = document.getElementById('flowbie-sm-restore-list');
  var elRestoreBtn = document.getElementById('flowbie-sm-restore-btn');
  var elRestoreResult = document.getElementById('flowbie-sm-restore-result');
  var elConflictsResult = document.getElementById('flowbie-sm-conflicts-result');
  ...
  function ensureMicroRow(key, step) {
    ...
    row.className = 'flowbie-sm-stream__row is-pending';
    row.setAttribute('role', 'listitem');
    row.dataset.stepKey = key;

    var head = document.createElement('div');
    head.className = 'flowbie-sm-stream__head';

    var label = document.createElement('span');
    label.className = 'flowbie-sm-stream__label';
    label.textContent = step.label || step.id || 'Step';

    var sig = document.createElement('span');
    sig.className = 'flowbie-sm-stream__sig';
    ...
    track.className = 'flowbie-sm-stream__track';
    var fill = document.createElement('span');
    fill.className = 'flowbie-sm-stream__fill';
  ...
  function renderMacroRow(job) {
    ...
    (job.macro || []).forEach(function (macro) {
      ...
      chip.className = 'flowbie-sm-hud__macro-chip';
  ...
  function renderMicroStream(job) {
    ...
    (job.micro || []).forEach(function (step, index) {
      ...
      var st = step.status || 'pending';
      parts.root.className = 'flowbie-sm-stream__row is-' + st;
  ...
  function renderRestorePlugins(plugins) {
    ...
    plugins.forEach(function (plugin) {
      ...
      li.className = 'flowbie-wp-super-migrate__restore-item';
  ...
  function renderConflictPlugins(plugins) {
    ...
    plugins.forEach(function (plugin) {
      var li = document.createElement('li');
      li.className = 'flowbie-wp-super-migrate__conflict-item';

      var input = document.createElement('input');
      input.type = 'checkbox';
      input.className = 'flowbie-sm-conflict-checkbox';
      input.value = plugin.file;
      input.checked = true;
      input.id = 'flowbie-sm-conflict-' + plugin.file.replace(/[^a-z0-9_-]/gi, '-');
  ...
  function selectedConflictFiles() {
    ...
    var boxes = elConflictList.querySelectorAll('.flowbie-sm-conflict-checkbox:checked');
    var files = [];
