(function () {
  ...
  var copyLogBtn = document.getElementById('flowbie-chat-demo-copy-log');
  if (copyLogBtn) {
  ...
  btn.addEventListener('mousedown', function () {
    if (window.FlowbieChatPrefetch && input && input.value.trim()) {
      FlowbieChatPrefetch.prefetchOnSendIntent(input);
    }
  });
  ...
  if (window.FlowbieChatPrefetch && input) {
    FlowbieChatPrefetch.bindComposer(input, prefetchOptions());
    var starterPrompts = [];
    root.querySelectorAll('.fcw-demo-starter').forEach(function (chip) {
    ...
    if (starterPrompts.length) {
      FlowbieChatPrefetch.prefetchSuggestions(starterPrompts, prefetchOptions());
    }
  }
  ...
  function runStream(text) {
    ...
    var prefetchHit = window.FlowbieChatPrefetch
      ? FlowbieChatPrefetch.consumeForSubmit(text, history.slice(-10))
      : null;
    if (window.FlowbieChatDebugLog) {
    ...
    if (streamCtx && window.FlowbieChatStream) {
      FlowbieChatStream.setTypingIndicator(streamCtx, msgs, scrollDown);
