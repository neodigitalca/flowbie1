...
(function ($) {
  ...
  function collectTokens($root) {
    var tokens = {};
    var source = $root.find('[data-flowbie-color-source]:checked').val() || 'site_branding';
    var widget = $root.data('flowbie-design-widget') || 'search';
    var resolved = (typeof flowbieDesignPreview !== 'undefined' && flowbieDesignPreview)
      ? (widget === 'chat' ? flowbieDesignPreview.chatTokens : flowbieDesignPreview.searchTokens)
      : null;
    ...
    $root.find('[name^="flowbie_design[tokens]"]').each(function () {
      var name = this.name;
      var m = name.match(/flowbie_design\[tokens\]\[([^\]]+)\]/);
      if (!m) return;
  ...
  function collectUi($root) {
    var ui = {};
    $root.find('[name^="flowbie_design[ui]"]').each(function () {
      var name = this.name;
      var m = name.match(/flowbie_design\[ui\]\[([^\]]+)\]/);
      if (!m) return;
  ...
  function refresh() {
    var $root = $('.flowbie-ai-widget-design').first();
    ...
    var searchDemo = document.querySelector('.flowbie-wp-search-demo .flowbie-search-wrap');
    if (!searchDemo) {
      searchDemo = document.querySelector('.flowbie-search-wrap');
    }
    if (searchDemo) {
  ...
  $(document).on('flowbie-design-color-change', refresh);
  $(document).on('input change', '.flowbie-ai-widget-design', refresh);
  $(function () {
    refresh();
  });
})(jQuery);
