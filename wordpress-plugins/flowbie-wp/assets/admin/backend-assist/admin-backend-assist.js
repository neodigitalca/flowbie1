(function(){
	...
	try{
		var storedMode=sessionStorage.getItem('flowbie_fba_mode');
		if(storedMode==='chat'||storedMode==='backend'){assistMode=storedMode;}
	}catch(_){}

	try{
		var prefill=sessionStorage.getItem('flowbie_fba_prefill');
		if(prefill&&input){
			input.value=prefill;
			input.focus();
			sessionStorage.removeItem('flowbie_fba_prefill');
			var taskId=sessionStorage.getItem('flowbie_fba_prefill_task_id')||'';
			sessionStorage.removeItem('flowbie_fba_prefill_task_id');
			if(taskId&&msgs){
	...
	function setAssistMode(mode){
		...
		try{sessionStorage.setItem('flowbie_fba_mode',assistMode);}catch(_){}
	...
	function runFlowChat(text){
		...
		var url=chatAjaxUrl+'?action=flowbie_chat_stream&_nonce='+encodeURIComponent(chatStreamNonce||'');
		fetch(url,{
			method:'POST',
