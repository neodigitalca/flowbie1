(function ($) {
	...
	var cfg = window.FlowbieSeoBlockEditor || {};
	...
	function updateLinkedSummary(panel, block) {
		...
		var html = '<div class="flowbie-seo-block-linked-summary">' +
			...
			'</div>';
		panel.$el.find('.flowbie-seo-block-linked-summary').html(html);
	}
	...
	function bindRegistrySelector(model, panel) {
		var settings = model.get('settings');
		if (!settings || settings.flowbieRegistryBound) {
			return;
		}
		settings.flowbieRegistryBound = true;
		...
		settings.on('change:registry_block_id', function () {
			...
			if (id < 1) {
				...
				if (panel && panel.$el) {
					panel.$el.find('.flowbie-seo-block-linked-summary').text(cfg.i18n.selectBlock || 'Select a block from the Agent Hub table first.');
				}
				return;
			}
	...
	function injectWands() {
		...
		elementor.hooks.addFilter('panel/elements/regionViews', function (panel) {
			if (panel.flowbieSeoWandsBound) return panel;
			panel.flowbieSeoWandsBound = true;
			var orig = panel.footer && panel.footer.currentView;
			return panel;
		});

		elementor.hooks.addAction('panel/open_editor/widget/flowbie_seo_section', function panelOpen(panel, model) {
			setTimeout(function () {
	...
	function mountWandControls(panel, model) {
		...
		if (!$footer.length || $footer.find('.flowbie-seo-block-wands').length) return;
		var html = '<div class="flowbie-seo-block-wands">' +
			'<button type="button" class="flowbie-seo-block-wand flowbie-seo-block-wand--black" data-mode="full" title="' + esc(cfg.i18n.optimizeFull) + '">✦</button>' +
			'<button type="button" class="flowbie-seo-block-wand flowbie-seo-block-wand--white" data-mode="intent" title="' + esc(cfg.i18n.optimizeIntent) + '">✦</button>' +
			'</div>';
		$footer.prepend(html);
		$footer.find('.flowbie-seo-block-wand').on('click', function () {
	...
	function showModal(data) {
		...
		var $modal = $('<div class="flowbie-seo-block-modal"><div class="flowbie-seo-block-modal__panel">' +
			'<h3>' + esc(cfg.i18n.previewTitle) + '</h3>' +
			'<p><strong>Before</strong></p><pre></pre>' +
			'<p><strong>After</strong></p><pre></pre>' +
			'<div class="flowbie-seo-block-modal__actions">' +
			'<button type="button" class="button button-primary flowbie-seo-block-apply">' + esc(cfg.i18n.apply) + '</button>' +
			'<button type="button" class="button flowbie-seo-block-cancel">' + esc(cfg.i18n.cancel) + '</button>' +
			'</div></div></div>');
		$modal.find('pre').eq(0).text(before);
		$modal.find('pre').eq(1).text(after);
		$('body').append($modal);
		$modal.on('click', '.flowbie-seo-block-cancel', closeModal);
		$modal.on('click', '.flowbie-seo-block-apply', applyPreview);
	}

	function closeModal() {
		$('.flowbie-seo-block-modal').remove();
	}
