(function () {
	...
	function readEntriesFromRow(row) {
		var entries = [];
		row.querySelectorAll('.flowbie-wp-ai-faq-item').forEach(function (item) {
	...
	function syncRowToDraft(ctrl, row) {
		...
		row.classList.toggle('is-dirty', window.FlowbieAiShared.isFieldDirty(ctrl, 'faq'));
		var applyLink = row.querySelector('.flowbie-wp-ai-editor-row__apply');
		if (applyLink) {
			applyLink.style.display = window.FlowbieAiShared.isFieldDirty(ctrl, 'faq') && ctrl.status && ctrl.status.canApply ? '' : 'none';
		}
		if (window.FlowbieAiModal && window.FlowbieAiModal.updateModalFooter) {
			window.FlowbieAiModal.updateModalFooter(ctrl);
		}
	}
	...
	function setFaqWandLoading(row, loading, label) {
		if (!row) return;
		var btn = row.querySelector('.flowbie-wp-ai-spark-btn');
		if (!btn) return;
	...
	function mountFaqAccordion(ctrl, row, entries, openIndex) {
		...
		var str = S.str;
		var el = D.el;
		var accordion = row.querySelector('.flowbie-wp-ai-faq-accordion');
		if (!accordion) return;

		accordion.innerHTML = '';
		(entries || []).forEach(function (entry, index) {
			var item = el('div', 'flowbie-wp-ai-faq-item' + (index === openIndex ? ' is-open' : ''));
			var triggerLabel = entry.question || (str('faqQuestion', 'Question') + ' ' + (index + 1));
			var trigger = el('button', 'flowbie-wp-ai-faq-item__trigger', triggerLabel);
			trigger.type = 'button';
			trigger.setAttribute('aria-expanded', index === openIndex ? 'true' : 'false');
			trigger.title = triggerLabel;

			var panel = el('div', 'flowbie-wp-ai-faq-item__panel');
			panel.appendChild(el('label', 'flowbie-wp-ai-faq-item__label', str('faqQuestion', 'Question')));
			var qInput = document.createElement('input');
			qInput.type = 'text';
			qInput.className = 'flowbie-wp-ai-editor-row__input widefat';
			qInput.setAttribute('data-faq-q', '1');
			qInput.value = entry.question || '';
			panel.appendChild(qInput);

			panel.appendChild(el('label', 'flowbie-wp-ai-faq-item__label', str('faqAnswer', 'Answer')));
			var aInput = document.createElement('textarea');
			aInput.className = 'flowbie-wp-ai-editor-row__input widefat';
			aInput.rows = 5;
	...
	function runFaqWand(ctrl) {
		...
		return chain.then(function () {
			...
			setFaqWandLoading(row, false, str('wandTitle', 'Enhance with Flowbie AI'));
		}).catch(function (err) {
			ctrl.faqWandRunning = false;
			setFaqWandLoading(row, false, str('wandTitle', 'Enhance with Flowbie AI'));
	...
	function renderModalRow(ctrl, label) {
		...
		var row = el('div', 'flowbie-wp-ai-editor-row flowbie-wp-ai-faq-editor' + (S.isFieldDirty(ctrl, draftKey) ? ' is-dirty' : ''));
		...
		var copyBtn = el('button', 'flowbie-wp-ai-faq-copy button-link', str('copyFaqSchema', 'Copy all schema'));
		copyBtn.type = 'button';
		...
		applyLink.addEventListener('click', function () {
			window.FlowbieAiModal.runApplyContentField(self, 'faq', draftKey);
		});
		...
		actions.appendChild(D.createSparkButton({
			...
			onClick: function () {
				window.FlowbieAiFaq.runFaqWand(self);
			},
		}));
		head.appendChild(actions);
		row.appendChild(head);

		var accordion = el('div', 'flowbie-wp-ai-faq-accordion');
