(function (wp) {
	...
	window.FlowbieBodyHarnessApply = {
		...
		applySectionInEditor: function (postId, sectionIndex, html) {
			...
			console.log('[FlowbieBody] Apply section ' + sectionIndex + ', title: "' + title + '"');
			var blocks = getTopLevelBlocks();
			var range = findSectionRange(blocks, title);
			console.log('[FlowbieBody] Range:', range, 'blocks:', blocks.length);
			...
			if (!range) {
				console.warn('[FlowbieBody] No matching H2 for "' + title + '", skipping insert.');
