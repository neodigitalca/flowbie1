(function () {
	...
	function ensureSessionThenPreview(ctrl, sectionIndex) {
		...
		if (state.sessionId) {
			return window.FlowbieBodyHarness.previewSection(ctrl.postId, sectionIndex, state.sessionId);
		}
		store.patchSection(sectionIndex, { status: 'generating' });
		store.setState({ loading: true, error: '' });
		return window.FlowbieBodyHarness.plan(ctrl.postId, ctrl).then(function () {
			var newState = store.state;
			return window.FlowbieBodyHarness.previewSection(ctrl.postId, sectionIndex, newState.sessionId);
		});
	}
	...
	function ensureSessionThenRunAll(ctrl) {
		...
		var state = store.state;
		if (state.sessionId) {
			return window.FlowbieBodyHarness.runAllPreviews(ctrl.postId, ctrl);
		}
		store.setState({ loading: true, error: '' });
		return window.FlowbieBodyHarness.plan(ctrl.postId, ctrl).then(function () {
			return window.FlowbieBodyHarness.runAllPreviews(ctrl.postId, ctrl);
	...
	function saveMetaLocal(ctrl) {
		if (window.FlowbieAiFaq) window.FlowbieAiFaq.syncDraftFromDom(ctrl);
		...
		S().api('/flowbie/v1/ai/save-meta', { method: 'POST', body: JSON.stringify(payload) })
			.then(function (body) {
				...
				if (savedKeys.faq && window.FlowbieAiFaq) window.FlowbieAiFaq.resetStorageBase(ctrl);
	...
	function callInsertElement(ctrl, sec, propPanel, store, elementType, customPrompt) {
		...
		S().api('/flowbie/v1/ai/body/insert-element', {
			method: 'POST',
			body: JSON.stringify({
				post_id: ctrl.postId,
	...
	function renderBody(ctrl) {
		...
		if (window.FlowbieAiUrlTool && status.isPublished) {
			var urlPanel = window.FlowbieAiUrlTool.renderPanel(ctrl);
		...
		if (state.structureDirty) {
			...
			saveStructBtn.addEventListener('click', function () {
				window.FlowbieBodyHarness.saveStructure(ctrl.postId).catch(function () {});
			});
		...
		if (status.bodyHarnessAvailable && state.sections.length) {
			...
			if (state.sessionId) {
				...
				clearBtn.addEventListener('click', function () {
					window.FlowbieBodyHarness.clearSession(ctrl.postId).then(function () {
				...
				if (sec.status === 'done' && sec.previewHtml) {
					...
					applyBtn.addEventListener('click', (function (idx, fallbackHtml) {
						return function (e) {
							...
							window.FlowbieBodyHarness.applySection(ctrl.postId, idx, state.sessionId, html, ctrl).catch(function () {});
						};
		...
		if (window.FlowbieAiFaq) {
			...
			faqZone.appendChild(window.FlowbieAiFaq.renderModalRow(ctrl, faqLabels['faq'] || 'FAQ'));
	...
	function loadSectionsFromPost(ctrl, store) {
		return S()
			.api('/flowbie/v1/ai/body/sections?post_id=' + encodeURIComponent(String(ctrl.postId)), { method: 'GET' })
			.then(function (resp) {
	...
	function open(ctrl) {
		...
		var overlay = el('div', 'flowbie-wp-ai-modal fbm-overlay');
		overlay.id = 'flowbie-body-modal';
		...
		loadSectionsFromPost(ctrl, store).then(function () {
			if (window.FlowbieBodyHarness) {
				return window.FlowbieBodyHarness.loadSession(ctrl.postId, true).then(function (resp) {
					if (resp.hasSession) {
	...
	function handleAutoLink(ctrl) {
		...
		if (linkDropdown) { linkDropdown.remove(); linkDropdown = null; }
		showAutoLinkLoading();
		S().api('/flowbie/v1/ai/body/suggest-link', {
			method: 'POST',
			body: JSON.stringify({
				post_id: ctrl.postId,
	...
	function handleManualLink(ctrl) {
		...
		function loadInventory() {
			...
			S().api('/flowbie/v1/ai/body/posts-inventory?post_id=' + encodeURIComponent(String(ctrl.postId)), { method: 'GET' })
				.then(function (resp) {
