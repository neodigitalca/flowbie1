(function () {
  ...
  var DEBOUNCE_MS = 300;
  var SESSION_KEY = 'flowbie_search_session_id';

  document.addEventListener('DOMContentLoaded', function () {
    var wraps = document.querySelectorAll('.flowbie-search-wrap:not(.flowbie-search-wrap--panel-inner)');
  ...
  function initSearch(wrap) {
    ...
    if (wrap.classList.contains('flowbie-search-wrap--panel-inner')) {
      return;
    }
    ...
    if (isSidebar && window.FlowbieAiSidebarShell && !isElementorEdit) {
      ...
      if (window.FlowbieAiSidebarUnify && window.FlowbieAiSidebarUnify.isMerged()) {
        sidebarShell = window.FlowbieAiSidebarUnify.getShell();
      } else {
        sidebarShell = window.FlowbieAiSidebarShell.init(wrap, {
    ...
    if (shellPanel) {
      var panelInner = shellPanel.querySelector('.flowbie-search-wrap--panel-inner');
      if (panelInner) {
    ...
    function doSearch() {
      ...
      if (isSidebar && sidebarShell && !sidebarShell.isOpen) {
        if (window.FlowbieAiSidebarUnify && window.FlowbieAiSidebarUnify.isMerged()) {
          window.FlowbieAiSidebarUnify.openTab('search');
        } else {
          sidebarShell.open();
        }
      }
  ...
  window.FlowbieSearch = window.FlowbieSearch || {};
  window.FlowbieSearch.initWrap = function (wrap) {
    if (wrap) {
      initSearch(wrap);
    }
  };
  window.FlowbieSearch.initAll = function (root) {
    var scope = root || document;
    var wraps = scope.querySelectorAll('.flowbie-search-wrap:not(.flowbie-search-wrap--panel-inner)');
    for (var i = 0; i < wraps.length; i++) {
      initSearch(wraps[i]);
    }
  };
