...
class Flowbie_Wp_Harness_Runner {
	...
	public static function preview_section( array $session, int $section_index, array $ctx ) {
		...
		if ( ! isset( $outline[ $section_index ] ) || ! is_array( $outline[ $section_index ] ) ) {
			return new WP_Error( 'flowbie_body_section', __( 'Invalid section index.', 'flowbie-wp' ) );
		}
		...
		if ( ! empty( $ctx['posts'] ) && is_array( $ctx['posts'] ) ) {
			$posts_block = Flowbie_Wp_Harness_Prompts::wordpress_posts_block(
				$ctx['posts'],
		...
		$user = Flowbie_Wp_Harness_Prompts::harness_section_user_prompt(
			array(
				...
				'outlineBlock'        => Flowbie_Wp_Harness_Prompts::format_outline_for_prompt( $outline ),
		...
		$result      = Flowbie_Wp_OpenRouter::complete_chat( $system, $user, $per_section, 1.0 );
		if ( is_wp_error( $result ) ) {
			return $result;
		}

		$html = Flowbie_Wp_Harness_Outline::strip_footer_from_section_html( (string) $result['content'] );
		if ( $html === '' ) {
			return new WP_Error( 'flowbie_body_empty', __( 'Harness returned empty section HTML.', 'flowbie-wp' ) );
		}
	...
	private static function maybe_length_compliance( string $html, string $section_title, array $siblings, string $article_title ): string {
		$raw = Flowbie_Wp_OpenRouter::complete(
			Flowbie_Wp_Harness_Prompts::length_compliance_system(),
			Flowbie_Wp_Harness_Prompts::length_compliance_user( $html, $section_title, $siblings, $article_title ),
			2048,
			0.3
		);
		if ( is_wp_error( $raw ) ) {
		...
		if ( is_array( $data ) && ! empty( $data['section_html'] ) && is_string( $data['section_html'] ) ) {
			return Flowbie_Wp_Harness_Outline::strip_footer_from_section_html( $data['section_html'] );
		}
		return $html;
	}
}
