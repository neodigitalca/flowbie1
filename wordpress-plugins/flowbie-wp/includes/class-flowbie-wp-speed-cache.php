...
/**
 * Stores optimized CSS/JS under wp-content/cache/flowbie-speed/.
 */
class Flowbie_Wp_Speed_Cache {

	const DIR_NAME = 'flowbie-speed';

	const LAST_WRITE_OPTION = 'flowbie_wp_speed_last_write';
	...
	private static function maybe_write_htaccess( string $base ): void {
		...
		if ( file_exists( $file ) ) {
			...
			if ( is_string( $existing ) && strpos( $existing, 'Flowbie-Cache-Control' ) !== false ) {
				return;
			}
			if ( is_string( $existing ) && strpos( $existing, 'Cache-Control' ) === false ) {
				$rules = rtrim( $existing ) . "\n\n# Flowbie-Cache-Control\n" . $rules;
	...
	public static function build_hash( string $source_key, string $type, array $config ): string {
		return md5( $type . '|' . FLOWBIE_WP_VERSION . '|' . wp_json_encode( $config ) . '|' . $source_key );
	}
	...
	public static function flush_all(): void {
		$base = self::cache_dir();
		if ( ! is_dir( $base ) ) {
			update_option( 'flowbie_wp_speed_last_flush', time(), false );
			return;
		}
	...
	public static function stats(): array {
		...
		return array(
			'file_count' => $count,
			'bytes'      => $bytes,
			'last_flush' => (int) get_option( 'flowbie_wp_speed_last_flush', 0 ),
		);
	}
}
