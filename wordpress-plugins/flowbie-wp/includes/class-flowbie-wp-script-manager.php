...
class Flowbie_Wp_Script_Manager {

	const TABLE_VERSION = '1.0';
	const OPTION_KEY    = 'flowbie_wp_script_manager_settings';

	const PLACEMENTS = array( 'header', 'footer', 'body' );

	public static function table_name(): string {
		global $wpdb;
		return $wpdb->prefix . 'flowbie_wp_scripts';
	}
	...
	public static function maybe_install(): void {
		if ( get_option( 'flowbie_wp_scripts_db_version', '' ) !== self::TABLE_VERSION ) {
			self::install();
		}
	}
	...
	public static function install(): void {
		...
		dbDelta( $sql );
		update_option( 'flowbie_wp_scripts_db_version', self::TABLE_VERSION, false );

		if ( get_option( self::OPTION_KEY, null ) === null ) {
			add_option(
				self::OPTION_KEY,
				array(
					'default_category'        => __( 'Uncategorized', 'flowbie-wp' ),
	...
	public static function get_settings(): array {
		...
		$default_category = isset( $raw['default_category'] ) ? sanitize_text_field( (string) $raw['default_category'] ) : __( 'Uncategorized', 'flowbie-wp' );
		if ( $default_category === '' ) {
			$default_category = __( 'Uncategorized', 'flowbie-wp' );
		}
		return array(
	...
	public static function save_settings( array $settings ): void {
		$default_category = isset( $settings['default_category'] ) ? sanitize_text_field( (string) $settings['default_category'] ) : __( 'Uncategorized', 'flowbie-wp' );
		if ( $default_category === '' ) {
			$default_category = __( 'Uncategorized', 'flowbie-wp' );
		}
		update_option(
	...
	public static function save( array $data ) {
		...
		if ( $name === '' ) {
			return array(
				'ok'    => false,
				'error' => __( 'Script name is required.', 'flowbie-wp' ),
			);
		}
		...
		if ( trim( $code ) === '' ) {
			return array(
				'ok'    => false,
				'error' => __( 'Script code is required.', 'flowbie-wp' ),
			);
		}
		...
		$rules_raw = isset( $data['display_rules'] ) ? $data['display_rules'] : Flowbie_Wp_Script_Manager_Rules::defaults();
		$normalized = Flowbie_Wp_Script_Manager_Rules::normalize( $rules_raw );
		if ( empty( $normalized['ok'] ) ) {
			return array(
				'ok'    => false,
				'error' => isset( $normalized['error'] ) ? (string) $normalized['error'] : __( 'Invalid display rules.', 'flowbie-wp' ),
			);
		}
		$display_rules = Flowbie_Wp_Script_Manager_Rules::encode( $normalized['rules'] );
		...
		if ( $id > 0 ) {
			...
			if ( ! $existing ) {
				return array(
					'ok'    => false,
					'error' => __( 'Script not found.', 'flowbie-wp' ),
				);
			}
			...
			if ( $conflict && (int) $conflict->id !== $id ) {
				return array(
					'ok'    => false,
					'error' => __( 'Another script already uses this name.', 'flowbie-wp' ),
				);
			}
			...
			if ( false === $updated ) {
				return array(
					'ok'    => false,
					'error' => $wpdb->last_error ? $wpdb->last_error : __( 'Database update failed.', 'flowbie-wp' ),
				);
			}
			return array(
				'ok' => true,
		...
		if ( false === $inserted ) {
			return array(
				'ok'    => false,
				'error' => $wpdb->last_error ? $wpdb->last_error : __( 'Database insert failed.', 'flowbie-wp' ),
			);
		}
	...
	public static function merge_import( array $rows ): array {
		...
		foreach ( $rows as $row ) {
			if ( ! is_array( $row ) ) {
				++$skipped;
				$errors[] = __( 'Skipped invalid import row.', 'flowbie-wp' );
				continue;
			}
			$id = isset( $row['id'] ) ? (int) $row['id'] : 0;
			$name = isset( $row['name'] ) ? (string) $row['name'] : __( 'Unnamed script', 'flowbie-wp' );
			if ( $id > 0 && self::get( $id ) ) {
				$row['id'] = $id;
				...
				if ( ! empty( $result['ok'] ) ) {
					...
					$errors[] = sprintf(
						...
						$name,
						isset( $result['error'] ) ? (string) $result['error'] : __( 'Save failed.', 'flowbie-wp' )
					);
				}
				continue;
			}
