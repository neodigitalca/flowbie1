...
class Flowbie_Wp_Ai_Gsc {
	...
	public static function get_suggestions( int $post_id, string $current_keyword = '' ) {
		if ( $post_id > 0 ) {
			...
			if ( $urls['pageUrl'] !== '' && Flowbie_Wp_Gsc::is_available() ) {
		...
		if ( ! Flowbie_Wp_Ai_Backend::is_available() ) {
			return new WP_Error(
				'flowbie_gsc_unavailable',
				__( 'GSC suggestions require Search Console credentials or the Flowbie API URL.', 'flowbie-wp' )
			);
		}
		...
		if ( $urls['pageUrl'] === '' ) {
			return array(
				...
				'queries'     => array(),
				'topKeyword'  => null,
				'message'     => __( 'No page URL available for GSC lookup.', 'flowbie-wp' ),
			);
		}

		$cache_key = 'flowbie_wp_gsc_kw_' . md5( $post_id . '|' . $urls['pageUrl'] );
		...
		$data = Flowbie_Wp_Ai_Backend::post_json(
		...
		if ( empty( $data['success'] ) ) {
			return array(
				...
				'queries'     => array(),
				'topKeyword'  => null,
				'message'     => isset( $data['error'] ) ? (string) $data['error'] : __( 'GSC data unavailable.', 'flowbie-wp' ),
			);
		}
		...
		if ( empty( $payload['suggestions'] ) && ! empty( $queries ) ) {
			$payload['message'] = __( 'No suitable GSC queries after filtering.', 'flowbie-wp' );
		} elseif ( empty( $payload['suggestions'] ) ) {
			$payload['message'] = __( 'No GSC data for this URL yet.', 'flowbie-wp' );
		}
	...
	private static function get_suggestions_from_direct( int $post_id, array $urls, string $current_keyword ) {
		$cache_key = 'flowbie_wp_gsc_kw_direct_' . md5( $post_id . '|' . $urls['pageUrl'] );
		$cached    = get_transient( $cache_key );
		if ( is_array( $cached ) ) {
			$cached['suggestions'] = self::filter_suggestions(
		...
		$page_data = Flowbie_Wp_Gsc::fetch_page_queries( $urls['pageUrl'] );
