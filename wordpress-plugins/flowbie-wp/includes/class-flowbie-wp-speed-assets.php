...
class Flowbie_Wp_Speed_Assets {
	...
	public static function process( string $html, array $config ): string {
		if ( ! empty( $config['optimize_css'] ) ) {
			if ( ! empty( $config['aggregate_css'] ) ) {
				$html = Flowbie_Wp_Speed_Aggregator::aggregate_stylesheets( $html, $config );
			} else {
		...
		if ( ! empty( $config['optimize_js'] ) ) {
			if ( ! empty( $config['aggregate_js'] ) ) {
				$html = Flowbie_Wp_Speed_Aggregator::aggregate_scripts( $html, $config );
			} else {
	...
	public static function minify_file_to_cache( string $path, string $type, array $config ): ?string {
		...
		$key   = Flowbie_Wp_Speed_Cache::build_hash( $path . ':' . $mtime, $type, $config );
		$url   = Flowbie_Wp_Speed_Cache::get_url( $type, $key );
		if ( $url !== null ) {
			return $url;
		}
		...
		$min = 'css' === $type ? Flowbie_Wp_Speed_Minify::css( $raw ) : Flowbie_Wp_Speed_Minify::js( $raw );
		if ( 'css' === $type && ! empty( $config['font_display_swap'] ) ) {
			$min = Flowbie_Wp_Speed_Minify::ensure_font_display_swap( $min );
		}
		return Flowbie_Wp_Speed_Cache::write( $type, $key, $min );
	}
	...
	private static function defer_scripts( string $html, array $config ): string {
		return (string) preg_replace_callback(
			...
			static function ( $m ) use ( $config ) {
				...
				if ( Flowbie_Wp_Speed_Excludes::is_defer_excluded( $url, $config ) ) {
					return $tag;
				}
