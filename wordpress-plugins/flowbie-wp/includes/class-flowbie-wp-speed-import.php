...
class Flowbie_Wp_Speed_Import {
	...
	public static function preset_labels(): array {
		return array(
			self::PRESET_ELEMENTOR_SAFE => __( 'Elementor-safe (guest-safe transforms off)', 'flowbie-wp' ),
			self::PRESET_DISABLE        => __( 'Disable Speed (diagnostic)', 'flowbie-wp' ),
		);
	}
	...
	public static function preset_file_path( string $preset_id ): string {
		...
		return FLOWBIE_WP_PLUGIN_DIR . 'presets/speed-' . $preset_id . '.json';
	}
	...
	public static function load_preset( string $preset_id ) {
		...
		if ( ! is_readable( $path ) ) {
			return new WP_Error( 'flowbie_speed_preset_missing', __( 'Preset file is missing from the plugin.', 'flowbie-wp' ) );
		}
		$raw = file_get_contents( $path );
		if ( ! is_string( $raw ) || trim( $raw ) === '' ) {
			return new WP_Error( 'flowbie_speed_preset_empty', __( 'Preset file is empty.', 'flowbie-wp' ) );
		}
	...
	public static function parse_json( string $json ) {
		...
		if ( $json === '' ) {
			return new WP_Error( 'flowbie_speed_import_empty', __( 'JSON file is empty.', 'flowbie-wp' ) );
		}

		$data = json_decode( $json, true );
		if ( ! is_array( $data ) ) {
			return new WP_Error( 'flowbie_speed_import_invalid', __( 'Invalid JSON. Use a file exported from Flowbie → Speed → Cache.', 'flowbie-wp' ) );
		}

		$data = Flowbie_Wp_Speed_Export::redact_secrets( $data );
		...
		if ( isset( $data['speed'] ) && is_array( $data['speed'] ) ) {
			...
			return new WP_Error( 'flowbie_speed_import_no_speed', __( 'JSON does not contain a "speed" settings object.', 'flowbie-wp' ) );
		}
	...
	public static function apply( array $payload, array $options = array() ) {
		...
		if ( $import_speed ) {
			if ( empty( $payload['speed'] ) || ! is_array( $payload['speed'] ) ) {
				return new WP_Error( 'flowbie_speed_import_no_speed', __( 'Nothing to import for Speed settings.', 'flowbie-wp' ) );
			}
			$config = Flowbie_Wp_Speed_Settings::sanitize_config(
				Flowbie_Wp_Speed_Settings::merge_with_defaults( $payload['speed'] )
			);
			if ( ! empty( $config['enabled'] ) ) {
				$config = Flowbie_Wp_Speed_Settings::apply_simple_enabled_config( $config );
			}
			Flowbie_Wp_Speed_Settings::save_config( $config );
			$parts[] = __( 'Speed', 'flowbie-wp' );
		}

		if ( $import_speed_images && ! empty( $payload['speed_images'] ) && is_array( $payload['speed_images'] ) ) {
			$images = Flowbie_Wp_Speed_Image_Settings::sanitize_config(
				wp_parse_args( $payload['speed_images'], Flowbie_Wp_Speed_Image_Settings::default_config() )
			);
			Flowbie_Wp_Speed_Image_Settings::save_config( $images );
			$parts[] = __( 'Speed Images', 'flowbie-wp' );
		}

		if ( empty( $parts ) ) {
			return new WP_Error( 'flowbie_speed_import_nothing', __( 'No settings were applied.', 'flowbie-wp' ) );
		}
		...
		update_option( Flowbie_Wp_Speed_Settings::ELEMENTOR_SAFE_MIGRATION_KEY, '1', false );

		return array(
			'success' => true,
			'message' => sprintf(
				/* translators: %s: comma-separated section names */
				__( 'Imported %s settings and ran a full WordPress + Flowbie cache flush. Test in a private window.', 'flowbie-wp' ),
				implode( ', ', $parts )
			),
		);
	}
	...
	public static function apply_preset( string $preset_id ) {
		...
		if ( ! isset( self::preset_labels()[ $preset_id ] ) ) {
			return new WP_Error( 'flowbie_speed_preset_unknown', __( 'Unknown preset.', 'flowbie-wp' ) );
		}
		...
		$result['message'] = sprintf(
			...
			__( 'Applied preset: %s. %s', 'flowbie-wp' ),
