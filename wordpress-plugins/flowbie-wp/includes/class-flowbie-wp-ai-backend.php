...
class Flowbie_Wp_Ai_Backend {

	public static function api_base(): string {
		$settings = Flowbie_Wp_Api::get_effective_settings();
		...
		return (string) apply_filters( 'flowbie_wp_flow_api_base', $base );
	}
	...
	public static function post_json( string $path, array $body, int $timeout = 120 ) {
		...
		if ( $base === '' ) {
			return new WP_Error(
				'flowbie_flow_api_missing',
				__( 'Flowbie API URL is not configured. Set it under Flowbie WP → Settings or FLOWBIE_WP_DEFAULT_API_BASE in the plugin build.', 'flowbie-wp' )
			);
		}
		...
		if ( $code < 200 || $code >= 300 ) {
			...
			return new WP_Error( 'flowbie_flow_api_http', $msg, array( 'status' => $code ) );
		}
	...
	public static function resolve_urls( int $post_id ): array {
		...
		if ( $page === '' ) {
			$page = esc_url_raw( trim( Flowbie_Wp_Ai_Context::read_field_value( $post_id, 'page_url' ) ) );
		}
