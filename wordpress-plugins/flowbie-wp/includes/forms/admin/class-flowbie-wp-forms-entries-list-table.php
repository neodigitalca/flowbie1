...
class Flowbie_Wp_Forms_Entries_List_Table extends WP_List_Table {
	...
	public function get_columns(): array {
		...
		$cols['status'] = __( 'Status', 'flowbie-wp' );
		return $cols;
	}
	...
	protected function column_id( $item ): string {
		$id  = (int) ( $item['id'] ?? 0 );
		$url = admin_url( 'admin.php?page=flowbie-wp-forms-entries&form_id=' . $this->form_id . '&entry_id=' . $id );
	...
	protected function column_default( $item, $column_name ) {
		...
		if ( is_numeric( $value ) && (string) (int) $value === (string) $value ) {
			...
			if ( $url ) {
				return '<a href="' . esc_url( $url ) . '" target="_blank" rel="noopener">' . esc_html__( 'File', 'flowbie-wp' ) . '</a>';
			}
		}
	...
	public function prepare_items(): void {
		...
		$result = Flowbie_Wp_Forms_Entries::list_entries(
			array(
	...
	public function no_items(): void {
		esc_html_e( 'No entries yet.', 'flowbie-wp' );
	}
}
