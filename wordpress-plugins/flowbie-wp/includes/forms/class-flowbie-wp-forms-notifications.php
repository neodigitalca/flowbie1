...
class Flowbie_Wp_Forms_Notifications {
	...
	public static function send_admin_notification( array $form, array $entry ): void {
		...
		$title   = (string) ( $form['title'] ?? __( 'Form', 'flowbie-wp' ) );
		$subject = sprintf(
			/* translators: %s: form title */
			__( 'New entry: %s', 'flowbie-wp' ),
			$title
		);

		$lines   = array();
		$lines[] = sprintf( __( 'Form: %s', 'flowbie-wp' ), $title );
		$lines[] = sprintf( __( 'Entry ID: %s', 'flowbie-wp' ), (string) ( $entry['entry_uid'] ?? $entry['id'] ?? '' ) );
		$lines[] = sprintf( __( 'Submitted: %s', 'flowbie-wp' ), (string) ( $entry['created_at'] ?? '' ) );
		$lines[] = '';
		$lines[] = __( 'Fields:', 'flowbie-wp' );
		...
		$admin_url = admin_url(
			'admin.php?page=flowbie-wp-forms-entries&form_id=' . (int) ( $form['ID'] ?? 0 ) . '&entry_id=' . (int) ( $entry['id'] ?? 0 )
		);
		$lines[]   = '';
		$lines[]   = __( 'View in WordPress admin:', 'flowbie-wp' ) . ' ' . $admin_url;
