...
class Flowbie_Wp_Forms_Rest {
	...
	public static function handle_submit( WP_REST_Request $request ) {
		...
		$nonce   = $request->get_header( 'X-Flowbie-Form-Nonce' );
		if ( ! $nonce ) {
			$nonce = $request->get_param( '_wpnonce' );
		}
		if ( ! is_string( $nonce ) || ! wp_verify_nonce( $nonce, 'flowbie_form_submit_' . $form_id ) ) {
		...
		$files = Flowbie_Wp_Forms_Submit::normalize_files();

		$result = Flowbie_Wp_Forms_Submit::process(
			$form_id,
			$input,
			array(
