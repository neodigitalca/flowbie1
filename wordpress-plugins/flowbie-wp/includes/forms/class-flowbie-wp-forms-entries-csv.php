...
class Flowbie_Wp_Forms_Entries_Csv {
	...
	public static function build_headers( array $fields ): array {
		...
		foreach ( $fields as $field ) {
			...
			foreach ( Flowbie_Wp_Forms_Field_Registry::csv_columns_for_field( $field ) as $col ) {
				$headers[] = $col;
			}
		}
		return $headers;
	...
	public static function meta_values_for_field( array $field, array $meta ): array {
		...
		if ( $name === '' || in_array( $type, Flowbie_Wp_Forms_Field_Registry::display_only_types(), true ) ) {
			return array();
		}
		$value = $meta[ $name ] ?? '';
		if ( $type === 'name' || $type === 'address' ) {
			$values = array();
			$cols   = Flowbie_Wp_Forms_Field_Registry::csv_columns_for_field( $field );
