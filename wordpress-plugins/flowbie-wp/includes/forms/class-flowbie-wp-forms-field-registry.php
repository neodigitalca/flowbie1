...
class Flowbie_Wp_Forms_Field_Registry {
	...
	public static function choices(): array {
		...
		return apply_filters( 'flowbie_wp_forms_field_types', $flat );
	}
	...
	public static function choices_grouped(): array {
		$groups = array(
			'standard' => array(
				'text'     => __( 'Single Line Text', 'flowbie-wp' ),
				'textarea' => __( 'Paragraph Text', 'flowbie-wp' ),
				'select'   => __( 'Drop Down', 'flowbie-wp' ),
				'number'   => __( 'Number', 'flowbie-wp' ),
				'checkbox' => __( 'Checkboxes', 'flowbie-wp' ),
				'radio'    => __( 'Radio Buttons', 'flowbie-wp' ),
				'hidden'   => __( 'Hidden', 'flowbie-wp' ),
				'html'     => __( 'HTML', 'flowbie-wp' ),
				'section'  => __( 'Section', 'flowbie-wp' ),
			),
			'advanced' => array(
				'name'    => __( 'Name', 'flowbie-wp' ),
				'date'    => __( 'Date', 'flowbie-wp' ),
				'time'    => __( 'Time', 'flowbie-wp' ),
				'phone'   => __( 'Phone', 'flowbie-wp' ),
				'address' => __( 'Address', 'flowbie-wp' ),
				'website' => __( 'Website', 'flowbie-wp' ),
				'email'   => __( 'Email', 'flowbie-wp' ),
				'file'    => __( 'File Upload', 'flowbie-wp' ),
				'consent' => __( 'Consent', 'flowbie-wp' ),
			),
		);
		return apply_filters( 'flowbie_wp_forms_field_type_groups', $groups );
	}
	...
	public static function default_field( string $type = 'text' ): array {
		...
		$label = self::choices()[ $type ] ?? __( 'Field', 'flowbie-wp' );
		...
		if ( in_array( $type, array( 'select', 'radio', 'checkbox' ), true ) ) {
			$field['choices'] = array(
				array( 'label' => __( 'Option 1', 'flowbie-wp' ), 'value' => 'option_1' ),
				array( 'label' => __( 'Option 2', 'flowbie-wp' ), 'value' => 'option_2' ),
			);
		}
		if ( $type === 'consent' ) {
			$field['required']      = true;
			$field['consent_label'] = __( 'I agree to the privacy policy.', 'flowbie-wp' );
		}
		if ( $type === 'section' ) {
			$field['label'] = __( 'Section', 'flowbie-wp' );
		}
		if ( $type === 'name' ) {
			$field['label'] = __( 'Name', 'flowbie-wp' );
			$field['name']  = 'name';
		}
		if ( $type === 'address' ) {
			$field['label'] = __( 'Address', 'flowbie-wp' );
			$field['name']  = 'address';
		}
		return $field;
	}
	...
	public static function default_settings(): array {
		return array(
			'description'          => '',
			'submit_button_label'  => __( 'Submit', 'flowbie-wp' ),
			'success_message'      => __( 'Thank you for your submission.', 'flowbie-wp' ),
			'redirect_url'         => '',
