<?php
/**
 * Shared Elementor repeater controls and schema conversion for Flowbie Forms.
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Forms_Field_Controls {
	...
	public static function get_type_options(): array {
		$options = array();
		foreach ( Flowbie_Wp_Forms_Field_Registry::choices_grouped() as $group => $types ) {
			$prefix = $group === 'advanced' ? __( 'Advanced', 'flowbie-wp' ) . ': ' : '';
	...
	public static function register_repeater_content_controls( $repeater ): void {
		$repeater->add_control(
			'field_id',
			array(
				'label'   => esc_html__( 'Field ID', 'flowbie-wp' ),
		...
		$repeater->add_control(
			'type',
			array(
				'label'   => esc_html__( 'Field type', 'flowbie-wp' ),
		...
		$repeater->add_control(
			'label',
			array(
				'label'   => esc_html__( 'Label', 'flowbie-wp' ),
		...
		$repeater->add_control(
			'name',
			array(
				'label'     => esc_html__( 'Field name', 'flowbie-wp' ),
				'type'      => \Elementor\Controls_Manager::TEXT,
				'default'   => '',
				'condition' => array(
		...
		$repeater->add_control(
			'required',
			array(
				'label'        => esc_html__( 'Required', 'flowbie-wp' ),
		...
		$repeater->add_control(
			'default_value',
			array(
				'label'     => esc_html__( 'Default value', 'flowbie-wp' ),
				'type'      => \Elementor\Controls_Manager::TEXT,
				'condition' => array(
					'type' => $text_types,
				),
		...
		$repeater->add_control(
			'choices_text',
			array(
				'label'       => esc_html__( 'Choices', 'flowbie-wp' ),
				'type'        => \Elementor\Controls_Manager::TEXTAREA,
				'description' => esc_html__( 'One per line: Label|value', 'flowbie-wp' ),
				'condition'   => array(
		...
		$repeater->add_control(
			'consent_label',
			array(
				'label'     => esc_html__( 'Consent text', 'flowbie-wp' ),
		...
		$repeater->add_control(
			'allowed_mime_types',
			array(
				'label'     => esc_html__( 'Allowed file types', 'flowbie-wp' ),
				'type'      => \Elementor\Controls_Manager::TEXT,
		...
		$repeater->add_control(
			'max_file_size_mb',
			array(
				'label'     => esc_html__( 'Max file size (MB)', 'flowbie-wp' ),
		...
		$repeater->add_control(
			'html_content',
			array(
				'label'     => esc_html__( 'HTML content', 'flowbie-wp' ),
				'type'      => \Elementor\Controls_Manager::WYSIWYG,
				'condition' => array(
					'type' => 'html',
				),
		...
		$repeater->add_control(
			'section_description',
			array(
				'label'     => esc_html__( 'Section description', 'flowbie-wp' ),
				'type'      => \Elementor\Controls_Manager::TEXTAREA,
				'condition' => array(
					'type' => 'section',
				),
		...
		self::register_subfield_toggles( $repeater, 'name', 'name_subfields', Flowbie_Wp_Forms_Field_Registry::default_name_subfields() );
		self::register_subfield_toggles( $repeater, 'address', 'address_subfields', Flowbie_Wp_Forms_Field_Registry::default_address_subfields() );

		$repeater->add_control(
			'css_class',
			array(
				'label'       => esc_html__( 'CSS classes', 'flowbie-wp' ),
				'type'        => \Elementor\Controls_Manager::TEXT,
				'description' => esc_html__( 'Space-separated custom classes.', 'flowbie-wp' ),
			)
		);
	}
	...
	public static function register_repeater_style_controls( $repeater ): void {
		$repeater->add_control(
			'field_width',
			array(
				'label'   => esc_html__( 'Field width', 'flowbie-wp' ),
				'type'    => \Elementor\Controls_Manager::SELECT,
				'default' => '100',
				'options' => array(
					'100' => '100%',
		...
		$repeater->add_control(
			'hide_label',
			array(
				'label'        => esc_html__( 'Hide label', 'flowbie-wp' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'return_value' => 'yes',
			)
		);
		...
		$repeater->add_control(
			'field_input_color',
			array(
				'label' => esc_html__( 'Input text color', 'flowbie-wp' ),
				'type'  => \Elementor\Controls_Manager::COLOR,
			)
		);
		...
		$repeater->add_control(
			'field_border_color',
			array(
				'label' => esc_html__( 'Input border color', 'flowbie-wp' ),
				'type'  => \Elementor\Controls_Manager::COLOR,
			)
		);
	}
	...
	public static function repeater_row_to_field( array $row ): array {
		...
		$field = Flowbie_Wp_Forms_Field_Registry::default_field( $type );
		$field['id']    = $field_id;
		$field['type']  = $type;
		...
		$field['name_subfields']    = self::row_subfields( $row, 'name_subfields', Flowbie_Wp_Forms_Field_Registry::default_name_subfields() );
		$field['address_subfields'] = self::row_subfields( $row, 'address_subfields', Flowbie_Wp_Forms_Field_Registry::default_address_subfields() );

		return Flowbie_Wp_Forms_Field_Registry::normalize_field( $field );
	}
	...
	public static function schema_to_repeater_rows( array $fields ): array {
		...
		foreach ( $fields as $field ) {
			...
			$field = Flowbie_Wp_Forms_Field_Registry::normalize_field( $field );
			$row   = array(
			...
			foreach ( Flowbie_Wp_Forms_Field_Registry::default_name_subfields() as $key => $default ) {
				$row[ 'name_subfields_' . $key ] = ! empty( $field['name_subfields'][ $key ] ) ? 'yes' : '';
			}
			foreach ( Flowbie_Wp_Forms_Field_Registry::default_address_subfields() as $key => $default ) {
	...
	public static function widget_settings_to_form_payload( array $settings ): array {
		...
		$key     = 'form_' . uniqid();
		if ( $form_id > 0 ) {
			$existing = Flowbie_Wp_Forms_Storage::get_form_by_id( $form_id );
		...
		return array(
			'ID'     => $form_id,
			'key'    => $key,
			'title'  => isset( $settings['form_title'] ) ? (string) $settings['form_title'] : __( 'Elementor Form', 'flowbie-wp' ),
			'active' => ! isset( $settings['form_active'] ) || $settings['form_active'] === 'yes',
			'settings' => array(
				'description'         => (string) ( $settings['form_description'] ?? '' ),
				'submit_button_label' => (string) ( $settings['submit_button_label'] ?? __( 'Submit', 'flowbie-wp' ) ),
	...
	public static function choices_to_text( array $choices ): string {
		$lines = array();
		foreach ( Flowbie_Wp_Forms_Field_Registry::normalize_choices( $choices ) as $choice ) {
	...
	private static function row_subfields( array $row, string $prefix, array $defaults ): array {
		...
		return Flowbie_Wp_Forms_Field_Registry::normalize_subfield_flags( $flags, $defaults );
	}
}
