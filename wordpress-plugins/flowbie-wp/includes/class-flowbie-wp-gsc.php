<?php
/**
 * Direct Google Search Console API client (no Flowbie Node API).
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Gsc {
	...
	const STATS_TTL     = 900;
	const EMAIL_FALLBACK = 'flowbie-812@flowbie-483717.iam.gserviceaccount.com';
	...
	public static function default_date_range(): array {
		...
		return array(
			'start' => $start,
			'end'   => $end,
			'label' => __( 'Last 28 days', 'flowbie-wp' ),
		);
	}
	...
	public static function clamp_date_range_for_gsc( string $from, string $to ): array {
		...
		if ( $to > $max_end ) {
			$note = sprintf(
				...
				__( 'GSC data ends %s due to Search Console reporting delay.', 'flowbie-wp' ),
				$max_end
			);
			$to = $max_end;
		}
	...
	public static function fetch_stats_for_range( string $from, string $to, int $page_limit = 30 ) {
		...
		$host = wp_parse_url( Flowbie_Wp_Api::get_site_url(), PHP_URL_HOST );
		$host = is_string( $host ) ? strtolower( $host ) : '';
		$key  = 'flowbie_wp_gsc_range_' . md5( $host . '|' . $start . '|' . $end . '|' . $page_limit . '|' . FLOWBIE_WP_VERSION );
	...
	public static function fetch_page_queries( string $page_url, string $from = '', string $to = '' ) {
		...
		$path_key = Flowbie_Wp_Overseer::normalize_path_url( $page_url );
		$host     = wp_parse_url( Flowbie_Wp_Api::get_site_url(), PHP_URL_HOST );
		$host     = is_string( $host ) ? strtolower( $host ) : '';
		$key      = 'flowbie_wp_gsc_page_' . md5( $host . '|' . $path_key . '|' . $start . '|' . $end . '|' . FLOWBIE_WP_VERSION );
		$cached   = get_transient( $key );
		if ( is_array( $cached ) ) {
			return $cached;
		}
	...
	private static function page_filter_urls( string $page_url ): array {
		...
		$site = rtrim( Flowbie_Wp_Api::get_site_url(), '/' );
	...
	public static function test_connection() {
		...
		$site_url = Flowbie_Wp_Api::get_site_url();
		...
		return array(
			...
			'message'         => $match['property'] !== null
				? __( 'Connected to Google Search Console.', 'flowbie-wp' )
				: __( 'Service account is valid but no matching GSC property was found for this site.', 'flowbie-wp' ),
	...
	public static function fetch_analytics_overview() {
		...
		$host  = wp_parse_url( Flowbie_Wp_Api::get_site_url(), PHP_URL_HOST );
		$host  = is_string( $host ) ? strtolower( $host ) : '';
		$key   = 'flowbie_wp_gsc_stats_' . md5( $host . '|' . $range['start'] . '|' . $range['end'] . '|' . FLOWBIE_WP_VERSION );
		$cached = get_transient( $key );
		if ( is_array( $cached ) ) {
			return $cached;
		}
	...
	public static function flush_stats_cache(): void {
		...
		$like = $wpdb->esc_like( '_transient_flowbie_wp_gsc_stats_' ) . '%';
		$rows = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s",
				$like,
				$wpdb->esc_like( '_transient_timeout_flowbie_wp_gsc_stats_' ) . '%'
			)
		);
		...
		foreach ( $rows as $option_name ) {
			...
			if ( 0 === strpos( $name, '_transient_timeout_flowbie_wp_gsc_stats_' ) ) {
				delete_option( $name );
				continue;
			}
			if ( 0 === strpos( $name, '_transient_flowbie_wp_gsc_stats_' ) ) {
	...
	private static function get_credentials() {
		...
		$sources = array(
			self::read_json_constant( 'FLOWBIE_WP_GSC_SERVICE_ACCOUNT_JSON' ),
			self::read_json_env( 'FLOWBIE_WP_GSC_SERVICE_ACCOUNT_JSON' ),
		...
		return new WP_Error(
			'flowbie_gsc_missing',
			__( 'GSC service account JSON is not configured for this plugin.', 'flowbie-wp' ),
			array(
				'clientEmail' => self::EMAIL_FALLBACK,
				'hint'        => sprintf(
					/* translators: %s: service account email */
					__( 'Add the service account JSON via wp-config (FLOWBIE_WP_GSC_SERVICE_ACCOUNT_JSON) or includes/credentials/gsc-service-account.json. In Google Search Console, add %s under Settings → Users and permissions.', 'flowbie-wp' ),
					self::EMAIL_FALLBACK
				),
			)
		);
	...
	private static function credentials_file_path(): string {
		if ( defined( 'FLOWBIE_WP_GSC_CREDENTIALS_FILE' ) && FLOWBIE_WP_GSC_CREDENTIALS_FILE !== '' ) {
			return (string) FLOWBIE_WP_GSC_CREDENTIALS_FILE;
		}
		return FLOWBIE_WP_PLUGIN_DIR . 'includes/credentials/gsc-service-account.json';
	}
	...
	private static function parse_credentials_json( string $raw ) {
		...
		if ( ! is_array( $data ) ) {
			return new WP_Error(
				'flowbie_gsc_bad_json',
				__( 'GSC service account JSON is invalid.', 'flowbie-wp' )
			);
		}
		...
		if ( $email === '' || $key === '' ) {
			return new WP_Error(
				'flowbie_gsc_incomplete',
				__( 'GSC service account JSON must include client_email and private_key.', 'flowbie-wp' )
			);
		}
	...
	private static function get_access_token() {
		...
		$cache_key = 'flowbie_wp_gsc_token_' . md5( $creds['client_email'] . '|' . FLOWBIE_WP_VERSION );
		...
		if ( $code < 200 || $code >= 300 || ! is_array( $data ) || empty( $data['access_token'] ) ) {
			...
			return new WP_Error( 'flowbie_gsc_token', $msg );
		}
	...
	private static function build_jwt( string $client_email, string $private_key ) {
		...
		if ( false === $key ) {
			return new WP_Error(
				'flowbie_gsc_key',
				__( 'Could not read GSC service account private key.', 'flowbie-wp' )
			);
		}
		...
		if ( ! $signed ) {
			return new WP_Error(
				'flowbie_gsc_sign',
				__( 'Could not sign GSC JWT.', 'flowbie-wp' )
			);
		}
	...
	private static function list_sites() {
		...
		if ( $code < 200 || $code >= 300 ) {
			$msg = is_array( $data ) && ! empty( $data['error']['message'] )
				...
				: ( $raw !== '' ? $raw : sprintf( 'HTTP %d', $code ) );
			return new WP_Error( 'flowbie_gsc_sites', $msg );
		}
	...
	private static function resolve_property( string $site_url, array $sites ): array {
		...
		if ( $requested === '' ) {
			return array(
				'property'        => null,
				'error'           => __( 'Could not determine site hostname.', 'flowbie-wp' ),
				'requestedDomain' => '',
			);
		}
		...
		return array(
			...
			'error'           => sprintf(
				...
				__( 'No GSC property matches %1$s. The service account can access %2$d properties.', 'flowbie-wp' ),
				$requested,
				count( $sites )
			),
			'requestedDomain' => $requested,
	...
	private static function search_analytics_query( string $property, array $body ) {
		...
		if ( $code < 200 || $code >= 300 ) {
			$msg = is_array( $data ) && ! empty( $data['error']['message'] )
				? (string) $data['error']['message']
				: ( $raw !== '' ? $raw : sprintf( 'HTTP %d', $code ) );
			return new WP_Error( 'flowbie_gsc_query', $msg );
		}
