<?php
/**
 * Rank Math redirect CSV import/export (ported from Flowbie app).
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Redirects_Csv {
	...
	public static function parse( string $csv_text ): array {
		...
		if ( empty( $out ) ) {
			return array(
				'rows'  => array(),
				'error' => __( 'No valid source/destination redirect rows found.', 'flowbie-wp' ),
			);
		}
