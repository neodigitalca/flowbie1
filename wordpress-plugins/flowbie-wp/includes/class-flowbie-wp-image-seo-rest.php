...
class Flowbie_Wp_Image_Seo_Rest {
	...
	public static function upload_permission() {
		...
		if ( ! Flowbie_Wp_Image_Seo_Gate::can_list() ) {
			return new WP_Error( 'rest_forbidden', __( 'You do not have permission to manage media.', 'flowbie-wp' ), array( 'status' => 403 ) );
		}
		return true;
	}
	...
	public static function attachment_permission( $request ) {
		...
		return Flowbie_Wp_Image_Seo_Gate::can_edit_attachment( $attachment_id, $post_id );
	}
	...
	public static function status() {
		return new WP_REST_Response( Flowbie_Wp_Image_Seo_Gate::get_status(), 200 );
	}
	...
	public static function get_attachment( $request ) {
		...
		return new WP_REST_Response(
			array(
				'ok'  => true,
				'row' => Flowbie_Wp_Image_Seo::attachment_row( $id ),
			),
			200
		);
	}
	...
	public static function bulk( $request ) {
		...
		if ( empty( $ids ) ) {
			return new WP_REST_Response(
				array(
					'ok'    => false,
					'error' => __( 'No attachments selected.', 'flowbie-wp' ),
				),
				400
			);
		}
		...
		$field_targets  = $fields ?? ( is_array( $config['fields'] ?? null ) ? $config['fields'] : Flowbie_Wp_Image_Seo::default_config()['fields'] );
		...
		foreach ( $ids as $id ) {
			...
			$preview = Flowbie_Wp_Image_Seo_Ai::preview( $id, 0, $use_ai, $field_targets, $overwrite_mode, $config );
			...
			$apply = Flowbie_Wp_Image_Seo_Ai::apply( $id, $preview['proposed'], $overwrite_mode, $field_targets );
	...
	public static function save( $request ) {
		...
		$result = Flowbie_Wp_Image_Seo::save_meta( (int) $params['attachment_id'], $values );
		if ( is_wp_error( $result ) ) {
			return self::error_response( $result );
		}
		return new WP_REST_Response(
			array(
				'ok'  => true,
				'row' => Flowbie_Wp_Image_Seo::attachment_row( (int) $params['attachment_id'] ),
			),
			200
		);
	}
