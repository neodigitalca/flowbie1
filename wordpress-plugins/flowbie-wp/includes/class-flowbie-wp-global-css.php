...
class Flowbie_Wp_Global_Css {
	...
	public static function install(): void {
		...
		$path = FLOWBIE_WP_PLUGIN_DIR . self::FIXTURE_FILE;
		if ( ! is_readable( $path ) ) {
			return;
		}

		$json = (string) file_get_contents( $path );
		Flowbie_Wp_Fields_Import_Export::import_json_string( $json );
	}
	...
	public static function get_stored_values(): array {
		...
		foreach ( self::get_field_map() as $name => $field ) {
			$values[ $name ] = Flowbie_Wp_Fields_Values::get_option( self::OPTION_SLUG, $field, false );
		}
		return $values;
	}
	...
	public static function handle_import_from_elementor(): void {
		...
		check_admin_referer( self::IMPORT_ACTION, 'flowbie_wp_import_global_css_elementor_nonce' );

		require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/super-migrate/class-flowbie-wp-migrate-elementor-global-css.php';
		$result = Flowbie_Wp_Migrate_Elementor_Global_Css::apply( false );

		$redirect = admin_url( 'admin.php?page=' . self::OPTION_SLUG );
		if ( ! empty( $result['ok'] ) ) {
			$redirect = add_query_arg(
				array(
					'flowbie_notice' => 'global_css_imported',
					'flowbie_msg'    => rawurlencode( (string) ( $result['message'] ?? '' ) ),
				),
				$redirect
			);
		} else {
			$redirect = add_query_arg(
				array(
					'flowbie_notice' => 'global_css_import_error',
					'flowbie_msg'    => rawurlencode( (string) ( $result['error'] ?? __( 'Import failed.', 'flowbie-wp' ) ) ),
				),
				$redirect
			);
		}
	...
	public static function render_import_notice(): void {
		...
		$notice = isset( $_GET['flowbie_notice'] ) ? sanitize_key( (string) wp_unslash( $_GET['flowbie_notice'] ) ) : '';
		if ( $notice === 'global_css_imported' ) {
			$msg = isset( $_GET['flowbie_msg'] ) ? sanitize_text_field( wp_unslash( (string) $_GET['flowbie_msg'] ) ) : __( 'Elementor global styles imported.', 'flowbie-wp' );
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html( $msg ) . '</p></div>';
		} elseif ( $notice === 'global_css_import_error' ) {
			$msg = isset( $_GET['flowbie_msg'] ) ? sanitize_text_field( wp_unslash( (string) $_GET['flowbie_msg'] ) ) : __( 'Import failed.', 'flowbie-wp' );
		...
		<div class="notice notice-info">
			<p><?php esc_html_e( 'Import Elementor Site Settings (global colors, typography, custom CSS) into this page.', 'flowbie-wp' ); ?></p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin:0 0 8px;">
				<input type="hidden" name="action" value="<?php echo esc_attr( self::IMPORT_ACTION ); ?>" />
				<?php wp_nonce_field( self::IMPORT_ACTION, 'flowbie_wp_import_global_css_elementor_nonce' ); ?>
				<?php submit_button( __( 'Import from Elementor', 'flowbie-wp' ), 'secondary', 'submit', false ); ?>
			</form>
		</div>
		<?php
	}
	...
	public static function enqueue_frontend_css(): void {
		...
		wp_register_style( 'flowbie-global-css', false, array(), FLOWBIE_WP_VERSION );
		wp_enqueue_style( 'flowbie-global-css' );
		wp_add_inline_style( 'flowbie-global-css', $css );
	}
	...
	public static function build_frontend_css(): string {
		...
		foreach ( $color_map as $token => $field ) {
			...
			if ( $color ) {
				$vars[] = '--flowbie-color-' . $token . ':' . $color;
			}
		}
