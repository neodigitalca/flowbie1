...
class Flowbie_Wp_Image_Seo {

	const OPTION_KEY = 'flowbie_wp_image_seo_config';
	...
	public static function save_meta( int $attachment_id, array $values ) {
		$check = Flowbie_Wp_Image_Seo_Gate::can_edit_attachment( $attachment_id );
	...
	public static function auto_optimize( int $attachment_id, int $post_id = 0, ?array $config = null ): bool {
		...
		if ( $use_ai && ! Flowbie_Wp_Image_Seo_Gate::can_ai( $post_id ) ) {
			$use_ai = false;
		}
		...
		if ( $use_ai ) {
			$proposed = Flowbie_Wp_Image_Seo_Ai::preview_ai( $attachment_id, $post_id, $fields, $config );
			if ( is_wp_error( $proposed ) ) {
				$proposed = Flowbie_Wp_Image_Seo_Ai::preview_filename( $attachment_id, $fields );
			}
		} else {
			$proposed = Flowbie_Wp_Image_Seo_Ai::preview_filename( $attachment_id, $fields );
		}
	...
	public static function render_attachment_seo_panel( int $attachment_id, string $context = 'gallery' ): void {
		...
		echo '<div class="flowbie-image-seo-panel" data-attachment-id="' . esc_attr( (string) $attachment_id ) . '" data-context="' . esc_attr( $context ) . '">';
		foreach ( $fields as $key => $label ) {
			$val = isset( $meta[ $key ] ) ? (string) $meta[ $key ] : '';
			echo '<div class="flowbie-image-seo-field flowbie-image-seo-field--' . esc_attr( $key ) . '">';
			echo '<label>' . esc_html( $label ) . '</label>';
			echo '<div class="flowbie-image-seo-field__row">';
			if ( $key === 'caption' || $key === 'description' ) {
				printf(
					'<textarea class="flowbie-image-seo-input" data-field="%1$s" rows="2">%2$s</textarea>',
					esc_attr( $key ),
					esc_textarea( $val )
				);
			} else {
				printf(
					'<input type="text" class="flowbie-image-seo-input" data-field="%1$s" value="%2$s" />',
					esc_attr( $key ),
					esc_attr( $val )
				);
			}
			printf(
				'<button type="button" class="button flowbie-image-seo-wand" data-field="%1$s" title="%2$s" aria-label="%2$s">✦</button>',
				esc_attr( $key ),
				esc_attr__( 'Optimize', 'flowbie-wp' )
			);
			echo '</div></div>';
		}
		echo '<button type="button" class="button flowbie-image-seo-optimize-all">' . esc_html__( 'Optimize all', 'flowbie-wp' ) . '</button>';
		echo '<button type="button" class="button flowbie-image-seo-save">' . esc_html__( 'Save metadata', 'flowbie-wp' ) . '</button>';
		echo '</div>';
	}
	...
	public static function render_gallery_item( int $attachment_id ): void {
		...
		echo '<li class="flowbie-gallery-item" data-id="' . esc_attr( (string) $attachment_id ) . '">';
		echo '<div class="flowbie-gallery-item__thumb">';
		echo wp_get_attachment_image( $attachment_id, 'thumbnail' );
		echo '<button type="button" class="flowbie-gallery-item__remove" aria-label="' . esc_attr__( 'Remove', 'flowbie-wp' ) . '">&times;</button>';
		echo '</div>';
