...
class Flowbie_Wp_Chat_Agents {
	...
	public static function run( string $user_message, array $history, string $site_name, array $site_index, array $training = array(), array $chat_settings = array(), ?array $page_context = null ): array {
		...
		$template = Flowbie_Wp_Chat_Suggestion_Templates::match_intent( $user_message, $page_context, $site_index );
		if ( is_array( $template ) ) {
			$card = Flowbie_Wp_Chat_Suggestion_Templates::build_card( $template, $page_context, $site_index, $chat_settings, $user_message );
		...
		if ( Flowbie_Wp_Chat_Lead::is_lead_message( $user_message, $site_index ) && Flowbie_Wp_Chat_Lead::is_enabled( $chat_settings ) ) {
			$phase_c = Flowbie_Wp_Chat_Lead::enrich_card( $phase_c, $user_message, $phase_b, $relevant_items, $site_index, $training, $chat_settings );
		}
		...
		$phase_c = Flowbie_Wp_Chat_Links::attach_to_card( $phase_c, $user_message, $phase_b, $relevant_items, $phase_a, $site_index, $seen_urls, $seen_topics );

		return Flowbie_Wp_Chat_Lead::maybe_attach_contact_human_cta( $phase_c, $user_message, $phase_a, $chat_settings );
	}
	...
	public static function phase_ack( string $message, string $site_name, array $training = array(), array $history = array(), array $settings = array() ) {
		...
		$subjects_block = Flowbie_Wp_Chat_Rag::build_gate_subjects_block( $settings );
		...
		$prior_chips = Flowbie_Wp_Chat_History::collect_seen_topics( $history );
...
PROMPT;
		...
		if ( null !== Flowbie_Wp_Chat_Links::resolve_lead_action( $message, Flowbie_Wp_Chat_Rag::get_site_index( $settings ) ) ) {
		...
		if ( Flowbie_Wp_Chat_Links::is_callback_request( $message ) ) {
	...
	public static function synthesize_classification_from_retrieve( string $message, array $retrieved ): ?array {
		...
		return array(
			'intent'       => ( Flowbie_Wp_Chat_Links::detect_lead_action( $message ) || Flowbie_Wp_Chat_Links::is_service_coverage_query( $message ) ) ? 'navigation' : 'question',
			'relevant_ids' => $ids,
	...
	public static function build_card_from_answer( string $answer, array $classification, array $items, string $user_message, array $site_index, array $history = array() ): array {
		...
		$card_type = ( Flowbie_Wp_Chat_Links::detect_lead_action( $user_message ) || Flowbie_Wp_Chat_Links::is_service_coverage_query( $user_message ) ) ? 'navigation' : self::intent_to_card_type( $intent );
		...
		return Flowbie_Wp_Chat_Links::attach_to_card( $card, $user_message, $card['body'] !== '' ? $card['body'] : $answer, $items, $classification, $site_index, $seen_urls, array() );
	}
	...
	public static function format_template_card(
		...
		if ( $source_body === '' || $page_title === '' ) {
			return new WP_Error( 'flowbie_template_empty', 'Missing page content for template card.' );
		}

		if ( strlen( $source_body ) > Flowbie_Wp_Chat_Page_Summary::SUMMARY_MD_MAX ) {
			$source_body = substr( $source_body, 0, Flowbie_Wp_Chat_Page_Summary::SUMMARY_MD_MAX ) . '…';
		}
...
PROMPT;
		...
		if ( null === $parsed || empty( $parsed['body'] ) ) {
			return new WP_Error( 'flowbie_template_format', 'Failed to format template card.' );
		}
...
PROMPT;
	...
	public static function phase_reason( string $message, array $history, string $site_name, array $items, array $classification, array $training = array(), array $site_index = array(), ?array $page_context = null ) {
		...
		$page_block     = Flowbie_Wp_Chat_Page_Context::format_for_prompt( $page_context );
		...
		$lead_action    = Flowbie_Wp_Chat_Links::resolve_lead_action( $message, $site_index );
		$unlisted_area   = Flowbie_Wp_Chat_Links::is_unlisted_service_area_query( $message, $site_index );
		$callback_request = Flowbie_Wp_Chat_Links::is_callback_request( $message );
...
RULE;
		...
		$blog_rule    = Flowbie_Wp_Chat_Rag::is_site_blog_discovery_query( $message )
...
PROMPT;
		...
		$unlinked = Flowbie_Wp_Chat_Links::unlinked_list_labels( $body );
		$unlinked_block = '';
		if ( ! empty( $unlinked ) ) {
		...
		$body = Flowbie_Wp_Chat_Links::apply_resolved_link_map( $body, $links );
		$body = Flowbie_Wp_Chat_Links::repair_malformed_markdown_links( $body );
...
PROMPT;
	...
	public static function select_relevant_items( array $classification, array $site_index, string $user_message = '', array $chat_settings = array(), ?array $page_context = null ): array {
		...
		if (
			...
			&& Flowbie_Wp_Chat_Page_Context::message_targets_current_page( $user_message )
		...
		if ( Flowbie_Wp_Chat_Links::resolve_lead_action( $user_message, $site_index ) ) {
		...
		if ( empty( $merged ) ) {
			...
			$ordered = Flowbie_Wp_Chat_Rag::ensure_topic_pages_in_items( $user_message, $ordered, $site_index, 3, $extra );
		}

		return Flowbie_Wp_Chat_Page_Context::merge_into_items( $ordered, $page_context, $user_message );
	}
	...
	private static function reason_length_rule( string $intent, string $message, array $history ): string {
		if ( $intent === 'navigation' || Flowbie_Wp_Chat_Links::detect_lead_action( $message ) || Flowbie_Wp_Chat_Links::is_callback_request( $message ) ) {
			return "- LENGTH: 1-2 sentences with one prominent inline link.\n";
		}
		if ( Flowbie_Wp_Chat_Links::is_service_coverage_query( $message ) ) {
...
PROMPT;
	...
	private static function title_echoes_user_message( string $title, string $user_message ): bool {
		...
		$title_terms = Flowbie_Wp_Chat_Rag::extract_terms( strtolower( self::plain_text_from_markdown( $title ) ) );
		$user_terms  = Flowbie_Wp_Chat_Rag::extract_terms( strtolower( $user_message ) );
	...
	private static function build_followup_context(
		...
		$exclude_urls = Flowbie_Wp_Chat_History::collect_seen_urls( $history );
		$url_seen     = array();
		...
		$add_url = static function ( string $url ) use ( &$url_seen, &$exclude_urls ): void {
			$norm = Flowbie_Wp_Chat_History::normalize_url( $url );
		...
		$intent = Flowbie_Wp_Chat_Suggestion_Templates::match_intent( $user_message, $page_context, $site_index );
		if ( is_array( $intent ) && ! empty( $intent['url'] ) ) {
			$add_url( (string) $intent['url'] );
		}

		$prior_topics = Flowbie_Wp_Chat_History::collect_seen_topics( $history );
		$history_block = Flowbie_Wp_Chat_History::format_for_prompt( array_slice( $history, -6 ) );
	...
	private static function deterministic_followup_topics(
		...
		$links  = Flowbie_Wp_Chat_Links::pick_topic_links( $query, $items, 4, $site_index, true, array(), $exclude_urls );
		$topics = array();
		$seen   = array();
	...
	private static function is_page_summary_request( string $message ): bool {
		...
		return Flowbie_Wp_Chat_Page_Context::message_targets_current_page( $lower );
	}
...
PROMPT;
	...
	private static function normalize_followup_chips(
		...
		foreach ( $chips as $chip ) {
			...
			$url_norm = Flowbie_Wp_Chat_History::normalize_url( (string) $item['url'] );
	...
	private static function build_followup_inventory_summary( array $items, array $site_index, array $exclude_urls = array() ): string {
		...
		foreach ( $by_id as $item ) {
			...
			$url_norm = Flowbie_Wp_Chat_History::normalize_url( $url );
	...
	private static function call_openrouter( string $model, string $system_prompt, string $user_prompt, int $max_tokens = 8192, float $temperature = 0.5 ) {
		...
		if ( self::is_truncated_finish_reason( $finish_reason ) ) {
			return new WP_Error(
				'flowbie_chat_truncated',
				__( 'AI response was cut off before completion. Please try again.', 'flowbie-wp' )
			);
		}
	...
	private static function followup_item_for_link( array $link, array $site_index ): ?array {
		...
		if ( $url !== '' ) {
			...
			foreach ( $site_index as $item ) {
				...
				if ( Flowbie_Wp_Chat_History::normalize_url( (string) $item['url'] ) === $url_norm ) {
					return $item;
				}
			}
		}
	...
	private static function followup_item_for_text( string $text, array $site_index ): ?array {
		...
		$fuzzy = Flowbie_Wp_Chat_Rag::find_fuzzy_topic_pages( $label, $site_index, 1 );
