...
class Flowbie_Wp_Chat_Rag {
	...
	const CACHE_KEY         = 'flowbie_chat_context_cache_v4';
	const BODY_CACHE_PREFIX = 'flowbie_chat_body_';
	...
	public static function read_post_body_for_chat( int $post_id ): string {
		...
		$parts = array();
		$body  = Flowbie_Wp_Rest::get_rendered_content_plain( $post_id );
		if ( $body === '' ) {
			...
			$body = Flowbie_Wp_Seo_Blocks_Context::extract_page_body_text( $post_id );
		}
		if ( $body !== '' ) {
			$parts[] = $body;
		}
		...
		$faq = Flowbie_Wp_Ai_Context::read_field_value( $post_id, 'faq' );
		if ( $faq !== '' ) {
			$parts[] = "FAQ:\n" . $faq;
		}
	...
	public static function merge_blog_discovery_items( string $message, array $retrieved, array $settings = array(), int $post_limit = 6 ): array {
		...
		return Flowbie_Wp_Chat_Links::dedupe_items_by_id( array_merge( $boosted, $retrieved ) );
	}

	/**
	 * Blog posts from the Flowbie post sitemap (category exclusions do not apply).
	 *
	 * @return array<int,array<string,mixed>>
	 */
	...
	public static function find_index_item_by_url( string $page_url, array $site_index = array() ): ?array {
		...
		$target = Flowbie_Wp_Chat_History::normalize_url( $page_url );
		if ( $target === '' ) {
			return null;
		}
		...
		foreach ( $site_index as $item ) {
			...
			$candidate = Flowbie_Wp_Chat_History::normalize_url( (string) $item['url'] );
			if ( $candidate === $target ) {
				return $item;
			}
		}
	...
	/**
	 * Build the full site index from Flowbie sitemap post types (every XML sitemap URL).
	 */
	private static function build_index( array $settings = array() ): array {
		if ( ! class_exists( 'Flowbie_Wp_Sitemap_Generator', false ) ) {
		...
		foreach ( Flowbie_Wp_Sitemap_Generator::collect_all_posts( $config ) as $row ) {
			$post = $row['post'];
	...
	private static function map_post_to_index_item( WP_Post $post, string $post_type, int $trim_length = 40 ): ?array {
		...
		return array(
			...
			'excerpt'       => Flowbie_Wp_Display_Text::decode( $excerpt ),
			'type'          => $post_type,
			'categories'    => $cats,
			'category_ids'  => $cat_ids,
			'tags'          => $tags,
			'focus_keyword' => Flowbie_Wp_Ai_Context::read_focus_keyword( $post->ID ),
		);
	}
	...
	public static function get_index_post_types( array $settings = array() ): array {
		...
		$config        = Flowbie_Wp_Sitemap_Settings::get_config();
		$sitemap_types = Flowbie_Wp_Sitemap_Generator::enabled_post_types( $config );
	...
	public static function invalidate_cache(): void {
		...
		Flowbie_Wp_Chat_Starters::invalidate_cache();
		Flowbie_Wp_Chat_Lead::invalidate_widget_contact_cache();
	}
