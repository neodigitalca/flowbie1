...
class Flowbie_Wp_Chat_Logs_Gap_Csv {
		...
		if ( 'page' === $content_type ) {
			return implode(
				...
				array(
					...
					'1. REPEATED_CUSTOMER_QUESTIONS (>= ' . self::MIN_CHIP_CLICKS . ' mentions or sessions) — strongest gap signal',
	...
	private static function normalize_ideas_for_csv( array $candidates ): array {
		...
		foreach ( $candidates as $idea ) {
			...
			$key = self::normalize_text( $keyword );
			if ( isset( $seen[ $key ] ) ) {
				continue;
			}
			$seen[ $key ] = true;
	...
	private static function predefined_prompt_texts(): array {
		...
		$texts    = Flowbie_Wp_Chat_Starters::get( $settings );
		$texts[]  = __( 'Summarize this page', 'flowbie-wp' );
		$out      = array();
	 ...
	 * }
	 */
	private static function extract_priority_signals( array $messages ): array {
		...
		$accepted_out = array_values( $accepted_bucket );
	...
	private static function build_user_prompt( array $signals, array $context, string $date_from, string $date_to, string $source_filter, string $content_type ): string {
		...
		$lines[] = '';
		$lines[] = '1. REPEATED_CUSTOMER_QUESTIONS (>= ' . self::MIN_CHIP_CLICKS . ' mentions or sessions):';
