...
class Flowbie_Wp_Ai_Meta {
	...
	public static function run_optimize_meta( int $post_id, ?string $primary_keyword = null ) {
		...
		if ( null === $primary_keyword || trim( $primary_keyword ) === '' ) {
			$primary_keyword = Flowbie_Wp_Ai_Context::read_focus_keyword( $post_id );
		}
		...
		$seo_brief = Flowbie_Wp_Ai_Context::read_acf_or_meta( $post_id, array( 'seo_research' ) );
		...
		$result = Flowbie_Wp_OpenRouter::complete( $system_prompt, $user_prompt, 4000, 0.7 );
		...
		if ( ! empty( $optimized['rank_math_title'] ) ) {
			$optimized['rank_math_title'] = Flowbie_Wp_Ai_Seo_Limits::normalize_title( (string) $optimized['rank_math_title'] );
		}

		$desc = isset( $optimized['rank_math_description'] ) ? (string) $optimized['rank_math_description'] : '';
		if ( $desc !== '' ) {
			$optimized['rank_math_description'] = Flowbie_Wp_Ai_Seo_Limits::normalize_description( $desc, $primary_keyword );
		}
	...
	public static function preview_meta_field( int $post_id, string $field ) {
		...
		if ( $value === '' ) {
			return new WP_Error(
				'flowbie_meta_empty',
				__( 'Meta optimizer returned no value for this field. Check focus keyword and post content.', 'flowbie-wp' )
			);
		}
...
Return ONLY a JSON object with the optimized meta fields.";
	...
	private static function parse_meta_json( string $ai_response ) {
		...
		if ( ! is_array( $decoded ) ) {
			return new WP_Error( 'flowbie_meta_json', __( 'Failed to parse meta optimizer JSON.', 'flowbie-wp' ) );
		}
		return $decoded;
	}
}
