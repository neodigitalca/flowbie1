...
abstract class Flowbie_Wp_Field_Type_Base implements Flowbie_Wp_Field_Type_Interface {

	/**
	 * @param array<string, mixed> $field Field config.
	 */
	protected function field_id( array $field ): string {
		return 'flowbie-field-' . sanitize_key( (string) ( $field['key'] ?? $field['name'] ?? 'field' ) );
	}
	...
	protected function field_name( array $field ): string {
		return 'flowbie_fields[' . esc_attr( (string) ( $field['name'] ?? '' ) ) . ']';
	}
