...
class Flowbie_Wp_Field_Type_Link extends Flowbie_Wp_Field_Type_Base {

	public function type(): string { return 'link'; }
	public function label(): string { return __( 'Link', 'flowbie-wp' ); }
	...
	public function render_input( array $field, $value, int $post_id ): void {
		...
		printf( '<p><label>%1$s <input type="url" name="%2$s[url]" value="%3$s" class="widefat" /></label></p>',
			esc_html__( 'URL', 'flowbie-wp' ), esc_attr( $base ), esc_attr( (string) ( $data['url'] ?? '' ) ) );
		printf( '<p><label>%1$s <input type="text" name="%2$s[title]" value="%3$s" class="widefat" /></label></p>',
			esc_html__( 'Link Text', 'flowbie-wp' ), esc_attr( $base ), esc_attr( (string) ( $data['title'] ?? '' ) ) );
		printf( '<p><label><input type="checkbox" name="%1$s[target]" value="_blank" %2$s /> %3$s</label></p>',
			esc_attr( $base ), checked( ( $data['target'] ?? '' ) === '_blank', true, false ), esc_html__( 'Open in new tab', 'flowbie-wp' ) );
...
abstract class Flowbie_Wp_Field_Type_Post_List_Base extends Flowbie_Wp_Field_Type_Base {
...
class Flowbie_Wp_Field_Type_Post_Object extends Flowbie_Wp_Field_Type_Post_List_Base {
	public function type(): string { return 'post_object'; }
	public function label(): string { return __( 'Post Object', 'flowbie-wp' ); }
...
class Flowbie_Wp_Field_Type_Page_Link extends Flowbie_Wp_Field_Type_Post_List_Base {
	public function type(): string { return 'page_link'; }
	public function label(): string { return __( 'Page Link', 'flowbie-wp' ); }
...
class Flowbie_Wp_Field_Type_Relationship extends Flowbie_Wp_Field_Type_Post_List_Base {
	public function type(): string { return 'relationship'; }
	public function label(): string { return __( 'Relationship', 'flowbie-wp' ); }
...
class Flowbie_Wp_Field_Type_Taxonomy extends Flowbie_Wp_Field_Type_Base {
	public function type(): string { return 'taxonomy'; }
	public function label(): string { return __( 'Taxonomy', 'flowbie-wp' ); }
	...
	public function render_input( array $field, $value, int $post_id ): void {
		...
		wp_dropdown_categories(
			array(
				...
				'show_option_none' => __( '- Select -', 'flowbie-wp' ),
				'hide_empty'       => 0,
			)
		);
		$this->wrap_close( $field );
...
class Flowbie_Wp_Field_Type_User extends Flowbie_Wp_Field_Type_Base {
	public function type(): string { return 'user'; }
	public function label(): string { return __( 'User', 'flowbie-wp' ); }
