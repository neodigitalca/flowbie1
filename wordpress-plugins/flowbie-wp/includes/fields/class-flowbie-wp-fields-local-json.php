...
class Flowbie_Wp_Fields_Local_Json {
	...
	public static function get_load_paths(): array {
		...
		$paths[] = trailingslashit( WP_CONTENT_DIR ) . 'flowbie-fields-json';
		$paths[] = trailingslashit( FLOWBIE_WP_PLUGIN_DIR ) . 'includes/fields/fixtures';
		return apply_filters( 'flowbie_wp_fields_json_load_paths', $paths );
	}

	public static function get_save_path(): string {
		$path = trailingslashit( get_stylesheet_directory() ) . 'acf-json';
		return apply_filters( 'flowbie_wp_fields_json_save_path', $path );
	}
