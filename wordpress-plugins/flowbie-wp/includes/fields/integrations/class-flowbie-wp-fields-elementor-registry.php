...
class Flowbie_Wp_Fields_Elementor_Registry {
	...
	private static function include_layout_fields_in_pickers(): bool {
		return class_exists( 'Flowbie_Wp_Fields_Elementor_Settings', false )
			&& Flowbie_Wp_Fields_Elementor_Settings::show_layout_fields();
	}
	...
	public static function get_options_page_choices(): array {
		...
		return apply_filters( 'flowbie_wp_fields_elementor_options_pages', $choices );
	}
	...
	public static function get_unified_field_choices( ?array $types = null ): array {
		...
		return apply_filters( 'flowbie_wp_fields_elementor_field_choices', $choices, 'unified', '', $types );
	}
	...
	public static function group_context_label( array $group ): string {
		$summary = Flowbie_Wp_Fields_Location::summarize( $group );
		if ( $summary !== '' ) {
			return $summary;
		}
		return (string) ( $group['title'] ?? __( 'Post', 'flowbie-wp' ) );
	}
	...
	public static function get_field_choices( string $context, string $options_slug = '', ?array $types = null ): array {
		if ( $context === 'post'
			&& class_exists( 'Flowbie_Wp_Fields_Elementor_Settings', false )
		...
		if ( $context === 'options' ) {
			...
			return apply_filters( 'flowbie_wp_fields_elementor_field_choices', $choices, $context, $options_slug, $types );
		}
	...
	private static function format_field_choice_label( string $name, string $detail ): string {
		return sprintf( 'Flowbie_%s — %s', $name, $detail );
	}
	...
	public static function get_groups_for_context( string $context, string $options_slug = '' ): array {
		$all = Flowbie_Wp_Fields_Storage::get_all_groups( true );
		if ( $context === 'options' ) {
			if ( $options_slug === '' ) {
				return array();
			}
