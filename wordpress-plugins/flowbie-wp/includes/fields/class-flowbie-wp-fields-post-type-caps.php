...
class Flowbie_Wp_Fields_Post_Type_Caps {
	...
	/**
	 * Post type slugs referenced by Flowbie field group location rules (post_type param).
	 *
	...
	public static function get_field_group_post_types(): array {
		...
		$types = array();
		foreach ( Flowbie_Wp_Fields_Storage::get_entities( Flowbie_Wp_Fields_Storage::CPT_GROUP ) as $group ) {
	...
	public static function maybe_warn_on_list_screen(): void {
		...
		$has_flowbie_config = Flowbie_Wp_Fields_Post_Types::get_config_for_slug( $typenow ) !== null;
		if ( empty( $targeted[ $typenow ] ) && ! $has_flowbie_config ) {
			return;
		}
		...
		add_action(
			'admin_notices',
			static function () use ( $typenow, $issues, $has_flowbie_config ): void {
				self::render_list_screen_notice( $typenow, $issues, $has_flowbie_config );
			}
		);
	}
	...
	public static function get_capability_issues( string $post_type ): array {
		...
		if ( ! $pto instanceof WP_Post_Type ) {
			return array(
				__( 'This post type is not registered.', 'flowbie-wp' ),
			);
		}
		...
		if ( $edit_others_cap === '' || ! current_user_can( $edit_others_cap ) ) {
			$issues[] = sprintf(
				...
				__( 'You cannot %s for this post type (bulk checkboxes and some list actions are hidden).', 'flowbie-wp' ),
		...
		if ( $sample_id > 0 && ! current_user_can( 'edit_post', $sample_id ) ) {
			$issues[] = __( 'You cannot edit individual posts in this list (titles stay plain text with no row checkboxes).', 'flowbie-wp' );
		}
		return $issues;
	}
	 ...
	 */
	private static function render_list_screen_notice( string $post_type, array $issues, bool $has_flowbie_config ): void {
		...
		echo esc_html(
			sprintf(
				...
				__( '%s: capability misconfiguration', 'flowbie-wp' ),
				$label
			)
		);
		...
		echo '</ul><p>';
		esc_html_e( 'Enable Map Meta Cap and set Capability Type to "post" in ACF Post Types or Flowbie Fields, then save and reload this page.', 'flowbie-wp' );
		if ( $has_flowbie_config ) {
			$url = admin_url( 'admin.php?page=flowbie-wp-post-types-edit&post_type=' . rawurlencode( $post_type ) );
			echo ' <a href="' . esc_url( $url ) . '">' . esc_html__( 'Edit in Flowbie Fields', 'flowbie-wp' ) . '</a>';
		}
		echo '</p>';
		if ( $pto instanceof WP_Post_Type ) {
			...
			echo esc_html(
				sprintf(
					...
					__( 'Current registration: capability_type=%1$s, map_meta_cap=%2$s.', 'flowbie-wp' ),
					(string) $pto->capability_type,
					$pto->map_meta_cap ? __( 'yes', 'flowbie-wp' ) : __( 'no', 'flowbie-wp' )
				)
			);
			echo '</p>';
		}
