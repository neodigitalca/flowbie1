...
class Flowbie_Wp_Fields_Storage {
	...
	const CPT_GROUP     = 'flowbie-field-group';
	const CPT_FIELD     = 'flowbie-field';
	const CPT_POST_TYPE = 'flowbie-post-type';
	const CPT_TAXONOMY  = 'flowbie-taxonomy';
	...
	public static function register_post_types(): void {
		register_post_type(
			...
			array(
				'labels'              => array(
					'name'          => __( 'Field Groups', 'flowbie-wp' ),
					'singular_name' => __( 'Field Group', 'flowbie-wp' ),
				),
				'public'              => false,
				'show_ui'             => false,
				'show_in_menu'        => false,
	...
	public static function save_group( array $group ): int {
		...
		update_post_meta( (int) $post_id, '_flowbie_field_group_key', $key );
		self::sync_child_fields( (int) $post_id, $group );
		Flowbie_Wp_Fields_Local_Json::sync_group( $group );
		self::flush_cache();
		return (int) $post_id;
	}
	...
	public static function import_groups( array $groups, bool $delete_missing = false ): array {
		...
		if ( $delete_missing ) {
			...
			foreach ( $query->posts as $pid ) {
				$meta_key = get_post_meta( (int) $pid, '_flowbie_field_group_key', true );
	...
	public static function delete_group( string $key ): bool {
		...
		Flowbie_Wp_Fields_Local_Json::delete_group_file( $key );
		self::flush_cache();
		return (bool) $result;
	}
	...
	private static function find_group_post_id( string $key ): int {
		$query = new WP_Query(
			array(
				...
				'posts_per_page' => 1,
				'fields'         => 'ids',
				'meta_key'       => '_flowbie_field_group_key',
				'meta_value'     => $key,
				'no_found_rows'  => true,
			)
		);
	...
	private static function decode_group_post( WP_Post $post ): ?array {
		...
		if ( empty( $decoded['key'] ) ) {
			$meta_key = get_post_meta( $post->ID, '_flowbie_field_group_key', true );
	...
	public static function save_entity( string $cpt, array $data, string $meta_key_field ): int {
		...
		update_post_meta( (int) $id, '_flowbie_entity_key', $key );
		return (int) $id;
	}
