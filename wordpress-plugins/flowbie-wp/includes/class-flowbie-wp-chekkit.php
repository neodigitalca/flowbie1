...
class Flowbie_Wp_Chekkit {

	const REQUEST_TIMEOUT = 15;

	const DEFAULT_WEBHOOK_URL = 'https://flowbie.ca/webhook';

	/**
	 * Chekkit Events webhook URL from env (server-side only).
	 */
	public static function get_webhook_url(): string {
		if ( defined( 'FLOWBIE_WP_CHEKKIT_WEBHOOK_URL' ) && FLOWBIE_WP_CHEKKIT_WEBHOOK_URL !== '' ) {
			return trim( (string) FLOWBIE_WP_CHEKKIT_WEBHOOK_URL );
		}

		$stored = function_exists( 'get_option' ) ? get_option( 'flowbie_wp_chat_settings', array() ) : array();
	...
	public static function send_contact( array $data ) {
		...
		if ( $url === '' ) {
			return new WP_Error(
				...
				__( 'Chekkit webhook is not configured.', 'flowbie-wp' )
			);
		}
	...
	public static function validate_contact_input( array $input ): ?array {
		...
		if ( $name === '' ) {
			$errors['name'] = __( 'Name is required.', 'flowbie-wp' );
		} elseif ( strlen( $name ) > 120 ) {
			$errors['name'] = __( 'Name is too long.', 'flowbie-wp' );
		}

		$phone = isset( $input['phone'] ) ? trim( (string) $input['phone'] ) : '';
		if ( $phone === '' ) {
			$errors['phone'] = __( 'Phone is required.', 'flowbie-wp' );
		} elseif ( strlen( $phone ) > 40 ) {
			$errors['phone'] = __( 'Phone is too long.', 'flowbie-wp' );
		}

		$email = isset( $input['email'] ) ? trim( (string) $input['email'] ) : '';
		if ( $email !== '' && ! is_email( $email ) ) {
			$errors['email'] = __( 'Enter a valid email address.', 'flowbie-wp' );
		}

		$message = isset( $input['message'] ) ? trim( (string) $input['message'] ) : '';
		if ( strlen( $message ) > 2000 ) {
			$errors['message'] = __( 'Message is too long.', 'flowbie-wp' );
		}
