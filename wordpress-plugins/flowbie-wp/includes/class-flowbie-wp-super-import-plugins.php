...
class Flowbie_Wp_Super_Import_Plugins {

	const OPTION_DEACTIVATED = 'flowbie_wp_super_import_deactivated_plugins';
	...
	public static function active_for_adapters( array $adapter_ids ): array {
		...
		$out    = array();
		$seen   = array();
		$flowbie = plugin_basename( FLOWBIE_WP_PLUGIN_FILE );
		...
		foreach ( $adapter_ids as $adapter_id ) {
			...
			foreach ( $map[ $adapter_id ] as $file => $label ) {
				if ( isset( $seen[ $file ] ) || $file === $flowbie ) {
					continue;
				}
	...
	private static function flo_sheet_has_import(): bool {
		if ( ! class_exists( 'Flowbie_Wp_Flo_Sheet', false ) ) {
			return false;
		}
	...
	public static function deactivate_for_adapters( array $adapter_ids, array $plugin_files ): array {
		...
		if ( empty( $allowed ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'No conflicting plugins are active for this import.', 'flowbie-wp' ),
			);
		}
		...
		foreach ( $plugin_files as $file ) {
			...
			if ( $file === '' || $file === $flowbie || ! isset( $allowed_files[ $file ] ) ) {
				continue;
			}
			if ( ! is_plugin_active( $file ) ) {
				continue;
		...
		if ( empty( $to_run ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'Select at least one plugin to deactivate.', 'flowbie-wp' ),
			);
		}
			...
			if ( is_plugin_active( $file ) ) {
				return array(
					...
					'error' => sprintf(
						...
						__( 'Could not deactivate `%s`. Try deactivating it manually under Plugins.', 'flowbie-wp' ),
						$file
					),
				);
			}
	...
	/**
	 * Reactivate plugins deactivated by Super Import and deactivate Flowbie WP.
	 *
	 * @return array{ok: bool, activated?: array<int, string>, failures?: array<int, array{file: string, label: string, error: string}>, flowbie_deactivated?: bool, error?: string}
	 */
	public static function restore_deactivated_plugins(): array {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'You do not have permission to activate plugins.', 'flowbie-wp' ),
			);
		}
		...
		if ( empty( $snapshot ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'No Super Import plugin snapshot to restore.', 'flowbie-wp' ),
			);
		}
		...
		foreach ( $to_activate as $file ) {
			...
			if ( ! self::is_active_plugin_file( $file ) ) {
				$failures[] = array(
					'file'  => $file,
					'label' => $label,
					'error' => __( 'Plugin did not stay active after activation.', 'flowbie-wp' ),
				);
				continue;
			}
		...
		$flowbie_deactivated = false;
		if ( self::is_active_plugin_file( $flowbie ) ) {
			ob_start();
			deactivate_plugins( $flowbie, true );
			ob_end_clean();
			self::refresh_plugin_activation_cache();
			$flowbie_deactivated = ! self::is_active_plugin_file( $flowbie );
		}
		...
		$response = array(
			...
			'flowbie_deactivated' => $flowbie_deactivated,
		);
