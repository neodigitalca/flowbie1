...
class Flowbie_Wp_Ai_Body {

	/**
	 * OpenRouter for body harness: wp-config, env, or agency settings only (no Supabase).
	 *
	 * @return string
	 */
	public static function body_openrouter_configured(): bool {
		return Flowbie_Wp_OpenRouter::get_body_api_key() !== '';
	}
	...
	public static function validate_prerequisites( int $post_id ) {
		...
		if ( ! self::body_openrouter_configured() ) {
			$reasons[] = __( 'Body optimizer requires an OpenRouter API key in wp-config, environment, or Flowbie WP → Settings. Cloud-only sync is not used for body harness.', 'flowbie-wp' );
		}
		$brief = Flowbie_Wp_Ai_Context::read_acf_or_meta( $post_id, array( 'seo_research' ) );
		if ( trim( $brief ) === '' || self::is_empty_json_brief( $brief ) ) {
			$reasons[] = __( 'Run SEO research first or paste a research brief before using the body optimizer.', 'flowbie-wp' );
		}
		$focus = Flowbie_Wp_Ai_Context::read_focus_keyword( $post_id );
		if ( trim( $focus ) === '' ) {
			$reasons[] = __( 'Set a focus keyword before using the body optimizer.', 'flowbie-wp' );
		}
		if ( ! empty( $reasons ) ) {
			return new WP_Error( 'flowbie_body_prereq', implode( ' ', $reasons ) );
		}
		return true;
	}
	...
	public static function plan( int $post_id ) {
		...
		$built = Flowbie_Wp_Harness_Blueprint::build( $ctx );
		if ( is_wp_error( $built ) ) {
			return $built;
		}

		$blueprint = $built['blueprint'];
		$outline   = Flowbie_Wp_Harness_Outline::from_agents(
		...
		Flowbie_Wp_Harness_Session::save( $post_id, $session );

		$shape = Flowbie_Wp_Harness_Session::public_shape( $session );
		$shape['ok'] = true;
		return $shape;
	}
	...
	public static function get_session( int $post_id ) {
		$session = Flowbie_Wp_Harness_Session::get( $post_id );
		if ( ! $session ) {
			return array(
				'ok'       => true,
				'hasSession' => false,
	...
	public static function preview_section( int $post_id, int $section_index, string $session_id = '' ) {
		...
		$session = Flowbie_Wp_Harness_Session::get( $post_id );
		if ( ! $session ) {
			return new WP_Error( 'flowbie_body_session', __( 'No body harness session. Run Plan first.', 'flowbie-wp' ) );
		}
		if ( $session_id !== '' && isset( $session['sessionId'] ) && (string) $session['sessionId'] !== $session_id ) {
			return new WP_Error( 'flowbie_body_session', __( 'Session mismatch. Run Plan again.', 'flowbie-wp' ) );
		}

		$session['phase']       = 'generating';
		$session['activeIndex'] = $section_index;
		Flowbie_Wp_Harness_Session::patch_section( $session, $section_index, array( 'status' => 'generating' ) );
		Flowbie_Wp_Harness_Session::save( $post_id, $session );

		$ctx = isset( $session['ctx'] ) && is_array( $session['ctx'] ) ? $session['ctx'] : self::build_context( $post_id, get_post( $post_id ), Flowbie_Wp_Ai_Backend::resolve_urls( $post_id ) );

		$result = Flowbie_Wp_Harness_Runner::preview_section( $session, $section_index, $ctx );
		if ( is_wp_error( $result ) ) {
			Flowbie_Wp_Harness_Session::patch_section( $session, $section_index, array( 'status' => 'error', 'error' => $result->get_error_message() ) );
			$session['phase'] = 'ready';
			Flowbie_Wp_Harness_Session::save( $post_id, $session );
			return $result;
		}
		...
		$current_html = '';
		if ( $post instanceof WP_Post ) {
			$current_html = Flowbie_Wp_Content_Sections::extract_section_html( (string) $post->post_content, $result['title'] );
		}

		Flowbie_Wp_Harness_Session::patch_section(
			$session,
			$section_index,
			array(
				'status'    => 'done',
		...
		return array(
			...
			'truncated'    => ! empty( $result['truncated'] ),
			'session'      => Flowbie_Wp_Harness_Session::public_shape( $session ),
		);
	}
	...
	public static function apply_section( int $post_id, int $section_index, string $session_id = '', string $html_override = '', bool $editor_applied = false, string $section_title_override = '' ) {
		if ( ! Flowbie_Wp_Ai_Gate::can_apply( $post_id ) ) {
			return new WP_Error( 'flowbie_body_apply', __( 'Apply is not allowed for this post.', 'flowbie-wp' ) );
		}

		$session = Flowbie_Wp_Harness_Session::get( $post_id );
		if ( ! $session ) {
			return new WP_Error( 'flowbie_body_session', __( 'No body harness session.', 'flowbie-wp' ) );
		}
		if ( $session_id !== '' && isset( $session['sessionId'] ) && (string) $session['sessionId'] !== $session_id ) {
			return new WP_Error( 'flowbie_body_session', __( 'Session mismatch.', 'flowbie-wp' ) );
		}
		...
		if ( $html === '' ) {
			return new WP_Error( 'flowbie_body_apply', __( 'Preview this section before applying.', 'flowbie-wp' ) );
		}
		...
		$new_content = Flowbie_Wp_Content_Sections::replace_section_html( (string) $post->post_content, $title, $html );
		...
		Flowbie_Wp_Ai_Apply::stamp_date_modifier( $post_id );
	...
	public static function clear_session( int $post_id ): array {
		Flowbie_Wp_Harness_Session::delete( $post_id );
	...
	public static function get_posts_inventory( int $exclude_post_id = 0 ): array {
		$posts = Flowbie_Wp_Harness_Blueprint::fetch_linkable_posts();
		$out   = array();
		foreach ( $posts as $p ) {
	...
	public static function suggest_link( int $post_id, string $selected_text, string $context = '' ) {
		if ( ! self::body_openrouter_configured() ) {
			return new WP_Error( 'flowbie_body_key', __( 'OpenRouter API key required.', 'flowbie-wp' ) );
		}

		$selected_text = trim( $selected_text );
		if ( $selected_text === '' ) {
			return new WP_Error( 'flowbie_body_link', __( 'No text selected.', 'flowbie-wp' ) );
		}
		...
		$result = Flowbie_Wp_OpenRouter::complete_chat( $system, $user, 30, 0.0 );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
	...
	public static function generate_element( int $post_id, string $section_title, string $section_html, string $element_type, string $custom_prompt = '' ) {
		...
		if ( ! isset( $type_labels[ $element_type ] ) ) {
			return new WP_Error( 'flowbie_element_type', __( 'Invalid element type.', 'flowbie-wp' ) );
		}
		...
		$result = Flowbie_Wp_OpenRouter::complete_chat( $system, $user, 1024, 0.7 );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
	...
	private static function build_context( int $post_id, $post, array $urls ): array {
		...
		$gsc_json  = '';
		$gsc = Flowbie_Wp_Ai_Gsc::get_suggestions( $post_id, $ctx['focusKeyword'] );
		...
		return array(
			...
			'posts'              => $posts,
			'postsBlock'         => Flowbie_Wp_Harness_Prompts::wordpress_posts_block( $posts, $site_name ),
