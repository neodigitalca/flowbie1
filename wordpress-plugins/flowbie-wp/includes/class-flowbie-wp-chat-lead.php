<?php
/**
 * Lead conversion specialist for Flowbie Chat.
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Chat_Lead {
	...
	public static function is_lead_message( string $message, array $site_index = array() ): bool {
		return null !== Flowbie_Wp_Chat_Links::resolve_lead_action( $message, $site_index );
	}
	...
	public static function is_chekkit_available( array $settings ): bool {
		...
		return class_exists( 'Flowbie_Wp_Chekkit' ) && Flowbie_Wp_Chekkit::is_configured();
	}
	...
	public static function should_suggest_contact_human( string $message, array $classification ): bool {
		if ( null !== Flowbie_Wp_Chat_Links::detect_lead_action( $message ) ) {
			return true;
		}
	...
	public static function enrich_card( array $card, string $message, string $answer, array $items, array $site_index, array $training, array $settings ): array {
		...
		$lead_pages    = Flowbie_Wp_Chat_Links::find_lead_pages( $message, $site_index, 3 );
		$enriched_lead = Flowbie_Wp_Chat::enrich_relevant_items( $lead_pages, false, $settings );
...
PROMPT;
	...
	public static function get_widget_contact_facts( array $settings ): array {
		...
		$lead_pages = Flowbie_Wp_Chat_Links::find_lead_pages( 'contact us', $site_index, 3 );
		$enriched   = Flowbie_Wp_Chat::enrich_relevant_items( $lead_pages, false, $settings );
	...
	public static function phase_lead_convert( string $message, string $lead_action, string $answer, array $lead_pages, array $contact_facts, int $form_id ) {
		...
		$form_title  = '';
		if ( $form_id > 0 && class_exists( 'Flowbie_Wp_Forms_Storage' ) ) {
...
PROMPT;
		...
		$parsed = self::parse_json_response( (string) $result );
		if ( null === $parsed ) {
			return new WP_Error( 'flowbie_chat_lead_parse', __( 'Lead conversion specialist returned invalid JSON.', 'flowbie-wp' ) );
		}
	...
	public static function apply_conversion_to_card( array $card, array $specialist, array $contact_facts, int $form_id, array $lead_pages ): array {
		...
		if ( ! empty( $specialist['relatedTopics'] ) && is_array( $specialist['relatedTopics'] ) ) {
			...
			foreach ( $specialist['relatedTopics'] as $topic ) {
				...
				if ( count( $topics ) >= Flowbie_Wp_Chat_Agents::MAX_RELATED_TOPICS ) {
					break;
				}
			}
			if ( count( $topics ) >= Flowbie_Wp_Chat_Agents::MIN_RELATED_TOPICS ) {
	...
	private static function render_form_bundle( int $form_id ): array {
		if ( $form_id < 1 || ! class_exists( 'Flowbie_Wp_Forms' ) ) {
			return array(
				'formId'     => 0,
				'formHtml'   => '',
				'formConfig' => null,
		...
		$html = Flowbie_Wp_Forms::render_instance(
			array(
				'form_id'       => $form_id,
				'wrapper_class' => 'flowbie-form--chat',
				'full_width'    => 'yes',
			)
		);
	...
	private static function normalize_specialist_output( array $parsed, array $contact_facts ) {
		...
		if ( $headline === '' || $cta_label === '' ) {
			return new WP_Error( 'flowbie_chat_lead_incomplete', __( 'Lead conversion specialist omitted required fields.', 'flowbie-wp' ) );
		}
	...
	private static function form_id_from_page( array $page ): int {
		...
		foreach ( array( '_flowbie_form_id', 'flowbie_form_id' ) as $key ) {
	...
	private static function form_exists( int $form_id ): bool {
		if ( $form_id < 1 || ! class_exists( 'Flowbie_Wp_Forms_Storage' ) ) {
			return false;
		}
		return is_array( Flowbie_Wp_Forms_Storage::get_form_by_id( $form_id ) );
	}
	...
	private static function call_openrouter( string $model, string $system_prompt, string $user_prompt, int $max_tokens, float $temperature ) {
		...
		if ( $code < 200 || $code >= 300 ) {
			return new WP_Error( 'flowbie_chat_lead_ai', __( 'Lead conversion AI request failed.', 'flowbie-wp' ) );
		}

		if ( ! is_array( $data ) || empty( $data['choices'][0]['message']['content'] ) ) {
			return new WP_Error( 'flowbie_chat_lead_empty', __( 'Lead conversion AI returned empty content.', 'flowbie-wp' ) );
		}
