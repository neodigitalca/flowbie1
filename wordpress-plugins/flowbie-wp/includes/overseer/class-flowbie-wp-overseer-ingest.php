...
class Flowbie_Wp_Overseer_Ingest {
	...
	public static function record_event( array $data ) {
		...
		if ( ! in_array( $event_type, Flowbie_Wp_Overseer_Context::ALLOWED_EVENT_TYPES, true ) ) {
			$event_type = 'pageview';
		}
		...
		if ( ! Flowbie_Wp_Overseer_Context::is_valid_session_id( $session_id ) ) {
			$session_id = Flowbie_Wp_Overseer_Context::new_session_id();
		}
		...
		if ( $parent_visit_uid !== '' && ! Flowbie_Wp_Overseer_Context::is_valid_uuid( $parent_visit_uid ) ) {
			$parent_visit_uid = '';
		}
		...
		$visit_uid = Flowbie_Wp_Overseer_Context::new_uuid();
