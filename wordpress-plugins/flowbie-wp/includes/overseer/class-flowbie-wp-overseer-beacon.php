...
class Flowbie_Wp_Overseer_Beacon {

	public static function get_builtin_script_id(): int {
		return (int) get_option( Flowbie_Wp_Overseer_Context::BUILTIN_SCRIPT_ID_OPTION, 0 );
	}
	...
	public static function beacon_js_url(): string {
		return plugins_url( 'assets/frontend/flowbie-overseer.js', FLOWBIE_WP_PLUGIN_FILE );
	}
	public static function builtin_script_template(): string {
		return "<!-- Flowbie Page View Tag (Overseer) — first-party pageview beacon. Do not remove. -->\n"
			. '<script type="application/json" id="' . Flowbie_Wp_Overseer_Context::CONFIG_SCRIPT_ID . '">' . Flowbie_Wp_Overseer_Context::CONFIG_PLACEHOLDER . "</script>\n"
			. '<script src="' . Flowbie_Wp_Overseer_Context::JS_URL_PLACEHOLDER . '"></script>' . "\n";
	}
	public static function resolve_script_placeholders( string $code ): string {
		if ( strpos( $code, Flowbie_Wp_Overseer_Context::CONFIG_PLACEHOLDER ) !== false ) {
			$code = str_replace( Flowbie_Wp_Overseer_Context::CONFIG_PLACEHOLDER, self::beacon_config_json(), $code );
		}
		if ( strpos( $code, Flowbie_Wp_Overseer_Context::JS_URL_PLACEHOLDER ) !== false ) {
			$code = str_replace( Flowbie_Wp_Overseer_Context::JS_URL_PLACEHOLDER, esc_url( self::beacon_js_url() ), $code );
		}
		return $code;
	}
	public static function beacon_config(): array {
		$settings = Flowbie_Wp_Overseer_Settings::get_settings();
		return array(
			'endpoint'            => rest_url( 'flowbie/v1/overseer/collect' ),
	...
	public static function is_stub_script_code( string $code ): bool {
		return strpos( $code, 'Flowbie Page View Tag (Overseer)' ) !== false
	...
	public static function needs_builtin_script_resync( string $code ): bool {
		...
		return strpos( $code, Flowbie_Wp_Overseer_Context::CONFIG_PLACEHOLDER ) === false
			|| strpos( $code, Flowbie_Wp_Overseer_Context::JS_URL_PLACEHOLDER ) === false;
	}
	public static function has_script_manager_beacon_template(): bool {
		...
		$row = Flowbie_Wp_Script_Manager::get( $builtin_id );
		if ( ! $row ) {
			return false;
		}
	...
	public static function maybe_enqueue_beacon_fallback(): void {
		if ( ! Flowbie_Wp_Overseer_Settings::should_load_beacon_on_frontend() ) {
			return;
		}
	...
	public static function maybe_upgrade_script_template(): void {
		if ( get_option( Flowbie_Wp_Overseer_Context::SCRIPT_TEMPLATE_OPTION, '1' ) === Flowbie_Wp_Overseer_Context::SCRIPT_TEMPLATE_VERSION ) {
			return;
		}
		update_option( Flowbie_Wp_Overseer_Context::SCRIPT_TEMPLATE_OPTION, Flowbie_Wp_Overseer_Context::SCRIPT_TEMPLATE_VERSION, false );
	...
	public static function ensure_builtin_script(): void {
		...
		$by_name = Flowbie_Wp_Script_Manager::get_by_name( Flowbie_Wp_Overseer_Context::BUILTIN_SCRIPT_NAME );
		if ( $by_name ) {
			$canonical_id = (int) $by_name->id;
			if ( self::get_builtin_script_id() !== $canonical_id ) {
				update_option( Flowbie_Wp_Overseer_Context::BUILTIN_SCRIPT_ID_OPTION, $canonical_id, false );
			}
		...
		$result = Flowbie_Wp_Script_Manager::save(
			array(
				'name'          => Flowbie_Wp_Overseer_Context::BUILTIN_SCRIPT_NAME,
				'placement'     => 'footer',
		...
		if ( ! empty( $result['ok'] ) && ! empty( $result['id'] ) ) {
			update_option( Flowbie_Wp_Overseer_Context::BUILTIN_SCRIPT_ID_OPTION, (int) $result['id'], false );
			self::sync_builtin_script();
		}
	}
	...
	public static function sync_builtin_script(): void {
		...
		Flowbie_Wp_Script_Manager::save(
			array(
				'id'            => $builtin_id,
				'name'          => Flowbie_Wp_Overseer_Context::BUILTIN_SCRIPT_NAME,
				'placement'     => 'footer',
	...
	public static function is_overseer_tag_row( $row ): bool {
		...
		$name = isset( $row->name ) ? Flowbie_Wp_Script_Manager::normalize_name( (string) $row->name ) : '';
		return $name === Flowbie_Wp_Overseer_Context::BUILTIN_SCRIPT_NAME;
	}
	public static function filter_script_code( string $code, $row ): string {
		if ( is_object( $row ) && self::is_overseer_tag_row( $row ) ) {
			if ( ! Flowbie_Wp_Overseer_Settings::is_tracking_active() ) {
				return '';
			}
