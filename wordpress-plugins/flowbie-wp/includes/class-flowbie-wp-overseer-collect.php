...
class Flowbie_Wp_Overseer_Collect {
	...
	public static function handle_collect( WP_REST_Request $request ) {
		if ( ! Flowbie_Wp_Overseer::is_tracking_active() ) {
		...
		if ( ! is_string( $nonce ) || ! wp_verify_nonce( $nonce, 'wp_rest' ) ) {
			return new WP_Error(
				'flowbie_overseer_invalid_nonce',
				__( 'Invalid security token.', 'flowbie-wp' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( self::is_rate_limited() ) {
			return new WP_Error(
				'flowbie_overseer_rate_limited',
				__( 'Too many requests. Please wait a moment.', 'flowbie-wp' ),
				array( 'status' => 429 )
			);
		}
		...
		foreach ( $events as $event ) {
			...
			$result = Flowbie_Wp_Overseer::record_event(
				array(
	...
	private static function is_rate_limited(): bool {
		$ip  = Flowbie_Wp_Overseer::get_client_ip();
		$key = 'flowbie_ov_rl_' . md5( $ip !== '' ? $ip : 'unknown' );
