 ...
 */
class Flowbie_Wp_Speed_Image_Rest {
	...
	public static function rest_batch( WP_REST_Request $request ) {
		if ( ! Flowbie_Wp_Speed_Image_Settings::is_enabled() ) {
			return new WP_Error(
				'flowbie_speed_image_disabled',
				__( 'Enable image optimization under Speed → Images first.', 'flowbie-wp' ),
				array( 'status' => 400 )
			);
		}

		$limit = (int) apply_filters( 'flowbie_wp_speed_image_batch_time_limit', 30 );
		...
		$result = Flowbie_Wp_Speed_Image_Optimizer::batch_optimize( $per_page, $page, $force );
		...
		return new WP_REST_Response(
			array(
				...
				'batch'  => $result,
				'stats'  => Flowbie_Wp_Speed_Image_Stats::get(),
				'pending' => Flowbie_Wp_Speed_Image_Optimizer::count_pending( $force ),
			),
			200
		);
	}
