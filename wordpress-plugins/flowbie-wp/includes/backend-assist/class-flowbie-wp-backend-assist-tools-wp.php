...
class Flowbie_Wp_Backend_Assist_Tools_Wp {
	...
	public static function tool_create_page( array $params ): array {
		...
		if ( $focus_keyword !== '' ) {
			Flowbie_Wp_Ai_Apply::write_focus_keyword( $post_id, $focus_keyword );
		}
	...
	public static function tool_get_post( array $params ): array {
		...
		$meta_hub = Flowbie_Wp_Ai_Context::meta_hub_values( $post->ID );
	...
	public static function tool_save_post_meta( array $params ): array {
		...
		if ( $post_id < 1 && $title !== '' ) {
			Flowbie_Wp_Site_Inventory::warm( true );
			$item = Flowbie_Wp_Site_Inventory::find_item_by_title( $title );
		...
		if ( $post_id < 1 ) {
			return array(
				'success' => false,
				'error'   => __( 'Provide post_id or a resolvable title.', 'flowbie-wp' ),
			);
		}
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return array(
				'success' => false,
				'error'   => __( 'You do not have permission to edit this post.', 'flowbie-wp' ),
			);
		}
		...
		if ( $fields === array() ) {
			return array(
				'success' => false,
				'error'   => __( 'No meta fields to save.', 'flowbie-wp' ),
			);
		}

		$result = Flowbie_Wp_Ai_Apply::save_meta(
			$post_id,
			$fields,
			array(
	...
	public static function tool_run_seo_research_brief( array $params ): array {
		...
		if ( $post_id < 1 ) {
			return array(
				'success' => false,
				'error'   => __( 'Provide post_id for SEO research.', 'flowbie-wp' ),
			);
		}
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return array(
		...
		$result = Flowbie_Wp_Ai_Seo_Research::build_brief( $post_id, $focus, true );
		if ( is_wp_error( $result ) ) {
			return array(
				'success' => false,
				'error'   => $result->get_error_message(),
	...
	private static function build_meta_hub_fields_from_params( array $params ): array {
		...
		foreach ( $text_map as $hub_key => $keys ) {
			foreach ( $keys as $param_key ) {
				...
				if ( $post_id > 0 && Flowbie_Wp_Ai_Seo_Limits::meta_copy_drifts_from_post( $value, $post_id ) ) {
					continue;
				}
	...
	public static function tool_add_content( array $params ): array {
		...
		$content = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::strip_json_ld_from_html( $content );
		if ( $content === '' ) {
			return array( 'success' => false, 'error' => 'Content to add is required.' );
		}
		...
		$prev_links    = $is_ops_save ? Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::count_same_site_links( (string) $post->post_content ) : 0;
		...
		if ( $prev_words > 200 ) {
			...
			if ( ! $has_remove && $new_words_preview < (int) floor( $prev_words * 0.85 ) ) {
				return array(
					'success' => false,
					'error'   => __( 'Content loss blocked: this edit would drop more than 15% of the post body. Use restore previous revision to undo a bad edit, then retry with a narrower request.', 'flowbie-wp' ),
				);
			}
		}
		...
		if ( $body_ops ) {
			$new_links = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::count_same_site_links( $new_content );
	...
	public static function tool_update_post( array $params ): array {
		$post_id = isset( $params['post_id'] ) ? absint( $params['post_id'] ) : 0;
		if ( $post_id < 1 ) {
			return array( 'success' => false, 'error' => __( 'post_id is required.', 'flowbie-wp' ) );
		}
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return array( 'success' => false, 'error' => __( 'You do not have permission to edit this post.', 'flowbie-wp' ) );
		}
		...
		if ( isset( $params['title'] ) && trim( (string) $params['title'] ) !== '' ) {
			...
			if (
				class_exists( 'Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep' )
				&& Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::title_looks_instructional( $new_title )
			) {
				return array(
					'success' => false,
					'error'   => __( 'Title contains instructional text, not reader-facing copy.', 'flowbie-wp' ),
				);
			}
		...
		if ( count( $update ) === 1 ) {
			return array(
				'success' => false,
				'error'   => __( 'No post fields to update.', 'flowbie-wp' ),
			);
		}
	...
	public static function tool_restore_post_revision( array $params ): array {
		...
		if ( ! class_exists( 'Flowbie_Wp_Tools_Handlers' ) ) {
			require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/class-flowbie-wp-tools-handlers.php';
		}

		$rs = Flowbie_Wp_Tools_Handlers::wp_revision_restore( array( 'post_id' => $post_id ) );
		if ( is_wp_error( $rs ) ) {
			return array( 'success' => false, 'error' => $rs->get_error_message() );
		}
		...
		$post = get_post( $post_id );
		if ( ! $post instanceof WP_Post ) {
			return array( 'success' => false, 'error' => __( 'Post not found after restore.', 'flowbie-wp' ) );
		}
