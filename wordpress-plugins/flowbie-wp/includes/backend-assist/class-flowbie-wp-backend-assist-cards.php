...
class Flowbie_Wp_Backend_Assist_Cards {
	...
	public static function action_card( array $result, string $tool ): array {
		...
		if ( $success && self::should_offer_undo( $tool, $result ) ) {
			...
			if ( $post_id > 0 && Flowbie_Wp_Backend_Assist_Tools_Wp::agent_revision_available( $post_id ) ) {
		...
		switch ( $tool ) {
			...
			case 'add_content':
				if ( ! empty( $result['body_ops'] ) ) {
					if (
						...
						return __( 'Heading updated', 'flowbie-wp' );
					}
					return __( 'Post body updated (ops)', 'flowbie-wp' );
				}
			...
			case 'save_post_meta':
				...
				if (
					...
					return __( 'SEO research cleared', 'flowbie-wp' );
				}
				if ( ! empty( $result['saved'] ) && is_array( $result['saved'] ) && in_array( 'faq', $result['saved'], true ) ) {
					return __( 'FAQ schema saved to post meta', 'flowbie-wp' );
				}
				return __( 'SEO meta updated', 'flowbie-wp' );
			case 'run_seo_research_brief':
				return __( 'SEO research brief saved', 'flowbie-wp' );
			case 'faq_compound':
				return __( 'FAQ schema and table updated', 'flowbie-wp' );
			case 'body_schema_cleanup':
				return __( 'Body cleaned and FAQ schema saved', 'flowbie-wp' );
			case 'restore_post_revision':
				return __( 'Changes reverted', 'flowbie-wp' );
			default:
				$title_label = isset( $result['title'] ) ? (string) $result['title'] : str_replace( '_', ' ', $tool );
				return sprintf(
					/* translators: %s: action label */
					__( 'Done: %s', 'flowbie-wp' ),
					$title_label
				);
		}
	}
	...
	private static function tool_action_body( string $tool, array $result, bool $success ): string {
		if ( ! $success ) {
			return isset( $result['error'] ) ? (string) $result['error'] : __( 'Unknown error.', 'flowbie-wp' );
		}
		...
		switch ( $tool ) {
			...
			case 'add_content':
				...
				if ( ! empty( $result['body_ops'] ) || ! empty( $result['body_surgical'] ) ) {
					$lines[] = '**Edit:** ' . __( 'Post body updated (ops)', 'flowbie-wp' );
					...
					$lines[] = '**Edit:** ' . __( 'In-place body update', 'flowbie-wp' );
			...
			case 'save_post_meta':
				...
				if ( ! empty( $result['saved'] ) && is_array( $result['saved'] ) ) {
					...
					if ( in_array( 'faq', $saved_labels, true ) ) {
						$lines[] = '**FAQ schema:** ' . __( 'Saved to post meta (ACF faq field)', 'flowbie-wp' );
			...
			case 'faq_compound':
				...
				$lines[] = '**FAQ schema:** ' . __( 'Saved to post meta (ACF faq field)', 'flowbie-wp' );
				$lines[] = '**FAQ table:** ' . __( 'Appended to post content', 'flowbie-wp' );
				if ( isset( $result['word_count'] ) ) {
					$lines[] = '**Word count:** ' . (int) $result['word_count'];
				}
				if ( ! empty( $result['title'] ) ) {
			...
			case 'body_schema_cleanup':
				...
				$lines[] = '**Body:** ' . ( ! empty( $result['surgical_summary'] ) ? (string) $result['surgical_summary'] : __( 'JSON-LD removed from post content', 'flowbie-wp' ) );
				$lines[] = '**FAQ schema:** ' . __( 'Saved to post meta (ACF faq field)', 'flowbie-wp' );
				if ( isset( $result['word_count'] ) ) {
					$lines[] = '**Word count:** ' . (int) $result['word_count'];
				}
			...
			case 'restore_post_revision':
				...
				$lines[] = __( 'Restored the post body from before the last agent edit.', 'flowbie-wp' );
				return implode( "\n", $lines );
		}

		return sprintf(
			/* translators: %s: tool name */
			__( 'Successfully executed %s.', 'flowbie-wp' ),
			str_replace( '_', ' ', $tool )
		);
	}
	...
	private static function create_post_title_suggestions(): array {
		...
		$posts = Flowbie_Wp_Site_Inventory::get_type_items( 'post' );
		...
		$ideas = array(
			array(
				'title'   => __( 'How To Install Blinds: A Homeowner Step-by-Step Guide', 'flowbie-wp' ),
				'keyword' => 'install blinds',
				'signals' => array( 'install', 'blinds', 'how to' ),
			),
			array(
				'title'   => __( 'Room-by-Room Window Covering Guide for New Homeowners', 'flowbie-wp' ),
				'keyword' => 'window coverings guide',
				'signals' => array( 'window', 'room', 'guide' ),
			),
			array(
				'title'   => __( 'Motorized vs Manual Blinds: Which Fits Your Home?', 'flowbie-wp' ),
				'keyword' => 'motorized blinds',
				'signals' => array( 'motorized', 'manual', 'blinds' ),
			),
			array(
				'title'   => __( 'How To Fix Common Blind Problems Without Replacing Them', 'flowbie-wp' ),
				'keyword' => 'blind repair',
				'signals' => array( 'repair', 'blinds', 'how to' ),
			),
			array(
				'title'   => __( 'Choosing Child-Safe Window Treatments for Your Family', 'flowbie-wp' ),
				'keyword' => 'child safe window treatments',
				'signals' => array( 'child', 'safe', 'window' ),
			),
			array(
				'title'   => __( 'Energy-Efficient Window Coverings: What to Look For', 'flowbie-wp' ),
	...
	private static function create_page_title_suggestions(): array {
		Flowbie_Wp_Site_Inventory::warm( true );
		$pages = Flowbie_Wp_Site_Inventory::get_type_items( 'page' );
		...
		$ideas = array(
			array(
				'label'   => __( 'Free In-Home Consultation (keyword: window covering consultation)', 'flowbie-wp' ),
				'signals' => array( 'consultation', 'service', 'appointment' ),
			),
			array(
				'label'   => __( 'Blind Repair Services (keyword: blind repair)', 'flowbie-wp' ),
				'signals' => array( 'repair', 'service', 'blinds' ),
			),
			array(
				'label'   => __( 'Custom Window Coverings (keyword: custom blinds)', 'flowbie-wp' ),
				'signals' => array( 'custom', 'window', 'blinds' ),
			),
			array(
				'label'   => __( 'Hunter Douglas Products (keyword: hunter douglas)', 'flowbie-wp' ),
				'signals' => array( 'hunter', 'douglas', 'products' ),
			),
			array(
				'label'   => __( 'Operating Systems Guide (keyword: blind operating systems)', 'flowbie-wp' ),
	...
	public static function enrich_card( array $card, string $tool = '', array $exec_result = array() ): array {
		...
		if ( $has_exec && ! empty( $exec_result['post_id'] ) && ! self::links_have_undo( $links ) ) {
			...
			if ( $inferred_tool !== '' && self::should_offer_undo( $inferred_tool, $exec_result ) ) {
				...
				if ( Flowbie_Wp_Backend_Assist_Tools_Wp::agent_revision_available( $post_id ) ) {
	...
	public static function sanitize_card_links( array $links, bool $resolve_inventory = true ): array {
		...
		foreach ( $links as $link ) {
			...
			if ( $action === 'undo' && ! empty( $link['post_id'] ) ) {
				$out[] = array(
					'label'   => $label !== '' ? $label : __( 'Undo', 'flowbie-wp' ),
					'url'     => '#',
			...
			if ( $resolve_inventory && ! self::is_valid_card_link_url( $url ) && $label !== '' ) {
				$item = Flowbie_Wp_Site_Inventory::find_item_by_title( $label );
	...
	private static function links_from_tool_result( string $tool, array $result, string $body ): array {
		...
		if ( $tool === 'grade_post_library_seo' && ! empty( $result['posts'] ) && is_array( $result['posts'] ) ) {
			...
			foreach ( $result['posts'] as $post ) {
				...
				$links[] = array(
					'label' => (string) ( $post['title'] ?? __( 'View post', 'flowbie-wp' ) ),
					'url'   => (string) $post['url'],
					'icon'  => 'post',
				);
			}
	...
	private static function standard_action_links( array $result ): array {
		...
		if ( ! empty( $result['elementor_edit_url'] ) ) {
			...
			$edit_label = $status === 'draft'
				? ( $type === 'page' ? __( 'Edit draft page', 'flowbie-wp' ) : __( 'Edit draft post', 'flowbie-wp' ) )
				: ( $type === 'page' ? __( 'Edit page', 'flowbie-wp' ) : __( 'Edit post', 'flowbie-wp' ) );
			$links[] = array(
				'label' => $edit_label,
		...
		if ( ! empty( $result['view_url'] ) ) {
			$view_label = $status === 'publish'
				? ( $title !== '' ? $title : ( $type === 'page' ? __( 'View page', 'flowbie-wp' ) : __( 'View post', 'flowbie-wp' ) ) )
				: __( 'Preview', 'flowbie-wp' );
			$links[] = array(
				'label' => $view_label,
	...
	private static function attach_primary_cta( array $card, array $result ): array {
		...
		if ( $status === 'publish' && self::is_valid_card_link_url( $view ) ) {
			$card['cta'] = array(
				'label' => $type === 'page' ? __( 'View page', 'flowbie-wp' ) : __( 'View post', 'flowbie-wp' ),
				'url'   => $view,
			);
	...
	private static function ensure_body_links_to_result( array $card, array $result ): array {
		...
		foreach ( self::standard_action_links( $result ) as $link ) {
			...
			$label = trim( (string) ( $link['label'] ?? __( 'Open', 'flowbie-wp' ) ) );
			$append[] = '[' . $label . '](' . $url . ')';
		}
	...
	private static function links_from_body_inventory_refs( string $body ): array {
		...
		if ( preg_match_all( '/\bID:(\d+)\b/i', $body, $matches ) ) {
			...
			foreach ( $ids as $post_id ) {
				...
				$links[] = array(
					'label' => $title !== '' ? $title : sprintf( __( 'Post %d', 'flowbie-wp' ), $post_id ),
					'url'   => $url,
					'icon'  => 'post',
				);
			}
