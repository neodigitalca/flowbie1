...
class Flowbie_Wp_Backend_Assist_Ai {
	...
	public static function call_openrouter( string $model, string $system_prompt, string $user_prompt, int $max_tokens = 1024, float $temperature = 0.3 ) {
		...
		if ( $key === '' ) {
			return new WP_Error( 'flowbie_openrouter_key', __( 'OpenRouter API key not configured.', 'flowbie-wp' ) );
		}
		...
		if ( $code < 200 || $code >= 300 ) {
			...
			return new WP_Error( 'flowbie_backend_ai', $msg ?: sprintf( 'HTTP %d', $code ) );
		}
		...
		if ( $text === '' ) {
			return new WP_Error( 'flowbie_backend_empty', __( 'AI returned empty content.', 'flowbie-wp' ) );
		}
