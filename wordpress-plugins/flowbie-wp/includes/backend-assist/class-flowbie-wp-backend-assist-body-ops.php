...
class Flowbie_Wp_Backend_Assist_Body_Ops {
		...
		foreach ( $filtered_ops as $op ) {
			...
			switch ( $name ) {
				...
				case 'remove_sections_after':
					...
					break;
				case 'strip_json_ld':
					$html = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::strip_json_ld_from_html( $html );
					break;
				case 'wrap':
				...
				default:
					return new WP_Error(
						'flowbie_body_op_unknown',
						sprintf(
							/* translators: %s: operation name */
							__( 'Unknown body edit operation: %s', 'flowbie-wp' ),
							$name
						)
					);
			}
	...
	public static function validate_ops_result( string $before, string $after, array $ops ) {
		...
		foreach ( $before_sections as $sec ) {
			...
			if ( ! $found ) {
				return new WP_Error(
					'flowbie_body_op_table_lost',
					__( 'Body edit would remove or reformat a table section. Operation rejected.', 'flowbie-wp' )
				);
			}
		}
				...
				case 'remove_section':
					if ( isset( $op['section_index'] ) ) {
						$parts[] = sprintf(
							...
							__( 'removed section %d', 'flowbie-wp' ),
							(int) $op['section_index']
						);
					} elseif ( ! empty( $op['heading_match'] ) ) {
						$parts[] = sprintf(
							/* translators: %s: heading text */
							__( 'removed section matching "%s"', 'flowbie-wp' ),
							(string) $op['heading_match']
						);
					}
					break;
				case 'strip_json_ld':
					$parts[] = __( 'stripped JSON-LD from body', 'flowbie-wp' );
					break;
				case 'wrap':
					$parts[] = __( 'applied inline wrap', 'flowbie-wp' );
					break;
				case 'replace_text':
					$parts[] = __( 'replaced text', 'flowbie-wp' );
					break;
				case 'replace_heading':
					if ( ! empty( $op['new_heading'] ) ) {
						$parts[] = sprintf(
							/* translators: %s: new heading text */
							__( 'updated heading to "%s"', 'flowbie-wp' ),
							(string) $op['new_heading']
						);
					} else {
						$parts[] = __( 'updated section heading', 'flowbie-wp' );
					}
					break;
				case 'convert_section_to_table':
					if ( ! empty( $op['heading_match'] ) ) {
						$parts[] = sprintf(
							/* translators: %s: heading text */
							__( 'converted section "%s" to table', 'flowbie-wp' ),
							(string) $op['heading_match']
						);
					} elseif ( isset( $op['section_index'] ) ) {
						$parts[] = sprintf(
							/* translators: %d: section index */
							__( 'converted section %d to table', 'flowbie-wp' ),
							(int) $op['section_index']
						);
					} else {
						$parts[] = __( 'converted section to table', 'flowbie-wp' );
					}
					break;
				case 'insert_table_in_section':
					if ( ! empty( $op['heading_match'] ) ) {
						$parts[] = sprintf(
							/* translators: %s: heading text */
							__( 'inserted new table in section "%s"', 'flowbie-wp' ),
							(string) $op['heading_match']
						);
					} elseif ( isset( $op['section_index'] ) ) {
						$parts[] = sprintf(
							/* translators: %d: section index */
							__( 'inserted new table in section %d', 'flowbie-wp' ),
							(int) $op['section_index']
						);
					} else {
						$parts[] = __( 'inserted new table in section', 'flowbie-wp' );
					}
					break;
				case 'replace_section_html':
					if ( ! empty( $op['heading_match'] ) ) {
						$parts[] = sprintf(
							/* translators: %s: heading text */
							__( 'replaced section "%s" in place', 'flowbie-wp' ),
							(string) $op['heading_match']
						);
					} elseif ( isset( $op['section_index'] ) ) {
						$parts[] = sprintf(
							/* translators: %d: section index */
							__( 'replaced section %d in place', 'flowbie-wp' ),
							(int) $op['section_index']
						);
					} else {
						$parts[] = __( 'replaced section in place', 'flowbie-wp' );
					}
					break;
				case 'convert_list_in_section':
					if ( ! empty( $op['heading_match'] ) ) {
						$parts[] = sprintf(
							...
							__( 'converted list in section "%1$s" to %2$s', 'flowbie-wp' ),
							(string) $op['heading_match'],
							sanitize_key( (string) ( $op['list_type'] ?? 'ul' ) )
						);
					} else {
						$parts[] = __( 'converted list format in section', 'flowbie-wp' );
					}
					break;
				case 'remove_table_in_section':
					if ( ! empty( $op['heading_match'] ) ) {
						$parts[] = sprintf(
							/* translators: %s: heading text */
							__( 'removed table from section "%s"', 'flowbie-wp' ),
							(string) $op['heading_match']
						);
					} else {
						$parts[] = __( 'removed table from section', 'flowbie-wp' );
					}
					break;
				case 'add_internal_links':
					...
					$parts[] = $count > 0
						? sprintf(
							...
							__( 'added up to %d internal links', 'flowbie-wp' ),
							$count
						)
						: __( 'added internal links', 'flowbie-wp' );
					break;
				case 'insert_overview_links':
					$parts[] = __( 'inserted overview with H2 anchor links', 'flowbie-wp' );
					break;
				case 'truncate_after_table':
					$parts[] = __( 'removed all content after table', 'flowbie-wp' );
					break;
				case 'remove_sections_after':
					if ( ! empty( $op['after_heading_match'] ) ) {
						$parts[] = sprintf(
							/* translators: %s: heading text */
							__( 'removed all sections after "%s"', 'flowbie-wp' ),
							(string) $op['after_heading_match']
						);
					} elseif ( isset( $op['after_section_index'] ) ) {
						$parts[] = sprintf(
							/* translators: %d: section index */
							__( 'removed all sections after section %d', 'flowbie-wp' ),
							(int) $op['after_section_index']
						);
					} else {
						$parts[] = __( 'removed all sections after anchor', 'flowbie-wp' );
					}
					break;
			}
		}
			...
			switch ( $name ) {
				case 'insert_table_in_section':
					$target = ! empty( $op['heading_match'] )
						? (string) $op['heading_match']
						: ( isset( $op['section_index'] ) ? sprintf( __( 'section %d', 'flowbie-wp' ), (int) $op['section_index'] ) : __( 'target section', 'flowbie-wp' ) );
					...
					$lines[] = sprintf(
						...
						__( '%1$d. `%2$s` on "%3$s" — new table with columns (%4$s), placed %5$s. Existing paragraphs in that section stay unchanged.', 'flowbie-wp' ),
						$n,
				...
				case 'convert_section_to_table':
					...
					$gen_note = ! empty( $op['generate_rows'] )
						? __( 'Rows generated from section content.', 'flowbie-wp' )
						: __( 'Rows parsed from existing section content.', 'flowbie-wp' );
					$lines[] = sprintf(
						/* translators: 1: step number, 2: op name, 3: section, 4: columns, 5: row source note */
						__( '%1$d. `%2$s` on "%3$s" — list replaced with table (%4$s). %5$s Intro paragraphs in that section stay unchanged.', 'flowbie-wp' ),
						$n,
				...
				case 'replace_section_html':
					...
					$lines[] = sprintf(
						...
						__( '%1$d. `%2$s` on "%3$s" — entire section HTML is swapped in place. Other sections stay unchanged.', 'flowbie-wp' ),
						$n,
				...
				case 'convert_list_in_section':
					...
					$lines[] = sprintf(
						...
						__( '%1$d. `%2$s` on "%3$s" — change list to %4$s only. Heading, paragraphs, and other sections stay unchanged.', 'flowbie-wp' ),
						$n,
				...
				case 'remove_table_in_section':
					...
					$lines[] = sprintf(
						/* translators: 1: step number, 2: op name, 3: section */
						__( '%1$d. `%2$s` on "%3$s" — remove table from section body only. Prose and lists stay unchanged.', 'flowbie-wp' ),
						$n,
	...
	public static function convert_section_to_table( string $html, array $op ): string {
		...
		$rows = array();
		if ( $needs_generate && class_exists( 'Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep' ) ) {
			$generated = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::generate_table_rows_for_section(
				$user_message,
	...
	public static function apply_replace_section_html( string $html, array $op ): string {
		...
		if ( ! empty( $op['new_html'] ) ) {
			...
			$generated = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::generate_section_html_for_replace(
				(string) ( $op['user_message'] ?? '' ),
				(string) $sec['heading_text'],
				$chunk
			);
	...
	public static function add_internal_links( string $html, array $op ): string {
		...
		$allowed   = Flowbie_Wp_Harness_Links::allowed_url_set( $matched, $site_url );
		...
		$html = Flowbie_Wp_Harness_Links::strip_unknown_internal_links( $scope_html, $allowed, $site_url );
		return $html;
	}
	...
	public static function insert_overview_links( string $html, array $op ): string {
		if ( preg_match( '/class=["\']flowbie-overview["\']/i', $html ) ) {
			return $html;
		}
		...
		$block = '<p class="flowbie-overview"><strong>' . esc_html( $label ) . '</strong></p>' . "\n"
			. '<ul class="flowbie-overview">' . implode( '', $items ) . '</ul>' . "\n\n";
