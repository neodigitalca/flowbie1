...
class Flowbie_Wp_Backend_Assist_Subagent_Wysiwyg {
	...
	public static function run_agent( string $agent_id, string $message, array $history, int $post_id, array $prior = array() ) {
		switch ( $agent_id ) {
			...
			default:
				return new WP_Error( 'flowbie_wysiwyg_agent', __( 'WYSIWYG agent not implemented in this release.', 'flowbie-wp' ) );
		}
	}
	...
	public static function run_body_heading( string $message, array $history, int $post_id ) {
		if ( is_array( self::$test_outputs ) && isset( self::$test_outputs['body_heading'] ) ) {
			...
			$ops = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::build_heading_change_ops_public( $message, $post_id, $history );
			if ( is_wp_error( $ops ) ) {
				return $ops;
			}
			if ( ! is_array( $ops ) || empty( $ops[0]['new_heading'] ) ) {
				return new WP_Error( 'flowbie_body_heading', __( 'Heading agent returned empty copy.', 'flowbie-wp' ) );
			}
			return array(
	...
	public static function run_body_section( string $message, array $history, int $post_id, array $prior = array() ) {
		if ( is_array( self::$test_outputs ) && isset( self::$test_outputs['body_section'] ) ) {
			...
			if ( ! $post instanceof WP_Post ) {
				return new WP_Error( 'flowbie_body_section', __( 'Post not found.', 'flowbie-wp' ) );
			}
			...
			$generated = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::generate_section_html_for_replace( $message, $heading, trim( $chunk ) );
		...
		if ( trim( $html ) === '' ) {
			return new WP_Error( 'flowbie_body_section', __( 'Section agent returned empty HTML.', 'flowbie-wp' ) );
		}
	...
	public static function run_body_faq_table( string $message, array $history, int $post_id, array $prior = array() ) {
		...
		$html = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::faq_entries_to_table_html( $entries );
		if ( trim( $html ) === '' ) {
			return new WP_Error( 'flowbie_faq_table', __( 'FAQ table generation failed.', 'flowbie-wp' ) );
		}
	...
	public static function run_body_full_post( string $message, array $history, int $post_id ) {
		...
		$html   = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::generate_post_body_html( $message, $history, $params );
		if ( is_wp_error( $html ) ) {
			return $html;
		}
		return array(
	...
	private static function resolve_target_section( string $message, int $post_id, array $history ) {
		...
		if ( is_array( $ops ) && ! empty( $ops[0]['section_index'] ) ) {
			...
			$post = get_post( $post_id );
			if ( ! $post instanceof WP_Post ) {
				return new WP_Error( 'flowbie_body_target', __( 'Post not found.', 'flowbie-wp' ) );
			}
		...
		$post = get_post( $post_id );
		if ( ! $post instanceof WP_Post ) {
			return new WP_Error( 'flowbie_body_target', __( 'Could not resolve body section.', 'flowbie-wp' ) );
		}
		$sections = Flowbie_Wp_Backend_Assist_Body_Ops::index_html_sections( (string) $post->post_content );
		if ( $sections === array() ) {
			return new WP_Error( 'flowbie_body_target', __( 'Could not resolve body section.', 'flowbie-wp' ) );
		...
		$intent = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::classify_body_edit_intent( $message, $post_id, $history );
