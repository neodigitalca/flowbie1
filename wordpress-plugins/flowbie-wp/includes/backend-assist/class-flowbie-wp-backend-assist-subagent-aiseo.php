...
class Flowbie_Wp_Backend_Assist_Subagent_Aiseo {
	...
	public static function build_context( int $post_id ): array {
		...
		if ( $post instanceof WP_Post ) {
			...
			$ctx['post_title'] = Flowbie_Wp_Display_Text::decode( (string) $post->post_title );
		...
		if ( class_exists( 'Flowbie_Wp_Ai_Context' ) ) {
			...
			$ctx['seo_title']        = trim( Flowbie_Wp_Ai_Context::read_seo_title( $post_id ) );
			$ctx['meta_description'] = trim( Flowbie_Wp_Ai_Context::read_meta_description( $post_id ) );
		...
		if ( $ctx['focus_keyword'] === '' && class_exists( 'Flowbie_Wp_Ai_Seo_Limits' ) ) {
	...
	public static function run_agent( string $agent_id, string $message, array $history, array $ctx, array $prior = array() ) {
		switch ( $agent_id ) {
			...
			default:
				return new WP_Error( 'flowbie_agent_unknown', __( 'Unknown AISEO agent.', 'flowbie-wp' ) );
		}
	}
	...
	public static function run_seo_title( string $message, array $history, array $ctx, string $retry_note = '' ) {
		...
		if ( $title === '' ) {
			return new WP_Error( 'flowbie_meta_title_parse', __( 'SEO title generation returned invalid JSON.', 'flowbie-wp' ) );
		}
	...
	public static function run_meta_description( string $message, array $history, array $ctx, string $seo_title, string $retry_note = '' ) {
		...
		if ( $desc === '' ) {
			return new WP_Error( 'flowbie_meta_desc_parse', __( 'Meta description generation returned invalid JSON.', 'flowbie-wp' ) );
		}
	...
	public static function run_focus_keyword( string $message, array $history, array $ctx ) {
		if ( is_array( self::$test_outputs ) && isset( self::$test_outputs['focusKeyword'] ) ) {
			...
			return $kw === ''
				? new WP_Error( 'flowbie_focus_kw_empty', __( 'Focus keyword generation returned empty copy.', 'flowbie-wp' ) )
...
PROMPT;
		...
		if ( $kw === '' || Flowbie_Wp_Ai_Seo_Limits::is_placeholder_copy( $kw ) ) {
			return new WP_Error( 'flowbie_focus_kw_empty', __( 'Focus keyword generation returned empty copy.', 'flowbie-wp' ) );
		}
	...
	public static function run_faq_schema( string $message, array $history, array $ctx ) {
		...
		$schema = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::faq_entries_to_schema_json( $entries );
		if ( $schema === '' ) {
			return new WP_Error( 'flowbie_faq_schema', __( 'FAQ schema generation failed.', 'flowbie-wp' ) );
		}
	...
	private static function build_user_prompt( string $message, array $history, array $ctx, array $constraints, string $retry_note ): string {
		$user  = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::build_planner_history_block( $history );
		$user .= self::rewrite_intent_block( $message );
		if ( $retry_note !== '' ) {
			$user .= $retry_note . "\n\n";
		}
		$user .= "USER REQUEST:\n{$message}\n\n";
		$user .= Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::build_post_context_for_plan( (int) ( $ctx['post_id'] ?? 0 ) );
		...
		if ( class_exists( 'Flowbie_Wp_Gsc_Prompt' ) && ! empty( $ctx['post_id'] ) ) {
			$gsc = Flowbie_Wp_Gsc_Prompt::for_post( (int) $ctx['post_id'], $fk );
			if ( $gsc !== '' ) {
				$user .= "\n{$gsc}\n";
			}
		}

		$constraint_block = Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::meta_copy_constraints_prompt_block( $constraints, $message );
		if ( $constraint_block !== '' ) {
			$user .= "\n{$constraint_block}";
		}
	...
	private static function complete_json( string $system, string $user, int $max_tokens, float $temperature ) {
		...
		return is_array( $parsed ) ? $parsed : new WP_Error( 'flowbie_agent_json', __( 'Agent returned invalid JSON.', 'flowbie-wp' ) );
	}
	...
	public static function validate_seo_title( int $post_id, string $title, array $ctx, string $message = '', array $constraints = array() ) {
		...
		$title = Flowbie_Wp_Ai_Seo_Limits::normalize_title( trim( sanitize_text_field( $title ) ) );
		if ( $title === '' || Flowbie_Wp_Ai_Seo_Limits::is_placeholder_copy( $title ) ) {
			return new WP_Error( 'flowbie_meta_title_empty', __( 'SEO title generation returned empty or placeholder copy.', 'flowbie-wp' ) );
		}

		$post_title = trim( (string) ( $ctx['post_title'] ?? '' ) );
		$current    = trim( (string) ( $ctx['seo_title'] ?? '' ) );
		if ( $current !== '' && strcasecmp( $current, $title ) === 0 ) {
			return new WP_Error( 'flowbie_meta_title_unchanged', __( 'SEO title must differ from the current saved title.', 'flowbie-wp' ) );
		}
		if ( $post_title !== '' && self::copy_too_similar( $title, $post_title ) ) {
			return new WP_Error( 'flowbie_meta_title_paraphrase', __( 'SEO title is too similar to the post title. Write distinct SERP copy with keyword near the start.', 'flowbie-wp' ) );
		}

		if ( $constraints !== array() && ! Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::meta_copy_meets_constraints( $title, $constraints ) ) {
			return new WP_Error( 'flowbie_meta_title_constraints', __( 'SEO title did not meet formatting constraints.', 'flowbie-wp' ) );
		}
	...
	public static function validate_meta_description( int $post_id, string $desc, array $ctx, string $message = '', array $constraints = array() ) {
		...
		$fk   = trim( (string) ( $ctx['focus_keyword'] ?? '' ) );
		$desc = Flowbie_Wp_Ai_Seo_Limits::normalize_description( trim( sanitize_textarea_field( $desc ) ), $fk );
		if ( $desc === '' || Flowbie_Wp_Ai_Seo_Limits::is_placeholder_copy( $desc ) ) {
			return new WP_Error( 'flowbie_meta_desc_empty', __( 'Meta description generation returned empty or placeholder copy.', 'flowbie-wp' ) );
		}

		$post_title = trim( (string) ( $ctx['post_title'] ?? '' ) );
		$current    = trim( (string) ( $ctx['meta_description'] ?? '' ) );
		if ( $current !== '' && self::copy_too_similar( $desc, $current ) ) {
			return new WP_Error( 'flowbie_meta_desc_unchanged', __( 'Meta description must differ materially from the current saved description.', 'flowbie-wp' ) );
		}
		if ( $post_title !== '' && empty( $ctx['is_page'] ) && self::copy_too_similar( $desc, $post_title ) ) {
			return new WP_Error( 'flowbie_meta_desc_paraphrase', __( 'Meta description is too similar to the post title.', 'flowbie-wp' ) );
		}

		if ( ! Flowbie_Wp_Backend_Assist_Pipeline_Content_Prep::meta_copy_meets_constraints( $desc, $constraints ) ) {
			return new WP_Error( 'flowbie_meta_desc_constraints', __( 'Meta description did not meet formatting constraints.', 'flowbie-wp' ) );
		}
