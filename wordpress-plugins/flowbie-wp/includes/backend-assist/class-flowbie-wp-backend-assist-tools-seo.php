...
class Flowbie_Wp_Backend_Assist_Tools_Seo {
	...
	public static function tool_modify_seo_block_slots( array $params ): array {
		...
		if ( ! is_array( $block ) ) {
			return array(
				'success' => false,
				'error'   => __( 'No SEO block found. Open a block in Agent Hub or provide block_id.', 'flowbie-wp' ),
			);
		}
		...
		if ( $action === 'remove' ) {
			$result = Flowbie_Wp_Seo_Blocks_Mutation::remove_slot( $block, $target );
			...
			$result = Flowbie_Wp_Seo_Blocks_Mutation::update_slot( $block, $target, $patch );
		} else {
			...
			$result    = Flowbie_Wp_Seo_Blocks_Mutation::add_slot( $block, $slot, $placement );
		}
		...
		$manifest = Flowbie_Wp_Seo_Blocks_Agent::normalize_manifest_public( $result );
		...
		return array(
			...
			'summary'        => sprintf(
				...
				__( 'SEO block updated (%s slot). Apply to builder or save when ready.', 'flowbie-wp' ),
				$action
			),
		);
	}
	public static function tool_list_seo_blocks( array $params ): array {
		...
		return array(
			...
			'summary' => sprintf(
				...
				_n( '%d SEO block found.', '%d SEO blocks found.', count( $items ), 'flowbie-wp' ),
				count( $items )
			),
		);
	}
	public static function tool_create_seo_block( array $params ): array {
		...
		if ( $title === '' ) {
			return array( 'success' => false, 'error' => __( 'Title or focus keyword is required.', 'flowbie-wp' ) );
		}

		$result = Flowbie_Wp_Seo_Blocks_Storage::save(
			array(
				'title'         => $title,
				'focus_keyword' => $fk,
				'status'        => 'draft',
		...
		return array(
			...
			'block_id' => $id,
			'title'    => $title,
			'summary'  => sprintf( __( 'Created draft SEO block "%s".', 'flowbie-wp' ), $title ),
	...
	public static function tool_delete_seo_block( array $params ): array {
		...
		if ( $id < 1 ) {
			return array( 'success' => false, 'error' => __( 'block_id is required.', 'flowbie-wp' ) );
		}

		$row = Flowbie_Wp_Seo_Blocks_Storage::get( $id );
		if ( ! is_array( $row ) ) {
			return array( 'success' => false, 'error' => __( 'SEO block not found.', 'flowbie-wp' ) );
		}

		$deleted = Flowbie_Wp_Seo_Blocks_Storage::delete( $id, ! empty( $params['trash_library'] ) );
		...
		return array(
			...
			'block_id' => $id,
			'title'    => (string) ( $row['title'] ?? '' ),
			'summary'  => sprintf( __( 'Deleted SEO block "%s".', 'flowbie-wp' ), (string) ( $row['title'] ?? $id ) ),
		);
	}
	public static function tool_save_seo_block( array $params ): array {
		...
		if ( empty( $payload ) ) {
			return array( 'success' => false, 'error' => __( 'No block manifest to save.', 'flowbie-wp' ) );
		}

		$result = Flowbie_Wp_Seo_Blocks_Storage::save( $payload );
		if ( is_wp_error( $result ) ) {
			return array( 'success' => false, 'error' => $result->get_error_message() );
		}
		...
		return array(
			...
			'summary'        => sprintf( __( 'Saved SEO block "%s".', 'flowbie-wp' ), (string) ( $result['title'] ?? '' ) ),
			'edit_url'       => admin_url( 'admin.php?page=flowbie-wp-agent-hub-edit&block_id=' . absint( $result['id'] ?? 0 ) ),
		);
	}
	public static function tool_apply_seo_block_to_page( array $params ): array {
		...
		if ( ! class_exists( 'Flowbie_Wp_Seo_Blocks_Page_Insert', false ) ) {
		...
		$result = Flowbie_Wp_Seo_Blocks_Page_Insert::insert_registry_widget(
			$post_id,
			$block_id,
			array(
				'mode'                    => $mode,
