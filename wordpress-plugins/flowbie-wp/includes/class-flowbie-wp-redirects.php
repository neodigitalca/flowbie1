...
class Flowbie_Wp_Redirects {

	const TABLE_VERSION = '1.0';
	const OPTION_KEY    = 'flowbie_wp_redirects_settings';
	...
	public static function table_name(): string {
		global $wpdb;
		return $wpdb->prefix . 'flowbie_wp_redirects';
	}
	...
	public static function maybe_install(): void {
		if ( get_option( 'flowbie_wp_redirects_db_version', '' ) !== self::TABLE_VERSION ) {
			self::install();
		}
	}
	...
	public static function install(): void {
		...
		dbDelta( $sql );
		update_option( 'flowbie_wp_redirects_db_version', self::TABLE_VERSION, false );
	...
	public static function get_by_source( string $source ) {
		global $wpdb;
		$norm = Flowbie_Wp_Redirects_Csv::normalize_relative_path( $source );
		if ( ! $norm ) {
			return null;
		}
		$table = self::table_name();
	...
	public static function save( array $data ) {
		...
		$source = isset( $data['source'] ) ? Flowbie_Wp_Redirects_Csv::normalize_relative_path( (string) $data['source'] ) : null;
		if ( ! $source ) {
			return array(
				'ok'    => false,
				'error' => __( 'Source path is required.', 'flowbie-wp' ),
			);
		}

		$destination = isset( $data['destination'] ) ? trim( (string) $data['destination'] ) : '';
		if ( $destination === '' || ! Flowbie_Wp_Redirects_Csv::destination_path_key( $destination ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'Destination URL or path is required.', 'flowbie-wp' ),
			);
		}
		...
		$category = isset( $data['category'] ) ? sanitize_text_field( (string) $data['category'] ) : __( 'Uncategorized', 'flowbie-wp' );
		if ( $category === '' ) {
			$category = __( 'Uncategorized', 'flowbie-wp' );
		}
		...
		if ( $id > 0 ) {
			...
			if ( ! $existing ) {
				return array(
					'ok'    => false,
					'error' => __( 'Redirect not found.', 'flowbie-wp' ),
				);
			}
			...
			if ( $conflict && (int) $conflict->id !== $id ) {
				return array(
					'ok'    => false,
					'error' => __( 'Another redirect already uses this source path.', 'flowbie-wp' ),
				);
			}
	...
	public static function maybe_redirect(): void {
		...
		$source = Flowbie_Wp_Redirects_Csv::normalize_relative_path( $path );
		if ( ! $source ) {
			return;
		}
		...
		$target = Flowbie_Wp_Redirects_Csv::resolve_destination_url( (string) $row->destination );
		if ( ! $target ) {
			return;
		}
	...
	public static function maybe_fallback_home(): void {
		...
		$normalized = Flowbie_Wp_Redirects_Csv::normalize_relative_path( $path );
		if ( ! $normalized ) {
			return;
		}
