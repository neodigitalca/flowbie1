...
require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/super-migrate/adapters/class-flowbie-wp-migrate-source-autoptimize.php';
...
class Flowbie_Wp_Super_Migrate_Registry {

	/** @var array<string, Flowbie_Wp_Migrate_Adapter>|null */
	private static $adapters = null;

	/**
	 * @return array<string, Flowbie_Wp_Migrate_Adapter>
	 */
	public static function all(): array {
		...
		$list = array(
			new Flowbie_Wp_Migrate_Source_Acf(),
			new Flowbie_Wp_Migrate_Source_Rank_Math(),
			new Flowbie_Wp_Migrate_Source_Hfcm(),
			new Flowbie_Wp_Migrate_Source_Autoptimize(),
			new Flowbie_Wp_Migrate_Source_Elementor(),
			new Flowbie_Wp_Migrate_Source_Flowbie_Native(),
		);
		self::$adapters = array();
	...
	public static function macro_label( string $group_id ): string {
		foreach ( Flowbie_Wp_Admin_Menu::get_menu_definition() as $group ) {
		...
		$fallback = array(
			...
			'seo'         => __( 'SEO', 'flowbie-wp' ),
			'performance' => __( 'Performance', 'flowbie-wp' ),
		);
	 ...
	 */
	public static function get_adapter( string $adapter_id ): ?Flowbie_Wp_Migrate_Adapter {
		$all = self::all();
