<?php
/**
 * Elementor ACF dynamic tags → Flowbie Fields dynamic tags migration.
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Migrate_Elementor_Dynamic_Tags {

	const BATCH_POSTS = 50;
	const META_KEY    = '_elementor_data';

	const WALK_MODE_IDLE             = 'idle';
	const WALK_MODE_MIGRATE_TO_FLOWBIE = 'migrate_to_flowbie';
	...
	public static function set_walk_mode( string $mode ): void {
		$allowed = array(
			self::WALK_MODE_IDLE,
			self::WALK_MODE_MIGRATE_TO_FLOWBIE,
			self::WALK_MODE_SCAN,
	...
	private static function should_migrate_widgets(): bool {
		return in_array(
			self::$walk_mode,
			array( self::WALK_MODE_MIGRATE_TO_FLOWBIE, self::WALK_MODE_SCAN ),
			true
		);
	}
	...
	private static function begin_migration_tag_registration(): void {
		...
		Flowbie_Wp_Fields_Elementor_Settings::force_options_tags_for_migration( true );
	}

	private static function end_migration_tag_registration(): void {
		if ( class_exists( 'Flowbie_Wp_Fields_Elementor_Settings', false ) ) {
			Flowbie_Wp_Fields_Elementor_Settings::force_options_tags_for_migration( false );
		}
	}

	/**
	 * ACF Elementor tag slug → Flowbie tag family (text|image|url|gallery).
	 *
	...
	public static function build_field_index(): array {
		...
		self::$field_index = Flowbie_Wp_Fields_Elementor_Acf_Resolver::build_field_index();
	...
	public static function reset_field_index(): void {
		...
		require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/fields/integrations/class-flowbie-wp-fields-elementor-acf-resolver.php';
		Flowbie_Wp_Fields_Elementor_Acf_Resolver::reset_field_index();
	}
	...
	public static function map_acf_tag( string $tag_name, array $settings ) {
		...
		require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/fields/integrations/class-flowbie-wp-fields-elementor-acf-resolver.php';
		$resolved = Flowbie_Wp_Fields_Elementor_Acf_Resolver::resolve_from_settings( $settings );
		if ( $resolved === null ) {
			return null;
		}

		$is_options = ! empty( $resolved['options_slug'] );
		$flowbie    = self::flowbie_tag_name( $family, $is_options );

		$new_settings = array();
		if ( $is_options ) {
			$new_settings['field_name'] = self::flowbie_options_field_key(
	...
	private static function flowbie_tag_name( string $family, bool $is_options ): string {
		$map = array(
			'text'    => $is_options ? 'flowbie-options-field' : 'flowbie-field',
			'image'   => $is_options ? 'flowbie-options-image' : 'flowbie-image',
			'url'     => $is_options ? 'flowbie-options-url' : 'flowbie-url',
			'gallery' => $is_options ? 'flowbie-options-gallery' : 'flowbie-gallery',
		);
		return $map[ $family ] ?? ( $is_options ? 'flowbie-options-field' : 'flowbie-field' );
	}

	public static function flowbie_options_field_key( string $options_slug, string $field_name ): string {
		if ( class_exists( 'Flowbie_Wp_Fields_Elementor_Registry', false ) ) {
			return Flowbie_Wp_Fields_Elementor_Registry::options_field_key( $options_slug, $field_name );
		}
		return $options_slug . '::' . $field_name;
	}

	/**
	 * Build a Flowbie Elementor tag shortcode from a resolved field binding.
	 *
	 * @param array{field_name: string, options_slug?: string|null} $resolved
	 */
	public static function build_flowbie_tag_from_resolved( string $family, array $resolved, string $id_seed = '' ): string {
		$is_options = ! empty( $resolved['options_slug'] );
		$name       = self::flowbie_tag_name( $family, $is_options );
		$settings   = array();
		if ( $is_options ) {
			$settings['field_name'] = self::flowbie_options_field_key(
				(string) $resolved['options_slug'],
				(string) $resolved['field_name']
			);
		} else {
	...
	public static function resolve_acf_key_setting( string $key ): ?array {
		require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/fields/integrations/class-flowbie-wp-fields-elementor-acf-resolver.php';
		return Flowbie_Wp_Fields_Elementor_Acf_Resolver::resolve_acf_key( $key );
	}
	...
	public static function rewrite_string( string $value ): array {
		...
		if ( self::WALK_MODE_REVERT_TO_ACF === $mode ) {
			return self::revert_flowbie_string( $value );
		}

		if ( ! self::should_migrate_widgets()
			&& ! apply_filters( 'flowbie_wp_migrate_rewrite_elementor_tags', false ) ) {
			return array(
				'value'        => $value,
				'replacements' => 0,
				'skipped'      => 0,
		...
		return self::migrate_acf_string_to_flowbie( $value );
	}

	/**
	 * Forward-migrate ACF Elementor tag shortcodes to Flowbie tags.
	 *
	 * @return array{value: string, replacements: int, skipped: int, tags_found: array<string, int>}
	 */
	private static function migrate_acf_string_to_flowbie( string $value ): array {
		$stats = array(
			'replacements' => 0,
			'skipped'      => 0,
			'tags_found'   => array(),
		...
		foreach ( $elements as &$element ) {
			...
			if ( ! empty( $element['settings'] ) && is_array( $element['settings'] ) ) {
				...
				if ( class_exists( 'Flowbie_Wp_Fields_Elementor_Cache_Fix', false )
					&& Flowbie_Wp_Fields_Elementor_Cache_Fix::settings_have_dynamic_tags( $element['settings'] ) ) {
	...
	public static function elementor_document_ids(): array {
		...
		$post_types = apply_filters(
			'flowbie_wp_migrate_elementor_dynamic_tags_post_types',
			array_merge(
	 ...
	 */
	private static function flowbie_to_acf_tag_map(): array {
		return array(
			'flowbie-options-field'   => 'acf-text',
			'flowbie-options-url'     => 'acf-url',
			'flowbie-options-image'   => 'acf-image',
			'flowbie-options-gallery' => 'acf-gallery',
			'flowbie-field'           => 'acf-text',
			'flowbie-url'             => 'acf-url',
			'flowbie-image'           => 'acf-image',
			'flowbie-gallery'         => 'acf-gallery',
		);
	}

	/**
	 * Restore legacy ACF dynamic tag shortcodes from migrated Flowbie tag names.
	 *
	 * @return array{value: string, replacements: int, skipped: int, tags_found: array<string, int>}
	 */
	public static function revert_flowbie_string( string $value ): array {
		require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/fields/integrations/class-flowbie-wp-fields-elementor-acf-resolver.php';
		self::build_field_index();
		...
		$map   = self::flowbie_to_acf_tag_map();
		$tags  = self::parse_elementor_tag_shortcodes( $value );
		if ( empty( $tags ) ) {
			return array(
				'value'        => $value,
		...
		foreach ( $tags as $tag ) {
			...
			$resolved = Flowbie_Wp_Fields_Elementor_Acf_Resolver::resolve_from_tag_settings( $settings );
			if ( $resolved === null ) {
				++$stats['skipped'];
				continue;
			}
			$key = Flowbie_Wp_Fields_Elementor_Acf_Resolver::to_acf_key_setting( $resolved );
			if ( $key === '' ) {
				++$stats['skipped'];
				continue;
			}
	...
	public static function repair_icon_list_settings( array $settings ): array {
		...
		$replacements = 0;
		$hour_fields  = Flowbie_Wp_Fields_Elementor_Acf_Resolver::options_fields_for_patterns( array( '/hours/i' ) );
		if ( count( $hour_fields ) === count( $items ) && self::icon_list_needs_repair( $items ) ) {
			foreach ( $items as $index => &$item ) {
				...
				$item['text']             = '';
				$item['__dynamic__text'] = self::build_flowbie_tag_from_resolved(
					'text',
					$resolved,
		...
		if ( count( $items ) === count( $contact_map ) ) {
			foreach ( $items as $index => &$item ) {
				...
				$text_fields = Flowbie_Wp_Fields_Elementor_Acf_Resolver::options_fields_for_patterns( (array) $rule['fields'] );
				$url_fields  = Flowbie_Wp_Fields_Elementor_Acf_Resolver::options_fields_for_patterns( (array) $rule['url'] );
				...
				if ( ! empty( $url_fields ) ) {
					...
					if ( $url_resolved['field_name'] !== '' && $url_resolved['options_slug'] !== '' ) {
						$item['__dynamic__link.url'] = self::build_flowbie_tag_from_resolved(
							'url',
							$url_resolved,
