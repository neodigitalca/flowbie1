<?php
/**
 * Elementor active kit globals → Flowbie Global CSS mapper.
 *
...
class Flowbie_Wp_Migrate_Elementor_Global_Css {
	...
	public static function build_mapped_values( ?array $existing = null ): array {
		...
		Flowbie_Wp_Global_Css::install();

		$base     = is_array( $existing ) ? $existing : Flowbie_Wp_Global_Css::get_stored_values();
		$settings = self::read_kit_settings();
		$mapped   = self::map_from_elementor( $settings );
	...
	public static function apply( bool $dry = false ): array {
		if ( ! self::kit_available() && ! defined( 'ELEMENTOR_VERSION' ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'Elementor kit settings were not found in the database.', 'flowbie-wp' ),
			);
		}
		...
		if ( empty( $settings ) && ! self::kit_available() ) {
			return array(
				'ok'      => true,
				'message' => __( 'No Elementor global style data to import.', 'flowbie-wp' ),
		...
		if ( $dry ) {
			return array(
				'ok'      => true,
				'message' => __( 'Dry run: would import Elementor global styles.', 'flowbie-wp' ),
		...
		Flowbie_Wp_Global_Css::install();
		$field_map = Flowbie_Wp_Global_Css::get_field_map();
		$updated   = 0;
		...
		foreach ( $values as $name => $value ) {
			...
			Flowbie_Wp_Fields_Values::update_option( Flowbie_Wp_Global_Css::OPTION_SLUG, $field_map[ $name ], $value );
			++$updated;
		}

		return array(
			'ok'      => true,
			'message' => __( 'Elementor global styles imported into Global CSS.', 'flowbie-wp' ),
			'stats'   => array(
				'updated' => $updated,
