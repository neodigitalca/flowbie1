<?php
/**
 * ACF → Flowbie Fields Super Migrate adapter.
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Migrate_Source_Acf implements Flowbie_Wp_Migrate_Adapter {
	...
	public function get_label(): string {
		return __( 'Advanced Custom Fields', 'flowbie-wp' );
	}
	...
	public function detect(): array {
		$active = class_exists( 'Flowbie_Wp_Fields', false ) && Flowbie_Wp_Fields::acf_is_active();
		$info   = array(
			'active'  => $active || $this->has_local_json(),
			'version' => defined( 'ACF_VERSION' ) ? ACF_VERSION : ( defined( 'ACF_PRO' ) ? 'pro' : '' ),
		);
		if ( $this->has_local_json() ) {
			$info['local_json_groups'] = count( Flowbie_Wp_Fields_Local_Json::load_all() );
		}
		return $info;
	}
		...
		if ( 'crawl' === $phase ) {
			$steps = array(
				array(
					...
					'label' => __( 'Crawl ACF field structure', 'flowbie-wp' ),
					'total' => 1,
				),
			);
			if ( $this->acf_plugin_active() ) {
				$steps[] = array(
					...
					'label' => __( 'Crawl ACF options page values', 'flowbie-wp' ),
					'total' => 1,
				);
				$steps[] = array(
					'id'    => 'acf_crawl_values',
					'label' => __( 'Crawl ACF post field values', 'flowbie-wp' ),
		...
		if ( 'apply' === $phase ) {
			return array(
				array(
					...
					'label' => __( 'Import field structure into Flowbie Fields', 'flowbie-wp' ),
					'total' => 1,
				),
				array(
					'id'    => 'acf_apply_options_values',
					'label' => __( 'Apply ACF options page values', 'flowbie-wp' ),
					'total' => 1,
				),
				array(
					'id'    => 'acf_apply_values',
					'label' => __( 'Apply ACF field values to posts', 'flowbie-wp' ),
	...
	public function run_step( string $step_id, string $phase, array &$sheet, array $context ): array {
		...
		return array(
			'ok'    => false,
			'error' => __( 'Unknown ACF import step.', 'flowbie-wp' ),
		);
	}
	...
	public static function is_acf_plugin_active(): bool {
		return class_exists( 'Flowbie_Wp_Fields', false ) && Flowbie_Wp_Fields::acf_is_active();
	}

	private function has_local_json(): bool {
		return class_exists( 'Flowbie_Wp_Fields_Local_Json', false ) && ! empty( Flowbie_Wp_Fields_Local_Json::load_all() );
	}
	...
	public static function fetch_structure_items(): array {
		...
		} elseif ( class_exists( 'Flowbie_Wp_Fields_Local_Json', false ) ) {
			$local = Flowbie_Wp_Fields_Local_Json::load_all();
			if ( ! empty( $local ) ) {
				$items = $local;
			}
		}
	...
	public static function resolve_options_page_slugs( array $sheet = array() ): array {
		...
		if ( empty( $slugs ) && class_exists( 'Flowbie_Wp_Fields_Storage', false ) ) {
	...
	public static function resolve_field_groups( array $sheet = array() ): array {
		...
		if ( class_exists( 'Flowbie_Wp_Fields_Storage', false ) ) {
			return Flowbie_Wp_Fields_Storage::get_all_groups( true );
		}
		return array();
	}
	...
	public static function top_level_fields_for_options_page( array $groups, string $menu_slug ): array {
		...
		$fields = array();
		foreach ( $groups as $group ) {
			if ( ! is_array( $group ) || ! class_exists( 'Flowbie_Wp_Fields_Location', false ) ) {
				continue;
			}
	...
	public static function top_level_fields_for_post( array $groups, int $post_id ): array {
		if ( $post_id < 1 || ! class_exists( 'Flowbie_Wp_Fields_Values', false ) ) {
			return array();
		}
	...
	public static function fields_for_options_apply( array $groups, string $menu_slug, array $values ): array {
		...
		if ( empty( $indexed ) && class_exists( 'Flowbie_Wp_Fields_Storage', false ) ) {
			$indexed = self::indexed_fields_for_options_page(
				Flowbie_Wp_Fields_Storage::get_all_groups( true ),
				$menu_slug
			);
		}
		...
		foreach ( array_keys( $values ) as $name ) {
			...
			if ( class_exists( 'Flowbie_Wp_Fields_Values', false ) ) {
				$found = Flowbie_Wp_Fields_Values::find_field( $name );
				if ( is_array( $found ) ) {
					$fields[] = $found;
					continue;
				}
	...
	public static function apply_field_values_to_post( int $post_id, array $fields, array $values, bool $dry = false ): int {
		...
		foreach ( $fields as $field ) {
			...
			if ( Flowbie_Wp_Fields_Values::update_value( $post_id, $field, $values[ $name ] ) ) {
				++$updated;
			}
		}
		return $updated;
	...
	public static function apply_field_values_to_options( string $menu_slug, array $fields, array $values, bool $dry = false ): int {
		...
		foreach ( $fields as $field ) {
			...
			Flowbie_Wp_Fields_Values::update_option( $menu_slug, $field, $values[ $name ] );
			++$updated;
		}
		return $updated;
	}
	...
	public static function import_structure_from_sources(): array {
		...
		if ( empty( $items ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'No ACF field structure found to import.', 'flowbie-wp' ),
			);
		}
		...
		$result = Flowbie_Wp_Fields_Import_Export::import_json_string( is_string( $json ) ? $json : '[]', false );

		return array(
			'ok'      => ! empty( $result['success'] ),
			'message' => isset( $result['message'] ) ? (string) $result['message'] : '',
			'error'   => empty( $result['success'] ) ? (string) ( $result['message'] ?? __( 'ACF structure import failed.', 'flowbie-wp' ) ) : '',
	...
	public static function import_all_from_database(): array {
		...
		if ( ! $adapter->is_available() ) {
			return array(
				'ok'    => false,
				'error' => __( 'ACF is not installed and no ACF local JSON was found.', 'flowbie-wp' ),
			);
		}
	...
	private function crawl_structure( array &$sheet ): array {
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Crawled %1$d field group(s), %2$d post type(s), %3$d taxonomy(ies), %4$d options page(s).', 'flowbie-wp' ),
				count( $groups ),
				count( $post_types ),
				count( $taxonomies ),
				count( $options_pages )
	...
	private function crawl_options_values( array &$sheet ): array {
		if ( ! function_exists( 'get_field' ) ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'ACF get_field unavailable; skipped options value crawl.', 'flowbie-wp' ),
			);
		}
		...
		if ( empty( $slugs ) ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'No ACF options pages to crawl.', 'flowbie-wp' ),
				'stats'   => array( 'pages' => 0 ),
			);
		}
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Crawled options values for %1$d page(s), %2$d field(s).', 'flowbie-wp' ),
				count( $slugs ),
				$field_count
			),
			'stats'   => array(
	...
	private function crawl_values_batch( array &$sheet, int $offset ): array {
		if ( ! function_exists( 'get_posts' ) || ! function_exists( 'get_field' ) ) {
			return array(
				'ok'   => true,
				'done' => true,
				'message' => __( 'ACF get_field unavailable; skipped value crawl.', 'flowbie-wp' ),
			);
		}
		...
		if ( ! is_array( $post_ids ) || empty( $post_ids ) ) {
			return array(
				'ok'   => true,
				'done' => true,
				'message' => __( 'ACF value crawl complete.', 'flowbie-wp' ),
			);
		}
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Crawled ACF values for %d post(s).', 'flowbie-wp' ),
				count( $post_ids )
			),
			'stats'   => array(
	...
	private function apply_structure( array $sheet, bool $dry ): array {
		...
		if ( empty( $items ) ) {
			if ( ! $dry ) {
				...
				if ( ! empty( $direct['ok'] ) ) {
					return array(
						'ok'      => true,
						'done'    => true,
						'message' => isset( $direct['message'] ) ? (string) $direct['message'] : __( 'ACF structure imported.', 'flowbie-wp' ),
			...
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'No ACF structure to import.', 'flowbie-wp' ),
		...
		if ( $dry ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'Dry run: would import %d ACF entities.', 'flowbie-wp' ),
					count( $items )
				),
	...
	private function apply_options_values( array $sheet, bool $dry ): array {
		...
		if ( empty( $pages ) ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'No ACF options page values to apply.', 'flowbie-wp' ),
			);
		}
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Applied ACF options values: %1$d field(s) updated, %2$d page(s) skipped.', 'flowbie-wp' ),
				$updated,
				$skipped
			),
			'stats'   => array(
	...
	private function apply_values_batch( array $sheet, int $offset, bool $dry ): array {
		...
		if ( empty( $batch ) ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'No ACF field values to apply.', 'flowbie-wp' ),
			);
		}
		...
		return array(
			...
			'message' => sprintf(
				/* translators: 1: updated count, 2: skipped count */
				__( 'Applied ACF post values: %1$d field(s) updated, %2$d post(s) skipped.', 'flowbie-wp' ),
				$updated,
				$skipped
			),
			'stats'   => array(
