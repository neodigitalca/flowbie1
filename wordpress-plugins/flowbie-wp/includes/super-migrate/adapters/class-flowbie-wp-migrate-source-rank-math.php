<?php
/**
 * Rank Math → Flowbie Redirects + SEO meta adapter.
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Migrate_Source_Rank_Math implements Flowbie_Wp_Migrate_Adapter {
	...
	public function get_label(): string {
		return __( 'Rank Math SEO', 'flowbie-wp' );
	}
		...
		if ( 'crawl' === $phase ) {
			...
			if ( self::redirections_table_exists() ) {
				...
				$steps[] = array(
					...
					'label' => __( 'Crawl Rank Math redirects', 'flowbie-wp' ),
					'total' => max( 1, (int) ceil( max( 1, $count ) / self::BATCH_REDIRECTS ) ),
				);
			}
			$steps[] = array(
				'id'    => 'rank_math_crawl_meta',
				'label' => __( 'Crawl Rank Math post meta', 'flowbie-wp' ),
		...
		if ( 'apply' === $phase ) {
			...
			$steps            = array(
				array(
					...
					'label' => __( 'Import redirects into Flowbie', 'flowbie-wp' ),
					'total' => $redirect_batches,
				),
				array(
					'id'    => 'rank_math_apply_meta',
					'label' => __( 'Apply Rank Math SEO meta to posts', 'flowbie-wp' ),
					'total' => $meta_batches,
				),
			);
			return $steps;
	...
	public function run_step( string $step_id, string $phase, array &$sheet, array $context ): array {
		...
		return array(
			'ok'    => false,
			'error' => __( 'Unknown Rank Math import step.', 'flowbie-wp' ),
		);
	}
	...
	private function crawl_redirects_batch( array &$sheet, int $offset ): array {
		if ( ! self::redirections_table_exists() ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'Rank Math redirections table not found.', 'flowbie-wp' ),
			);
		}
		...
		if ( ! is_array( $rows ) || empty( $rows ) ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'Rank Math redirect crawl complete.', 'flowbie-wp' ),
			);
		}
		...
		foreach ( $rows as $row ) {
			...
			$sheet['sheets']['redirects'][] = array(
				...
				'matching'    => 'exact',
				'category'    => __( 'Rank Math', 'flowbie-wp' ),
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Crawled %d Rank Math redirect(s).', 'flowbie-wp' ),
				count( $rows )
			),
	...
	private function crawl_meta_batch( array &$sheet, int $offset ): array {
		...
		if ( ! is_array( $rows ) || empty( $rows ) ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'Rank Math meta crawl complete.', 'flowbie-wp' ),
			);
		}
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Crawled %d Rank Math meta row(s).', 'flowbie-wp' ),
				count( $rows )
			),
			'stats'   => array( 'processed' => count( $rows ) ),
		);
	...
	private function apply_redirects_batch( array $sheet, int $offset, bool $dry ): array {
			...
			if ( ! empty( $direct['ok'] ) && ! empty( $direct['stats'] ) ) {
				...
				return array(
					...
					'message' => sprintf(
						...
						__( 'Imported Rank Math redirects: %1$d added, %2$d updated, %3$d skipped.', 'flowbie-wp' ),
						(int) ( $stats['added'] ?? 0 ),
						(int) ( $stats['updated'] ?? 0 ),
						(int) ( $stats['skipped'] ?? 0 )
					),
		...
		if ( empty( $batch ) ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'No redirects to apply.', 'flowbie-wp' ),
			);
		}
		...
		foreach ( $batch as $row ) {
			...
			$source = isset( $row['source'] ) ? Flowbie_Wp_Redirects_Csv::normalize_relative_path( (string) $row['source'] ) : null;
			if ( ! $source ) {
				++$skipped;
				continue;
			}
			if ( Flowbie_Wp_Redirects::get_by_source( $source ) ) {
				++$skipped;
				continue;
			}
			if ( $dry ) {
			...
			$result = Flowbie_Wp_Redirects::save( $row );
			if ( ! empty( $result['ok'] ) ) {
				++$added;
			} else {
				++$errors;
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Redirects: %1$d added, %2$d skipped, %3$d errors.', 'flowbie-wp' ),
				$added,
				$skipped,
				$errors
			),
	...
	private function apply_meta_batch( array $sheet, int $offset, bool $dry ): array {
		...
		if ( empty( $batch ) ) {
			return array(
				'ok'      => true,
				'done'    => true,
				'message' => __( 'No Rank Math meta to apply.', 'flowbie-wp' ),
			);
		}
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'SEO meta: %1$d updated, %2$d skipped.', 'flowbie-wp' ),
				$updated,
				$skipped
			),
			'stats'   => array(
		...
		if ( is_array( $data ) ) {
			foreach ( $data as $item ) {
				if ( is_array( $item ) && ! empty( $item['pattern'] ) ) {
					return Flowbie_Wp_Redirects_Csv::normalize_relative_path( (string) $item['pattern'] );
				}
			}
		}
		if ( is_string( $data ) ) {
			return Flowbie_Wp_Redirects_Csv::normalize_relative_path( $data );
		}
		return Flowbie_Wp_Redirects_Csv::normalize_relative_path( $sources_raw );
	}
	...
	/**
	 * One-click import of all active Rank Math redirects into Flowbie Redirects.
	 *
	...
	public static function import_redirects_from_database(): array {
		if ( ! self::is_rank_math_present() ) {
			return array(
				'ok'    => false,
				'error' => __( 'Rank Math redirections table not found. Is Rank Math SEO installed?', 'flowbie-wp' ),
			);
		}
		...
		if ( empty( $rows ) ) {
			return array(
				...
				'error' => self::is_rank_math_plugin_active()
					? __( 'No active Rank Math redirects found to import.', 'flowbie-wp' )
					: __( 'No Rank Math redirects found in the database.', 'flowbie-wp' ),
			);
		}

		require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/class-flowbie-wp-redirects.php';
		$stats = Flowbie_Wp_Redirects::merge_import( $rows );
		...
		return array(
			...
			'stats'    => $stats,
			'imported' => count( $rows ),
			'error'    => $ok ? '' : __( 'Rank Math redirects could not be imported.', 'flowbie-wp' ),
		);
	}
	...
	public static function import_all_from_database(): array {
		if ( ! self::is_rank_math_present() ) {
			return array(
				'ok'    => false,
				'error' => __( 'Rank Math data not found in the database.', 'flowbie-wp' ),
			);
		}
		...
		return array(
			...
			'message'  => sprintf(
				...
				__( 'Rank Math import: %1$d redirect(s) added, %2$d post meta updated.', 'flowbie-wp' ),
				(int) ( $stats['added'] ?? 0 ),
				(int) ( $stats['meta_updated'] ?? 0 )
			),
			'error'    => $redirect_ok ? '' : (string) ( $redirect_result['error'] ?? __( 'Rank Math import found nothing to migrate.', 'flowbie-wp' ) ),
		);
	}
}
