...
class Flowbie_Wp_Speed {
	...
	public static function init(): void {
		...
		add_action( 'wp_footer', array( 'Flowbie_Wp_Speed_Diagnostics', 'maybe_print_footer_marker' ), 9999 );
	...
	public static function is_active(): bool {
		return Flowbie_Wp_Speed_Settings::is_enabled();
	}

	public static function maybe_migrate_speed_settings(): void {
		Flowbie_Wp_Speed_Settings::maybe_migrate_elementor_safe_defaults();
		Flowbie_Wp_Speed_Settings::maybe_migrate_elementor_safe_defaults_v2();
		Flowbie_Wp_Speed_Settings::maybe_repair_simple_enabled_config();
		Flowbie_Wp_Speed_Settings::maybe_repair_simple_enabled_config_v2();
		Flowbie_Wp_Speed_Settings::maybe_repair_simple_enabled_config_v3();
		Flowbie_Wp_Speed_Settings::maybe_enable_speed_by_default();
	...
	public static function maybe_start_buffer(): void {
		Flowbie_Wp_Speed_Buffer::maybe_start();
	}

	/**
	 * Long-cache headers for minified assets served from wp-content/cache/flowbie-speed/.
	 */
	public static function maybe_send_speed_cache_headers(): void {
		...
		if ( $uri === '' || strpos( $uri, '/cache/flowbie-speed/' ) === false ) {
			return;
		}
		if ( ! preg_match( '#/cache/flowbie-speed/(css|js)/[a-f0-9]+\.(css|js)$#i', $uri ) ) {
			return;
		}
	...
	public static function rest_flush( WP_REST_Request $request ): WP_REST_Response {
		...
		return new WP_REST_Response(
			array(
				'ok'      => true,
				'message' => __( 'WordPress + Flowbie caches flushed.', 'flowbie-wp' ),
				'cache'   => Flowbie_Wp_Speed_Cache::stats(),
				'summary' => $summary,
			),
			200
		);
	...
	public static function tool_status_payload(): array {
		$res = self::rest_status( new WP_REST_Request( 'GET', '/flowbie/v1/speed/status' ) );
		return $res->get_data();
	}
	...
	public static function tool_flush_payload(): array {
		$res = self::rest_flush( new WP_REST_Request( 'POST', '/flowbie/v1/speed/flush' ) );
		return $res->get_data();
	}
}
