...
class Flowbie_Wp_Chat_Suggestion_Templates {
	...
	public static function build_card(
		...
		$source = trim( Flowbie_Wp_Chat_Page_Summary::read_markdown( $post_id ) );
		if ( $source === '' ) {
			return null;
		}

		$card = Flowbie_Wp_Chat_Agents::format_template_card(
		...
		$item = Flowbie_Wp_Chat_Page_Context::index_item_for_post( $post_id, $site_index );
		if ( ! is_array( $item ) ) {
			$item = array(
				'id'    => $post_id,
				'title' => $title,
		...
		$link_items = Flowbie_Wp_Chat_Links::items_for_template_attach(
			$source,
			(string) $card['body'],
			$title,
			$site_index,
		...
		$card['body'] = Flowbie_Wp_Chat_Links::finalize_template_body_links(
			(string) $card['body'],
			$source,
			$link_items,
			$site_index
