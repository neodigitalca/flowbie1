...
/**
 * Registers POST /flowbie/v1/overview-seo/bulk
 */
class Flowbie_Wp_Overview_Seo_Bulk {
	...
	public static function permission_check( $request ) {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			return new \WP_Error(
				'rest_forbidden',
				__( 'You do not have permission to edit posts.', 'flowbie-wp' ),
				array( 'status' => 403 )
			);
		}
		return true;
	...
	public static function bulk_import( $request ) {
		...
		if ( $items === array() ) {
			return new \WP_Error(
				'flowbie_empty',
				__( 'items array is empty.', 'flowbie-wp' ),
				array( 'status' => 400 )
			);
		}
	...
	private static function apply_item( int $post_id, array $raw ) {
		...
		if ( isset( $meta['rank_math_title'] ) && is_string( $meta['rank_math_title'] ) ) {
			...
			if ( $title !== '' ) {
				Flowbie_Wp_Ai_Apply::write_title( $post_id, Flowbie_Wp_Ai_Seo_Limits::normalize_title( $title ) );
				$did = true;
			}
		}
		...
		if ( isset( $meta['rank_math_description'] ) && is_string( $meta['rank_math_description'] ) ) {
			...
			if ( $desc !== '' ) {
				...
				Flowbie_Wp_Ai_Apply::write_excerpt(
					$post_id,
					Flowbie_Wp_Ai_Seo_Limits::normalize_description( $desc, $focus )
				);
				$did = true;
			}
		}
		...
		if ( isset( $meta['rank_math_focus_keyword'] ) && is_string( $meta['rank_math_focus_keyword'] ) ) {
			...
			if ( $kw !== '' ) {
				Flowbie_Wp_Ai_Apply::write_focus_keyword( $post_id, $kw );
				$did = true;
			}
		}
		...
		foreach ( $acf as $key => $value ) {
			...
			foreach ( $targets as $target ) {
				Flowbie_Wp_Ai_Apply::write_acf_or_meta( $post_id, $target, $text );
			}
			$did = true;
		}

		if ( ! $did ) {
			return new \WP_Error( 'flowbie_empty', __( 'Nothing to update (empty meta and acf).', 'flowbie-wp' ) );
		}
