 ...
 */
class Flowbie_Wp_Speed_Image_Optimizer {

	const SIDECAR_SUFFIX = '.flowbie-opt';
	...
	public static function optimize_attachment( int $attachment_id, bool $force = false ) {
		...
		if ( empty( $config['enabled'] ) ) {
			return new WP_Error( 'flowbie_speed_image_disabled', __( 'Image optimization is disabled.', 'flowbie-wp' ) );
		}

		if ( $attachment_id < 1 || get_post_type( $attachment_id ) !== 'attachment' ) {
			return new WP_Error( 'flowbie_speed_image_invalid', __( 'Invalid attachment.', 'flowbie-wp' ) );
		}

		if ( ! $force && get_post_meta( $attachment_id, Flowbie_Wp_Speed_Image_Settings::VERSION_META, true ) ) {
			return array(
				'ok'      => true,
				'skipped' => true,
		...
		if ( ! is_string( $file ) || $file === '' || ! is_readable( $file ) ) {
			return new WP_Error( 'flowbie_speed_image_missing', __( 'Attachment file not found.', 'flowbie-wp' ) );
		}
		...
		if ( empty( $sizes_meta ) ) {
			return new WP_Error( 'flowbie_speed_image_failed', __( 'Could not optimize image.', 'flowbie-wp' ) );
		}

		$payload = array(
			'version'      => Flowbie_Wp_Speed_Image_Settings::META_VERSION,
			'sizes'        => $sizes_meta,
			'optimized_at' => time(),
		);

		update_post_meta( $attachment_id, Flowbie_Wp_Speed_Image_Settings::META_KEY, $payload );
		update_post_meta( $attachment_id, Flowbie_Wp_Speed_Image_Settings::VERSION_META, Flowbie_Wp_Speed_Image_Settings::META_VERSION );

		Flowbie_Wp_Speed_Image_Stats::record( 1, $bytes_saved_total, $webp_count );
	...
	public static function is_supported_mime( string $mime, array $config ): bool {
		...
		if ( in_array( $mime, Flowbie_Wp_Speed_Image_Settings::skip_mime_list(), true ) ) {
			return false;
		}
	...
	private static function optimize_file( string $path, string $mime, array $config ) {
		...
		$webp_path = null;
		if ( ! empty( $config['generate_webp'] ) && Flowbie_Wp_Speed_Image_Settings::supports_webp_editor() ) {
	...
	public static function get_meta( int $attachment_id ): ?array {
		$raw = get_post_meta( $attachment_id, Flowbie_Wp_Speed_Image_Settings::META_KEY, true );
	...
	public static function flush_all_meta(): void {
		...
		$wpdb->delete( $wpdb->postmeta, array( 'meta_key' => Flowbie_Wp_Speed_Image_Settings::META_KEY ), array( '%s' ) );
		$wpdb->delete( $wpdb->postmeta, array( 'meta_key' => Flowbie_Wp_Speed_Image_Settings::VERSION_META ), array( '%s' ) );

		Flowbie_Wp_Speed_Image_Stats::reset();
	}
	...
	private static function pending_attachment_ids( int $per_page, int $page, bool $force ): array {
		...
		if ( ! $force ) {
			$args['meta_query'] = array(
				array(
					'key'     => Flowbie_Wp_Speed_Image_Settings::VERSION_META,
					'compare' => 'NOT EXISTS',
				),
			);
		}
