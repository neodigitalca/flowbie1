...
class Flowbie_Wp_Overseer_Tasks {
	...
	public static function get( int $id ) {
		...
		$table = Flowbie_Wp_Overseer::tasks_table_name();
	...
	public static function save( array $data ) {
		...
		$task_uid = isset( $data['task_uid'] ) ? sanitize_text_field( (string) $data['task_uid'] ) : Flowbie_Wp_Overseer::new_uuid();
		if ( ! Flowbie_Wp_Overseer::is_valid_uuid( $task_uid ) ) {
			$task_uid = Flowbie_Wp_Overseer::new_uuid();
		}
		...
		if ( $title === '' ) {
			return array(
				'ok'    => false,
				'error' => __( 'Task title is required.', 'flowbie-wp' ),
			);
		}
		...
		if ( false === $inserted ) {
			return array(
				'ok'    => false,
				'error' => __( 'Could not save task.', 'flowbie-wp' ),
			);
		}
