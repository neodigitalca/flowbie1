...
class Flowbie_Wp_Chat_Starters {

	const CACHE_KEY  = 'flowbie_chat_starters_v1';
	...
	public static function generate( array $site_index, string $site_name, array $settings = array() ): array {
		if ( empty( $site_index ) ) {
			return new WP_Error( 'flowbie_chat_starters_empty', __( 'Site index is empty.', 'flowbie-wp' ) );
		}
...
PROMPT;
		...
		if ( null === $parsed || empty( $parsed['starters'] ) || ! is_array( $parsed['starters'] ) ) {
			return new WP_Error( 'flowbie_chat_starters_parse', __( 'Starter generation returned invalid JSON.', 'flowbie-wp' ) );
		}
	...
	private static function call_openrouter( string $system_prompt, string $user_prompt ) {
		...
		if ( $key === '' ) {
			return new WP_Error( 'flowbie_openrouter_key', __( 'OpenRouter API key is not configured.', 'flowbie-wp' ) );
		}
		...
		if ( $code < 200 || $code >= 300 || ! is_array( $data ) || empty( $data['choices'][0]['message']['content'] ) ) {
			return new WP_Error( 'flowbie_chat_starters_ai', __( 'Starter generation failed.', 'flowbie-wp' ) );
		}
