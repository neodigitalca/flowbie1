...
class Flowbie_Wp_Gmb_Rest {
	...
	/**
	 * POST /flowbie/v1/ai/social-preview
	 *
	...
	public static function social_preview( WP_REST_Request $request ): WP_REST_Response {
		...
		$data    = Flowbie_Wp_Gmb::get_post_preview_data( $post_id );
		if ( is_wp_error( $data ) ) {
		...
		return new WP_REST_Response(
			array(
				'ok'          => true,
				'post'        => $data,
				'platforms'   => Flowbie_Wp_Gmb::PLATFORM_CONSTRAINTS,
				'gmb_status'  => array(
	...
	/**
	 * POST /flowbie/v1/ai/social-generate
	 *
	...
	public static function social_generate( WP_REST_Request $request ): WP_REST_Response {
		...
		$valid_platforms = array_keys( Flowbie_Wp_Gmb::PLATFORM_CONSTRAINTS );
		if ( ! in_array( $platform, $valid_platforms, true ) ) {
			$platform = 'gmb';
		}

		$summary = Flowbie_Wp_Gmb::summarize_post( $post_id, $platform );
		if ( is_wp_error( $summary ) ) {
			return self::wp_error_response( $summary );
		}

		$constraints = Flowbie_Wp_Gmb::PLATFORM_CONSTRAINTS[ $platform ];
	...
	public static function create_social_post( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( ! Flowbie_Wp_Gmb::is_configured() ) {
			return self::error_response( 'Configure GMB Client ID and Secret in Flowbie WP Settings → GMB.', 503 );
		}

		if ( ! Flowbie_Wp_Gmb::is_connected() ) {
		...
		if ( $summary === '' ) {
			$summary = Flowbie_Wp_Gmb::summarize_post( $post_id );
			if ( is_wp_error( $summary ) ) {
				return self::wp_error_response( $summary );
			}
		}
		...
		$result = Flowbie_Wp_Gmb::create_local_post( $summary, $cta_url, $image_url );
