<?php
/**
 * REST routes for Flowbie AI editor wands.
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Ai_Rest {
	...
	public static function edit_post_permission( $request ) {
		...
		if ( $post_id < 1 || ! current_user_can( 'edit_post', $post_id ) ) {
			return new WP_Error( 'rest_forbidden', __( 'You do not have permission to edit this post.', 'flowbie-wp' ), array( 'status' => 403 ) );
		}
		return true;
	}
	...
	public static function status( $request ) {
		$post_id = (int) $request->get_param( 'post_id' );
		return new WP_REST_Response( Flowbie_Wp_Ai_Gate::get_status( $post_id ), 200 );
	}
	...
	public static function preview( $request ) {
		...
		if ( $field === 'faq' && $faq_step !== '' ) {
			$result = Flowbie_Wp_Ai_Enhance::preview_faq_step( $post_id, $faq_step, $overrides, is_array( $body ) ? $body : array() );
		} else {
	...
	public static function apply( $request ) {
		...
		if ( is_array( $client ) ) {
			$result['optimization'] = Flowbie_Wp_Site_Progress::optimization_usage_for_client( $client );
		}
	...
	public static function change_url( $request ) {
		...
		$result = Flowbie_Wp_Ai_Url::change_url( $post_id, $slug, $create );
		if ( is_wp_error( $result ) ) {
			return self::error_response( $result );
		}
		return new WP_REST_Response( $result, 200 );
