<?php
/**
 * Google Business Profile (GMB) client for Flowbie WP.
 *
...
class Flowbie_Wp_Gmb {

	const OPTION_TOKENS      = 'flowbie_wp_gmb_tokens';
	const OPTION_CLIENT_ID   = 'flowbie_wp_gmb_client_id';
	const OPTION_CLIENT_SEC  = 'flowbie_wp_gmb_client_secret';
	const OPTION_LOCATION_ID = 'flowbie_wp_gmb_location_id';
	const OPTION_ACCOUNT_ID  = 'flowbie_wp_gmb_account_id';
	...
	public static function get_client_id(): string {
		if ( defined( 'FLOWBIE_WP_GMB_CLIENT_ID' ) && FLOWBIE_WP_GMB_CLIENT_ID !== '' ) {
			return trim( (string) FLOWBIE_WP_GMB_CLIENT_ID );
		}
		return trim( (string) get_option( self::OPTION_CLIENT_ID, '' ) );
	}

	public static function get_client_secret(): string {
		if ( defined( 'FLOWBIE_WP_GMB_CLIENT_SECRET' ) && FLOWBIE_WP_GMB_CLIENT_SECRET !== '' ) {
			return trim( (string) FLOWBIE_WP_GMB_CLIENT_SECRET );
		}
		return trim( (string) get_option( self::OPTION_CLIENT_SEC, '' ) );
	}

	public static function get_location_id(): string {
		if ( defined( 'FLOWBIE_WP_GMB_LOCATION_ID' ) && FLOWBIE_WP_GMB_LOCATION_ID !== '' ) {
			return trim( (string) FLOWBIE_WP_GMB_LOCATION_ID );
		}
	...
	public static function get_valid_access_token() {
		...
		if ( ! is_array( $tokens ) || empty( $tokens['access_token'] ) ) {
			return new WP_Error( 'flowbie_gmb_not_connected', __( 'Google Business Profile is not connected. Use Connect in Settings.', 'flowbie-wp' ) );
		}
		...
		if ( empty( $tokens['refresh_token'] ) ) {
			return new WP_Error( 'flowbie_gmb_no_refresh', __( 'No refresh token. Re-connect Google Business Profile.', 'flowbie-wp' ) );
		}
	...
	public static function get_redirect_uri(): string {
		return rest_url( 'flowbie/v1/gmb/callback' );
	}

	public static function get_authorize_url(): string {
		$state = wp_create_nonce( 'flowbie_gmb_oauth' );
	...
	public static function handle_oauth_callback( $request ) {
		...
		if ( ! $code || ! wp_verify_nonce( $state, 'flowbie_gmb_oauth' ) ) {
	...
	private static function redirect_with_notice( string $type, string $message ) {
		$url = admin_url( 'admin.php?page=flowbie-wp-settings&tab=gmb&flowbie_gmb_notice=' . rawurlencode( $type . '|' . $message ) );
	...
	public static function resolve_account( string $access_token ) {
		...
		if ( ! is_array( $data ) || empty( $data['accounts'][0]['name'] ) ) {
			return new WP_Error( 'flowbie_gmb_no_account', __( 'No Google Business Profile accounts found.', 'flowbie-wp' ) );
		}
	...
	public static function summarize_post( int $post_id, string $platform = 'gmb' ) {
		...
		if ( ! $post instanceof WP_Post ) {
			return new WP_Error( 'flowbie_gmb_post', __( 'Post not found.', 'flowbie-wp' ) );
		}
		...
		$result     = Flowbie_Wp_OpenRouter::complete( $system, $user, $max_tokens, 0.6 );
	...
	public static function create_local_post( string $summary, string $cta_url, ?string $image_url = null ) {
		...
		if ( $location_id === '' ) {
			return new WP_Error( 'flowbie_gmb_no_location', __( 'GBP Location ID is not set. Configure it in Flowbie WP Settings → GMB.', 'flowbie-wp' ) );
		}
		...
		if ( $code === 429 ) {
			return new WP_Error( 'flowbie_gmb_quota', __( 'Google Business Profile quota exceeded. Wait a minute and try again.', 'flowbie-wp' ) );
		}
		...
		if ( $code < 200 || $code >= 300 ) {
			...
			return new WP_Error( 'flowbie_gmb_api', sprintf( 'GBP %d: %s', $code, $msg ) );
		}
