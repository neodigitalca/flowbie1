...
class Flowbie_Wp_Ai_Body_Rest {
	...
	public static function register_routes(): void {
		$edit = array(
			'permission_callback' => array( 'Flowbie_Wp_Ai_Rest', 'edit_post_permission' ),
		);
	...
	public static function sections_from_post( $request ) {
		...
		if ( ! $post instanceof WP_Post ) {
			return self::respond( new WP_Error( 'flowbie_post', 'Post not found.' ) );
		}
		$split    = Flowbie_Wp_Content_Sections::split_html_by_h2( $post->post_content );
		$sections = array();
	...
	public static function session( $request ) {
		$post_id = (int) $request->get_param( 'post_id' );
		return self::respond( Flowbie_Wp_Ai_Body::get_session( $post_id ) );
	}
	...
	public static function delete_session( $request ) {
		$post_id = (int) $request->get_param( 'post_id' );
		return self::respond( Flowbie_Wp_Ai_Body::clear_session( $post_id ) );
	}
	...
	public static function section_preview( $request ) {
		...
		return self::respond( Flowbie_Wp_Ai_Body::preview_section( $post_id, $index, $sid ) );
	}
	...
	public static function section_apply( $request ) {
		...
		return self::respond( Flowbie_Wp_Ai_Body::apply_section( $post_id, $index, $sid, $html, false, $section_title ) );
	}
	...
	public static function save_structure( $request ) {
		...
		$current = Flowbie_Wp_Content_Sections::split_html_by_h2( $post->post_content );
		...
		Flowbie_Wp_Ai_Body::clear_session( $post_id );

		$new_sections = Flowbie_Wp_Content_Sections::split_html_by_h2( $new_content );
		$out          = array();
	...
	public static function insert_element( $request ) {
		...
		return self::respond( Flowbie_Wp_Ai_Body::generate_element( $post_id, $section_title, $section_html, $element_type, $custom_prompt ) );
	}
	...
	public static function posts_inventory( $request ) {
		$post_id = (int) $request->get_param( 'post_id' );
		return self::respond( Flowbie_Wp_Ai_Body::get_posts_inventory( $post_id ) );
	}
	...
	public static function suggest_link( $request ) {
		...
		return self::respond( Flowbie_Wp_Ai_Body::suggest_link( $post_id, $text, $context ) );
	}
