...
class Flowbie_Wp_Seo_Blocks_Library {

	const META_BLOCK_ID = '_flowbie_seo_block_id';
	...
	public static function sync_row( array $row ) {
		if ( ! self::elementor_available() ) {
			return new WP_Error( 'flowbie_elementor_missing', __( 'Elementor is required for library sync.', 'flowbie-wp' ) );
		}
		...
		if ( taxonomy_exists( 'elementor_library_category' ) ) {
			if ( ! term_exists( 'flowbie-seo', 'elementor_library_category' ) ) {
				wp_insert_term( 'Flowbie SEO', 'elementor_library_category', array( 'slug' => 'flowbie-seo' ) );
			}
			wp_set_object_terms( $post_id, array( 'flowbie-seo' ), 'elementor_library_category', false );
		}
	...
	public static function row_to_widget_settings( array $row ): array {
		...
		$slots = Flowbie_Wp_Seo_Blocks_Slots::add_elementor_ids(
			Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $row['slots'] ?? array() )
		);
		$block_id = (string) (int) ( $row['id'] ?? 0 );
		...
		return array(
			...
			'layout_config_json' => Flowbie_Wp_Seo_Blocks_Elementor_Widget::layout_to_json( $row['layout_config'] ?? array() ),
		);
	}
					...
					array(
						...
						'elements' => array(
							array(
								'id'         => $widget_id,
								'elType'     => 'widget',
								'widgetType' => 'flowbie_seo_section',
								'settings'   => $settings,
								'elements'   => array(),
							),
						),
