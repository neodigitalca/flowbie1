...
class Flowbie_Wp_Seo_Blocks_Mutation {
	...
	public static function add_slot( array $block, array $slot, ?array $placement = null ) {
		...
		$normalized = Flowbie_Wp_Seo_Blocks_Slots::normalize_slot( $slot );
		if ( ! $normalized ) {
			return new WP_Error( 'flowbie_seo_slot_invalid', __( 'Invalid slot type or content.', 'flowbie-wp' ) );
		}
		...
		$block['layout_config'] = Flowbie_Wp_Seo_Blocks_Layout::normalize_config( $layout, $slots );
	...
	public static function remove_slot( array $block, $target ) {
		...
		if ( $index === null ) {
			return new WP_Error( 'flowbie_seo_slot_missing', __( 'Could not find the slot to remove.', 'flowbie-wp' ) );
		}
	...
	public static function update_slot( array $block, $target, array $patch ) {
		$index = self::resolve_slot_index( $block, $target );
		if ( $index === null ) {
			return new WP_Error( 'flowbie_seo_slot_missing', __( 'Could not find the slot to update.', 'flowbie-wp' ) );
		}
		...
		$normalized = Flowbie_Wp_Seo_Blocks_Slots::normalize_slot( $merged );
		if ( ! $normalized ) {
			return new WP_Error( 'flowbie_seo_slot_invalid', __( 'Invalid slot update.', 'flowbie-wp' ) );
		}
		if ( ! empty( $slots[ $index ]['_id'] ) ) {
	...
	private static function place_slot_in_layout( array $block, string $slot_id, int $row, int $col ): array {
		...
		$grid     = isset( $layout['grid'] ) && is_array( $layout['grid'] ) ? $layout['grid'] : Flowbie_Wp_Seo_Blocks_Layout::default_grid();
		...
		foreach ( $sections as $i => $section ) {
			...
			if ( (int) ( $section['row'] ?? -1 ) === $row && (int) ( $section['col'] ?? -1 ) === $col ) {
				...
				$block['layout_config']     = Flowbie_Wp_Seo_Blocks_Layout::normalize_config( $layout, $block['slots'] ?? array() );
				return $block;
			}
		}
		...
		$sections[] = array(
			...
			'col_span' => 1,
			'align_h'  => 'left',
			'width'    => Flowbie_Wp_Seo_Blocks_Layout::width_for_cols( (int) ( $grid['cols'] ?? 3 ) ),
