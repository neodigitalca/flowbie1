...
class Flowbie_Wp_Seo_Blocks_Rest {
	...
	public static function manage_permission(): bool {
		return current_user_can( 'manage_options' ) || current_user_can( Flowbie_Wp_Admin::required_capability() );
	}
	...
	public static function get_block( WP_REST_Request $request ) {
		...
		if ( ! is_array( $row ) ) {
			return new WP_Error( 'flowbie_seo_block_missing', __( 'SEO block not found.', 'flowbie-wp' ), array( 'status' => 404 ) );
		}
		return rest_ensure_response(
			array(
				'block' => Flowbie_Wp_Seo_Blocks_Storage::format_block_response( $row ),
			)
		);
	}
	...
	public static function create_draft( WP_REST_Request $request ) {
		unset( $request );
		$result = Flowbie_Wp_Seo_Blocks_Storage::create_draft();
	...
	public static function block_choices( WP_REST_Request $request ) {
		...
		return rest_ensure_response(
			array(
				'choices' => Flowbie_Wp_Seo_Blocks_Storage::get_registry_choices(),
				'blocks'  => Flowbie_Wp_Seo_Blocks_Storage::list_all(),
			)
		);
	}
	...
	public static function save_block( WP_REST_Request $request ) {
		...
		$result = Flowbie_Wp_Seo_Blocks_Storage::save( is_array( $params ) ? $params : array() );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
		return rest_ensure_response( array( 'block' => $result ) );
	...
	public static function delete_block( WP_REST_Request $request ) {
		$id = absint( $request->get_param( 'id' ) );
		$result = Flowbie_Wp_Seo_Blocks_Storage::delete( $id, ! empty( $request->get_param( 'trash_library' ) ) );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
	...
	public static function sync_library( WP_REST_Request $request ) {
		...
		return rest_ensure_response( array( 'block' => Flowbie_Wp_Seo_Blocks_Storage::get( $id ) ) );
	}
	...
	public static function duplicate_block( WP_REST_Request $request ) {
		$id     = absint( $request->get_param( 'id' ) );
		$result = Flowbie_Wp_Seo_Blocks_Storage::duplicate_block( $id );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
		return rest_ensure_response( array( 'block' => $result ) );
	...
	public static function page_search( WP_REST_Request $request ) {
		...
		$types    = Flowbie_Wp_Seo_Blocks_Storage::allowed_primary_post_types();
		...
		foreach ( $query->posts as $post ) {
			...
			$items[] = Flowbie_Wp_Seo_Blocks_Storage::primary_post_summary( (int) $post->ID );
		}
	...
	public static function bulk_optimize( WP_REST_Request $request ) {
		$params = $request->get_json_params();
		if ( ! is_array( $params ) ) {
			return new WP_Error( 'flowbie_seo_block_bulk', __( 'Invalid bulk payload.', 'flowbie-wp' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $ids as $id ) {
			...
			$preview = Flowbie_Wp_Seo_Blocks_Optimizer::preview(
				array(
					'post_id'     => Flowbie_Wp_Seo_Blocks_Context::resolve_primary_post_id( 0, $id, $row ),
					'block_id'    => $id,
					'mode'        => $mode,
			...
			if ( $apply ) {
				$resolved_post = Flowbie_Wp_Seo_Blocks_Context::resolve_primary_post_id( 0, $id, $row );
	...
	public static function preview_block( WP_REST_Request $request ) {
		$params = $request->get_json_params();
		if ( ! is_array( $params ) ) {
			return new WP_Error( 'flowbie_seo_block_preview', __( 'Invalid preview payload.', 'flowbie-wp' ), array( 'status' => 400 ) );
		}

		$slots = isset( $params['slots'] ) && is_array( $params['slots'] ) ? Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $params['slots'] ) : array();
		$slots = Flowbie_Wp_Seo_Blocks_Slots::add_elementor_ids( $slots );
		$layout = isset( $params['layout_config'] ) && is_array( $params['layout_config'] )
			? Flowbie_Wp_Seo_Blocks_Layout::normalize_config( $params['layout_config'], $slots )
			: Flowbie_Wp_Seo_Blocks_Layout::normalize_config( array(), $slots );

		$html = Flowbie_Wp_Seo_Blocks_Slots::render_html( $slots, $layout );

		return rest_ensure_response(
			array(
				'html'          => $html,
				'slot_summary'  => Flowbie_Wp_Seo_Blocks_Slots::summary( $slots ),
	...
	public static function preview( WP_REST_Request $request ) {
		...
		$result = Flowbie_Wp_Seo_Blocks_Optimizer::preview( $params );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
	...
	public static function apply( WP_REST_Request $request ) {
		$params = $request->get_json_params();
		if ( ! is_array( $params ) ) {
			return new WP_Error( 'flowbie_seo_block_apply', __( 'Invalid apply payload.', 'flowbie-wp' ), array( 'status' => 400 ) );
		}
		$result = Flowbie_Wp_Seo_Blocks_Optimizer::apply( $params );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
		return rest_ensure_response( $result );
