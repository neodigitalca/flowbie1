...
class Flowbie_Wp_Seo_Blocks_Optimizer {
	...
	public static function preview( array $params ) {
		...
		$row = $block_id > 0 ? Flowbie_Wp_Seo_Blocks_Storage::get( $block_id ) : null;

		$post_id = Flowbie_Wp_Seo_Blocks_Context::resolve_primary_post_id(
			$post_id,
			$block_id,
		...
		if ( $post_id > 0 ) {
			if ( $registry_optimize ) {
				$valid = Flowbie_Wp_Seo_Blocks_Storage::validate_primary_post( $post_id );
				if ( is_wp_error( $valid ) ) {
					return $valid;
				}
			} elseif ( ! Flowbie_Wp_Ai_Gate::can_preview( $post_id ) ) {
				return new WP_Error( 'flowbie_seo_block_gate', __( 'AI optimization is not available for this post.', 'flowbie-wp' ) );
			}
		}

		$page_context_override = sanitize_textarea_field( (string) ( $params['page_context'] ?? '' ) );

		$ctx = Flowbie_Wp_Seo_Blocks_Context::build(
			array(
				'post_id'    => $post_id,
		...
		$slots   = $slots_in ? Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $slots_in ) : ( $current['slots'] ?? array() );
		...
		if ( is_array( $row ) && ! empty( $row['layout_config'] ) && is_array( $row['layout_config'] ) ) {
			...
			$layout_config = Flowbie_Wp_Seo_Blocks_Layout::normalize_config( $params['layout_config'], $slots );
		} else {
			$layout_config = Flowbie_Wp_Seo_Blocks_Layout::normalize_config( array(), $slots );
		}

		if ( empty( $slots ) ) {
			return new WP_Error( 'flowbie_seo_block_empty', __( 'No content slots to optimize.', 'flowbie-wp' ) );
		}
		...
		if ( $topic_focus === '' ) {
			return new WP_Error( 'flowbie_seo_block_topic', __( 'Topic focus or focus keyword is required before optimizing.', 'flowbie-wp' ) );
		}
		...
		$result = Flowbie_Wp_OpenRouter::complete_chat( $system, $user, 4096, $mode === 'intent' ? 0.4 : 0.7 );
	...
	public static function apply( array $params ) {
		...
		$block_id      = absint( $params['block_id'] ?? 0 );
		$element_id    = sanitize_text_field( (string) ( $params['element_id'] ?? '' ) );
		$preview_slots = isset( $params['preview_slots'] ) && is_array( $params['preview_slots'] ) ? Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $params['preview_slots'] ) : array();
		...
		if ( empty( $preview_slots ) ) {
			return new WP_Error( 'flowbie_seo_block_apply', __( 'Preview slots are required.', 'flowbie-wp' ) );
		}

		if ( $post_id > 0 && $element_id !== '' ) {
			$updated = Flowbie_Wp_Seo_Blocks_Sync::apply_to_page_element(
				$post_id,
				$element_id,
				$preview_slots,
				array(
		...
		if ( $block_id > 0 ) {
			...
			if ( is_array( $row ) ) {
				$original = Flowbie_Wp_Seo_Blocks_Slots::add_elementor_ids( $row['slots'] ?? array() );
				...
				Flowbie_Wp_Seo_Blocks_Storage::update_slots( $block_id, $preview_slots, true );
				if ( $topic_focus !== '' || $focus_keyword !== '' ) {
					global $wpdb;
					$table  = Flowbie_Wp_Seo_Blocks_Storage::table_name();
	...
	private static function user_prompt( string $mode, array $ctx, array $slots, array $layout_config, array $block_meta, string $topic_focus, string $focus_keyword, string $page_context_override = '' ): string {
		$context_block = $page_context_override !== ''
			...
			: Flowbie_Wp_Seo_Blocks_Context::format_for_prompt( $ctx );
	...
	private static function merge_copy_onto_slots( array $original_slots, array $ai_slots ): array {
		$original_slots = Flowbie_Wp_Seo_Blocks_Slots::add_elementor_ids( $original_slots );
		$ai_by_id       = array();
		...
		return Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $merged );
	}
	...
	private static function parse_slots_json( string $content ) {
		...
		if ( ! is_array( $decoded ) ) {
			return new WP_Error( 'flowbie_seo_block_json', __( 'AI returned invalid slot JSON.', 'flowbie-wp' ) );
		}
		...
		$normalized = Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $decoded );
		if ( empty( $normalized ) ) {
			return new WP_Error( 'flowbie_seo_block_json', __( 'AI returned empty slots.', 'flowbie-wp' ) );
		}
