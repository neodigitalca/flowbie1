...
class Flowbie_Wp_Seo_Blocks_Page_Insert {
	...
	/**
	 * Insert a registry-linked flowbie_seo_section widget on a page.
	 *
	...
	public static function insert_registry_widget( int $post_id, int $block_id, array $opts = array() ) {
		if ( $post_id < 1 || $block_id < 1 ) {
			return new WP_Error( 'flowbie_seo_page_insert', __( 'Missing post or block ID.', 'flowbie-wp' ) );
		}

		if ( ! Flowbie_Wp_Seo_Blocks_Library::elementor_available() ) {
			return new WP_Error( 'flowbie_elementor_missing', __( 'Elementor is required to apply SEO blocks to pages.', 'flowbie-wp' ) );
		}
		...
		$saved                  = Flowbie_Wp_Seo_Blocks_Storage::save( $row );
		...
		if ( $include_dynamic_heading ) {
			...
			if ( is_array( $heading_section ) ) {
				...
				$heading_note = __( 'Dynamic heading skipped (Elementor Pro dynamic tags unavailable).', 'flowbie-wp' );
			}
		}

		$widget_settings = self::registry_widget_settings( $row );
		$block_section   = Flowbie_Wp_Seo_Blocks_Library::build_section_elements( $widget_settings );
		...
		if ( $sync_library ) {
			$synced = Flowbie_Wp_Seo_Blocks_Library::sync_row( $row );
		...
		$summary = sprintf(
			...
			__( 'Applied SEO block "%1$s" to page "%2$s" with registry link.', 'flowbie-wp' ),
		...
		return array(
			...
			'block_edit_url'    => admin_url( 'admin.php?page=flowbie-wp-agent-hub-edit&block_id=' . $block_id ),
	...
	public static function registry_widget_settings( array $row ): array {
		...
		if ( ! class_exists( 'Flowbie_Wp_Seo_Blocks_Elementor_Widget', false ) ) {
		...
		return array(
			...
			'content_slots'      => array(),
			'layout_config_json' => Flowbie_Wp_Seo_Blocks_Elementor_Widget::layout_to_json(
	...
	private static function build_dynamic_heading_section( int $post_id ): ?array {
		if ( ! class_exists( 'Flowbie_Wp_Fields_Elementor', false ) ) {
			require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/fields/integrations/class-flowbie-wp-fields-elementor.php';
		}

		$tag = Flowbie_Wp_Fields_Elementor::build_post_meta_text_tag( '_flowbie_focus_keyword' );
		if ( $tag === '' ) {
			return null;
		}

		$section_id = Flowbie_Wp_Seo_Blocks_Library::generate_element_id();
		$column_id  = Flowbie_Wp_Seo_Blocks_Library::generate_element_id();
		$widget_id  = Flowbie_Wp_Seo_Blocks_Library::generate_element_id();
				...
				array(
					...
					'elements' => array(
						array(
							...
							'widgetType' => 'heading',
							'settings'   => array(
								'title'           => get_post_meta( $post_id, '_flowbie_focus_keyword', true ) ?: get_the_title( $post_id ),
								'header_size'     => 'h1',
								'__dynamic__'     => array(
									'title' => $tag,
								),
