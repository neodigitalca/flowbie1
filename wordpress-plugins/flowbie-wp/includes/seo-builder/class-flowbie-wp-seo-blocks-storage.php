...
class Flowbie_Wp_Seo_Blocks_Storage {
	...
	public static function table_name(): string {
		global $wpdb;
		return $wpdb->prefix . 'flowbie_wp_seo_blocks';
	}
	...
	public static function maybe_install(): void {
		if ( get_option( 'flowbie_wp_seo_blocks_db_version', '' ) !== self::TABLE_VERSION ) {
			self::install();
		}
	}
	...
	public static function install(): void {
		...
		update_option( 'flowbie_wp_seo_blocks_db_version', self::TABLE_VERSION, false );
	}
	...
	public static function save( array $input ) {
		...
		if ( $title === '' ) {
			return new WP_Error( 'flowbie_seo_block_title', __( 'Theme / title is required.', 'flowbie-wp' ) );
		}
		...
		$slots = Flowbie_Wp_Seo_Blocks_Slots::slots_from_row( $input, $existing_slots );
		if ( empty( $slots ) && ! empty( $input['h2'] ) ) {
			$slots = Flowbie_Wp_Seo_Blocks_Slots::slots_from_row( $input, null );
		}
		$slots = Flowbie_Wp_Seo_Blocks_Slots::add_elementor_ids( $slots );

		$layout_raw = $input['layout_config'] ?? null;
		if ( is_array( $layout_raw ) ) {
			$layout_config = Flowbie_Wp_Seo_Blocks_Layout::normalize_config( $layout_raw, $slots );
		} elseif ( is_array( $existing_layout ) ) {
			$layout_config = Flowbie_Wp_Seo_Blocks_Layout::normalize_config( $existing_layout, $slots );
		} else {
			$layout_config = Flowbie_Wp_Seo_Blocks_Layout::normalize_config( array(), $slots );
		}
		...
		$data  = array(
			...
			'slots'                => wp_json_encode( Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $slots ) ),
		...
		if ( $id > 0 ) {
			...
			if ( false === $updated ) {
				return new WP_Error( 'flowbie_seo_block_save', __( 'Could not update SEO block.', 'flowbie-wp' ) );
			}
			$row = self::get( $id );
		} else {
			$data['created_at'] = $now;
			...
			if ( ! $inserted ) {
				return new WP_Error( 'flowbie_seo_block_save', __( 'Could not create SEO block.', 'flowbie-wp' ) );
			}
			$row = self::get( (int) $wpdb->insert_id );
		}

		if ( ! is_array( $row ) ) {
			return new WP_Error( 'flowbie_seo_block_save', __( 'Could not load saved SEO block.', 'flowbie-wp' ) );
		}

		$sync = Flowbie_Wp_Seo_Blocks_Library::sync_row( $row );
		if ( is_wp_error( $sync ) ) {
	...
	public static function format_block_response( array $row ): array {
		$row['widget_settings'] = Flowbie_Wp_Seo_Blocks_Library::row_to_widget_settings( $row );
		return $row;
	}
	...
	public static function update_slots( int $id, array $slots, bool $mark_optimized = false ) {
		global $wpdb;
		if ( $id < 1 ) {
			return new WP_Error( 'flowbie_seo_block_id', __( 'Invalid block ID.', 'flowbie-wp' ) );
		}
		...
		if ( false === $ok ) {
			return new WP_Error( 'flowbie_seo_block_update', __( 'Could not update block slots.', 'flowbie-wp' ) );
		}

		$row = self::get( $id );
		if ( is_array( $row ) ) {
			Flowbie_Wp_Seo_Blocks_Library::sync_row( $row );
		}
	...
	public static function delete( int $id, bool $trash_library = false ) {
		...
		$row = self::get( $id );
		if ( ! is_array( $row ) ) {
			return new WP_Error( 'flowbie_seo_block_missing', __( 'SEO block not found.', 'flowbie-wp' ) );
		}
	...
	public static function decode_row( array $row ): array {
		...
		$row['slots']                 = Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $slots );
		$layout_raw = json_decode( (string) ( $row['layout_config'] ?? '{}' ), true );
		$row['layout_config']         = Flowbie_Wp_Seo_Blocks_Layout::normalize_config(
		...
		$row['slot_summary']          = Flowbie_Wp_Seo_Blocks_Slots::summary( $row['slots'] );
		$row['h2']                    = self::first_h2( $row['slots'] );
		$row['usage_count']           = Flowbie_Wp_Seo_Blocks_Usage::count_for_block( (int) $row['id'] );
	...
	public static function first_h2( array $slots ): string {
		foreach ( Flowbie_Wp_Seo_Blocks_Slots::normalize_list( $slots ) as $slot ) {
	...
	public static function get_registry_choices(): array {
		$choices = array(
			'' => __( '— Select Agent Hub block —', 'flowbie-wp' ),
		);
	...
	public static function format_choice_label( array $block ): string {
		...
		if ( $label === '' ) {
			$label = __( 'Untitled block', 'flowbie-wp' );
		}
	...
	public static function allowed_primary_post_types(): array {
		 ...
		 */
		return apply_filters( 'flowbie_wp_seo_block_primary_post_types', $types );
	}
	...
	public static function validate_primary_post( int $post_id ) {
		...
		if ( ! $post instanceof WP_Post ) {
			return new WP_Error( 'flowbie_seo_block_primary_post', __( 'Linked page not found.', 'flowbie-wp' ) );
		}
		if ( ! in_array( $post->post_type, self::allowed_primary_post_types(), true ) ) {
			return new WP_Error( 'flowbie_seo_block_primary_post', __( 'Linked page must be a page or post.', 'flowbie-wp' ) );
		}
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new WP_Error( 'flowbie_seo_block_primary_post', __( 'You cannot link this page.', 'flowbie-wp' ) );
		}
		return true;
	}
	...
	public static function primary_post_summary( int $post_id ): array {
		...
		return array(
			...
			'focus_keyword' => Flowbie_Wp_Ai_Context::read_focus_keyword( $post_id ),
		);
	}
	...
	public static function duplicate_block( int $id ) {
		...
		if ( $title === '' ) {
			$title = __( 'SEO block', 'flowbie-wp' );
		}
		$title .= ' ' . __( '(Copy)', 'flowbie-wp' );
