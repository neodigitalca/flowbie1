...
trait Flowbie_Wp_Admin_Trait_Handlers_Overseer {

	const ACTION_EXPORT_OVERSEER       = 'flowbie_wp_export_overseer';
	const ACTION_SAVE_OVERSEER_SETTINGS = 'flowbie_wp_save_overseer_settings';
	const ACTION_SAVE_OVERSEER_CONVERSION = 'flowbie_wp_save_overseer_conversion';
	const ACTION_DELETE_OVERSEER_CONVERSION = 'flowbie_wp_delete_overseer_conversion';
	const ACTION_DELETE_OVERSEER_VISIT = 'flowbie_wp_delete_overseer_visit';
	const ACTION_BULK_OVERSEER_VISITS  = 'flowbie_wp_bulk_overseer_visits';
	const ACTION_CLEAR_OVERSEER_VISITS = 'flowbie_wp_clear_overseer_visits';
	const ACTION_RUN_OVERSEER_ANALYSIS = 'flowbie_wp_run_overseer_analysis';
	const ACTION_DELETE_OVERSEER_REPORT = 'flowbie_wp_delete_overseer_report';
	const ACTION_APPROVE_OVERSEER_TASK  = 'flowbie_wp_approve_overseer_task';
	const ACTION_DISMISS_OVERSEER_TASK  = 'flowbie_wp_dismiss_overseer_task';
	const ACTION_DONE_OVERSEER_TASK     = 'flowbie_wp_done_overseer_task';
	...
	public static function handle_delete_overseer_visit(): void {
		...
		if ( $id > 0 ) {
			Flowbie_Wp_Overseer::delete_visit( $id );
		}
		...
		self::set_flash(
			array(
				'kind'    => 'overseer',
				'success' => true,
				'message' => __( 'Visit deleted.', 'flowbie-wp' ),
			)
		);
	...
	public static function handle_bulk_overseer_visits(): void {
		...
		if ( $action === 'delete' ) {
			foreach ( $ids as $visit_id ) {
				if ( $visit_id > 0 && Flowbie_Wp_Overseer::delete_visit( $visit_id ) ) {
					++$deleted;
				}
			}
		}
			...
			array(
				...
				'message' => $deleted > 0
					? sprintf(
						...
						_n( '%d visit deleted.', '%d visits deleted.', $deleted, 'flowbie-wp' ),
						$deleted
					)
					: __( 'No visits were deleted.', 'flowbie-wp' ),
			)
		);
		self::redirect_to_overseer( 'list', 0, self::overseer_list_redirect_query() );
	}
	...
	public static function handle_clear_overseer_visits(): void {
		...
		check_admin_referer( self::ACTION_CLEAR_OVERSEER_VISITS, 'flowbie_wp_clear_overseer_nonce' );

		Flowbie_Wp_Overseer::delete_all_visits();

		self::set_flash(
			array(
				'kind'    => 'overseer',
				'success' => true,
				'message' => __( 'All visits cleared.', 'flowbie-wp' ),
			)
		);
	...
	public static function handle_export_overseer(): void {
		...
		check_admin_referer( self::ACTION_EXPORT_OVERSEER, 'flowbie_wp_export_overseer_nonce' );
		...
		$rows     = Flowbie_Wp_Overseer::query_for_export( $args );
		$csv      = Flowbie_Wp_Overseer_Csv::build( $rows );
		$filename = 'flowbie-overseer-visits-' . gmdate( 'Y-m-d' ) . '.csv';
	...
	public static function handle_save_overseer_settings(): void {
		...
		check_admin_referer( self::ACTION_SAVE_OVERSEER_SETTINGS, 'flowbie_wp_overseer_settings_nonce' );

		Flowbie_Wp_Overseer::save_settings(
			array(
		...
		Flowbie_Wp_Overseer::sync_builtin_script();

		self::set_flash(
			array(
				'kind'    => 'overseer',
				'success' => true,
				'message' => __( 'Overseer settings saved.', 'flowbie-wp' ),
			)
		);
	...
	public static function handle_save_overseer_conversion(): void {
		...
		check_admin_referer( self::ACTION_SAVE_OVERSEER_CONVERSION, 'flowbie_wp_overseer_conversion_nonce' );

		$trigger_type = isset( $_POST['trigger_type'] ) ? sanitize_key( wp_unslash( (string) $_POST['trigger_type'] ) ) : 'form_success';
		if ( ! array_key_exists( $trigger_type, Flowbie_Wp_Overseer_Conversions::TRIGGER_TYPES ) ) {
		...
		$saved = Flowbie_Wp_Overseer_Conversions::save_goal( $payload );
		self::set_flash(
			array(
				'kind'    => 'overseer',
				'success' => (bool) $saved,
				'message' => $saved
					? __( 'Conversion goal saved.', 'flowbie-wp' )
					: __( 'Could not save conversion goal. Check the trigger, form, and rules.', 'flowbie-wp' ),
			)
		);
	...
	public static function handle_delete_overseer_conversion(): void {
		...
		$deleted = $goal_id !== '' && Flowbie_Wp_Overseer_Conversions::delete_goal( $goal_id );
		self::set_flash(
			array(
				'kind'    => 'overseer',
				'success' => $deleted,
				'message' => $deleted
					? __( 'Conversion goal deleted.', 'flowbie-wp' )
					: __( 'Conversion goal not found.', 'flowbie-wp' ),
			)
		);
		self::redirect_to_overseer( 'conversions' );
	}
	...
	public static function handle_run_overseer_analysis(): void {
		...
		check_admin_referer( self::ACTION_RUN_OVERSEER_ANALYSIS, 'flowbie_wp_overseer_analysis_nonce' );
		...
		$result = Flowbie_Wp_Overseer_Analysis::run(
			array(
				'date_from'   => $date_from,
				'date_to'     => $date_to,
	...
	public static function handle_delete_overseer_report(): void {
		...
		if ( $id > 0 ) {
			Flowbie_Wp_Overseer_Reports::delete( $id );
		}
	...
	public static function handle_approve_overseer_task(): void {
		self::handle_overseer_task_status( self::ACTION_APPROVE_OVERSEER_TASK, 'approved', __( 'Task approved.', 'flowbie-wp' ) );
	}

	public static function handle_dismiss_overseer_task(): void {
		self::handle_overseer_task_status( self::ACTION_DISMISS_OVERSEER_TASK, 'dismissed', __( 'Task dismissed.', 'flowbie-wp' ) );
	}

	public static function handle_done_overseer_task(): void {
		self::handle_overseer_task_status( self::ACTION_DONE_OVERSEER_TASK, 'done', __( 'Task marked done.', 'flowbie-wp' ) );
	}
	...
	private static function handle_overseer_task_status( string $action, string $status, string $message ): void {
		...
		if ( $id > 0 ) {
			Flowbie_Wp_Overseer_Tasks::update_status( $id, $status );
		}

		$task = Flowbie_Wp_Overseer_Tasks::get( $id );
		$query = array();
