<?php
/**
 * Shared Flowbie brand icon (brain — Lucide-style two-hemisphere mark).
 *
 * @package Flowbie_Wp
 */
...
trait Flowbie_Wp_Admin_Trait_Brand_Icon {

	/**
	 * Plugin URL for the wp-admin sidebar menu icon.
	 */
	public static function brand_icon_menu_url(): string {
		return plugin_dir_url( FLOWBIE_WP_PLUGIN_FILE ) . 'assets/admin/flowbie-brain-icon.svg';
	}
	...
	public static function brand_icon_svg( string $color = '#22d3ee', int $size = 20 ): string {
		...
		return '<svg class="flowbie-wp-brand-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="' . (int) $size . '" height="' . (int) $size . '" fill="none" stroke="' . esc_attr( $color ) . '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">'
	...
	public static function welcome_robot_svg( string $color = '#22d3ee', int $size = 120 ): string {
		...
		return '<svg class="flowbie-wp-welcome-robot" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120" width="' . (int) $size . '" height="' . (int) $size . '" aria-hidden="true" focusable="false">'
			. '<defs>'
			. '<filter id="flowbie-robot-glow" x="-20%" y="-20%" width="140%" height="140%">'
			. '<feGaussianBlur stdDeviation="3" result="blur"/>'
			. '<feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>'
			. '</filter>'
			. '</defs>'
			. '<rect x="28" y="38" width="64" height="52" rx="12" fill="#181b21" stroke="' . esc_attr( $color ) . '" stroke-width="2.5"/>'
			. '<rect x="38" y="48" width="18" height="18" rx="4" fill="' . esc_attr( $color ) . '" opacity="0.9" filter="url(#flowbie-robot-glow)"/>'
			. '<rect x="64" y="48" width="18" height="18" rx="4" fill="' . esc_attr( $color ) . '" opacity="0.9" filter="url(#flowbie-robot-glow)"/>'
			...
			. '<circle cx="60" cy="20" r="5" fill="' . esc_attr( $color ) . '" filter="url(#flowbie-robot-glow)"/>'
