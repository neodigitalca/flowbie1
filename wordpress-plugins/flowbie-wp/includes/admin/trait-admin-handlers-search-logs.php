...
trait Flowbie_Wp_Admin_Trait_Handlers_Search_Logs {

	const ACTION_EXPORT_SEARCH_LOGS       = 'flowbie_wp_export_search_logs';
	const ACTION_SAVE_SEARCH_LOG_SETTINGS = 'flowbie_wp_save_search_log_settings';
	const ACTION_DELETE_SEARCH_LOG        = 'flowbie_wp_delete_search_log';
	const ACTION_BULK_SEARCH_LOGS         = 'flowbie_wp_bulk_search_logs';
	...
	public static function handle_delete_search_log(): void {
		...
		if ( $id > 0 ) {
			Flowbie_Wp_Search_Logs::delete_event( $id );
		}
	...
	public static function handle_export_search_logs(): void {
		...
		check_admin_referer( self::ACTION_EXPORT_SEARCH_LOGS, 'flowbie_wp_export_search_logs_nonce' );
		...
		$rows     = Flowbie_Wp_Search_Logs::query_for_export( $args );
		$csv      = Flowbie_Wp_Search_Logs_Csv::build( $rows );
		$filename = 'flowbie-search-logs-' . gmdate( 'Y-m-d' ) . '.csv';
	...
	public static function handle_save_search_log_settings(): void {
		...
		check_admin_referer( self::ACTION_SAVE_SEARCH_LOG_SETTINGS, 'flowbie_wp_search_log_settings_nonce' );

		Flowbie_Wp_Search_Logs::save_settings(
			array(
