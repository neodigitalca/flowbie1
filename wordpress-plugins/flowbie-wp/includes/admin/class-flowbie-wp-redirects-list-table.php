...
class Flowbie_Wp_Redirects_List_Table extends WP_List_Table {
	...
	public function get_columns(): array {
		return array(
			...
			'source'           => __( 'From', 'flowbie-wp' ),
			'destination'      => __( 'To', 'flowbie-wp' ),
			'type'             => __( 'Type', 'flowbie-wp' ),
			'hits'             => __( 'Hits', 'flowbie-wp' ),
			'created_at'       => __( 'Created', 'flowbie-wp' ),
			'last_accessed_at' => __( 'Last Accessed', 'flowbie-wp' ),
	...
	protected function column_source( $item ): string {
		...
		$dest_url    = Flowbie_Wp_Redirects_Csv::resolve_destination_url( (string) $item->destination );
		...
		$actions = array(
			'edit' => sprintf(
				...
				esc_attr( sprintf( __( 'Edit redirect for %s', 'flowbie-wp' ), (string) $item->source ) ),
			...
			'view' => sprintf(
				...
				esc_url( $view_url ),
				/* translators: %s: redirect source path */
				esc_attr( sprintf( __( 'View source URL for %s', 'flowbie-wp' ), (string) $item->source ) ),
				esc_html__( 'View', 'flowbie-wp' )
			),
		);
		...
		if ( 'trash' === $status ) {
			...
			$actions['delete'] = sprintf(
				...
				esc_js( __( 'Delete this redirect permanently?', 'flowbie-wp' ) ),
	...
	public function prepare_items(): void {
		$this->status_counts = Flowbie_Wp_Redirects::status_counts();
	...
	private static function row_action_url( int $id, string $op ): string {
		$url = admin_url(
			add_query_arg(
				array(
					'action'      => 'flowbie_wp_redirect_row',
					'redirect_op' => $op,
					'id'          => $id,
				),
				'admin-post.php'
		...
		return wp_nonce_url( $url, 'flowbie_wp_redirect_row_' . $op . '_' . $id );
	}
}
