<?php
/**
 * Shared sidebar panel shell for Flowbie WP admin pages.
 *
 * @package Flowbie_Wp
 */
...
trait Flowbie_Wp_Admin_Trait_Render_Panel_Shell {
	...
	protected static function panel_page_header( string $lead, string $dashboard_url ): void {
		...
		?>
		<header class="flowbie-wp-settings__header">
			<h1 class="flowbie-wp-settings__title"><?php echo esc_html( get_admin_page_title() ); ?></h1>
			<p class="flowbie-wp-settings__lead"><?php echo esc_html( $lead ); ?></p>
			<p>
				<a href="<?php echo esc_url( $dashboard_url ); ?>"><?php esc_html_e( '← Dashboard', 'flowbie-wp' ); ?></a>
			</p>
		</header>
	...
	/**
	 * @param string                              $page_slug  Admin page slug (e.g. flowbie-wp-sitemap).
	...
	protected static function panel_layout_start( string $page_slug, array $nav_groups, string $active_tab, string $nav_label ): void {
		?>
		<div class="flowbie-wp-panel-layout">
			<nav class="flowbie-wp-panel-nav" aria-label="<?php echo esc_attr( $nav_label ); ?>">
				<?php foreach ( $nav_groups as $group ) : ?>
					...
					?>
					<?php if ( $heading !== '' ) : ?>
						<p class="flowbie-wp-panel-nav__heading"><?php echo esc_html( $heading ); ?></p>
					<?php endif; ?>
					<ul class="flowbie-wp-panel-nav__list">
						<?php foreach ( $tabs as $tab_key => $tab_label ) : ?>
							...
							?>
							<li>
								<a class="flowbie-wp-panel-nav__item <?php echo $is_active ? 'flowbie-wp-panel-nav__item--active' : ''; ?>" href="<?php echo esc_url( $url ); ?>">
			...
			<div class="flowbie-wp-panel-content">
		<?php
	}
		...
		<div class="flowbie-wp-panel-footer">
			<div class="flowbie-wp-panel-footer__left">
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="flowbie-wp-panel-inline-form">
					...
					<button type="submit" class="button flowbie-wp-panel-footer__reset"><?php esc_html_e( 'Reset Options', 'flowbie-wp' ); ?></button>
				</form>
			...
			<p class="flowbie-wp-settings__actions flowbie-wp-panel-footer__right">
				<button type="submit" form="<?php echo esc_attr( $form_id ); ?>" class="button button-primary flowbie-wp-settings__btn"><?php esc_html_e( 'Save Changes', 'flowbie-wp' ); ?></button>
			</p>
		</div>
		<?php
	}
	...
	protected static function render_gsc_connection_panel( bool $show_settings_link = true ): void {
		$connection = Flowbie_Wp_Gsc::test_connection();
		$connected  = ! is_wp_error( $connection ) && ! empty( $connection['connected'] );
		$client_email = Flowbie_Wp_Gsc::EMAIL_FALLBACK;
		$property   = '';
		$message    = '';
		$error_msg  = '';
		$hint       = '';
		...
		if ( $hint === '' && ! $connected ) {
			$hint = sprintf(
				...
				__( 'In Google Search Console, go to Settings → Users and permissions and add %s with at least Restricted access for this site.', 'flowbie-wp' ),
				$client_email
			);
		}

		$status_class = $connected ? 'flowbie-wp-settings__gsc-status--ok' : 'flowbie-wp-settings__gsc-status--error';
		$status_label = $connected
			? __( 'Connected', 'flowbie-wp' )
			: __( 'Not connected', 'flowbie-wp' );
		?>
		<section class="flowbie-wp-settings__card flowbie-wp-settings__card--gsc" aria-labelledby="flowbie-wp-gsc-connection-heading">
			<h2 id="flowbie-wp-gsc-connection-heading" class="flowbie-wp-settings__card-title"><?php esc_html_e( 'Google Search Console', 'flowbie-wp' ); ?></h2>
			<p class="flowbie-wp-settings__card-desc">
				<?php esc_html_e( 'Analytics uses this service account to read Search Console stats for this WordPress site.', 'flowbie-wp' ); ?>
			</p>

			<div class="flowbie-wp-settings__gsc-panel">
				<div class="flowbie-wp-settings__gsc-row">
					<span class="flowbie-wp-settings__gsc-status <?php echo esc_attr( $status_class ); ?>">
						<?php echo esc_html( $status_label ); ?>
					</span>
				</div>

				<dl class="flowbie-wp-settings__gsc-meta">
					<div class="flowbie-wp-settings__gsc-meta-row">
						<dt><?php esc_html_e( 'Service account', 'flowbie-wp' ); ?></dt>
						<dd><code><?php echo esc_html( $client_email ); ?></code></dd>
					</div>
					<?php if ( $property !== '' ) : ?>
						<div class="flowbie-wp-settings__gsc-meta-row">
							<dt><?php esc_html_e( 'GSC property', 'flowbie-wp' ); ?></dt>
				...
				<?php if ( $error_msg !== '' ) : ?>
					<p class="flowbie-wp-settings__gsc-error"><?php echo esc_html( $error_msg ); ?></p>
				<?php elseif ( $message !== '' ) : ?>
					<p class="flowbie-wp-settings__gsc-note"><?php echo esc_html( $message ); ?></p>
				<?php endif; ?>

				<?php if ( $hint !== '' && ! $connected ) : ?>
					<p class="flowbie-wp-settings__gsc-hint"><?php echo esc_html( $hint ); ?></p>
				<?php endif; ?>
			</div>

			<?php if ( $show_settings_link ) : ?>
			<p class="description" style="margin-top:12px;">
				<a href="<?php echo esc_url( $settings_url ); ?>"><?php esc_html_e( 'Open Settings for GSC credentials', 'flowbie-wp' ); ?></a>
			</p>
			<?php endif; ?>
		</section>
		<?php
	...
	protected static function flowbie_current_admin_page_slug(): string {
		return isset( $_GET['page'] ) ? sanitize_key( wp_unslash( (string) $_GET['page'] ) ) : '';
	}

	protected static function flowbie_shell_active_slug( string $page_slug ): string {
		$parent_map = array(
			'flowbie-wp-fields-edit'      => 'flowbie-wp-fields',
			'flowbie-wp-post-types-edit'  => 'flowbie-wp-post-types',
			'flowbie-wp-forms-edit'       => 'flowbie-wp-forms',
			'flowbie-wp-forms-entries'    => 'flowbie-wp-forms',
			'flowbie-wp-agent-hub-edit'   => 'flowbie-wp-agent-hub',
		);

		return $parent_map[ $page_slug ] ?? $page_slug;
	}

	protected static function flowbie_overseer_active_section(): string {
	...
	protected static function render_acf_shell_overseer_nav( bool $is_active_parent ): void {
		...
		?>
		<div class="flowbie-wp-acf-shell-nav__menu flowbie-wp-acf-shell-nav__menu--overseer<?php echo $is_active_parent ? ' is-active' : ''; ?>">
			<button type="button" class="flowbie-wp-acf-shell-nav__menu-btn" aria-haspopup="true" aria-expanded="false">
				<?php esc_html_e( 'Overseer', 'flowbie-wp' ); ?>
				<span class="flowbie-wp-acf-shell-nav__menu-caret" aria-hidden="true"></span>
			</button>
			<div class="flowbie-wp-acf-shell-nav__dropdown" role="menu">
				<?php foreach ( self::get_overseer_subnav_items() as $item ) : ?>
					<a class="flowbie-wp-acf-shell-nav__dropdown-item<?php echo $active === $item['slug'] ? ' is-active' : ''; ?>" role="menuitem" href="<?php echo esc_url( $item['url'] ); ?>"><?php echo esc_html( $item['label'] ); ?></a>
				<?php endforeach; ?>
			</div>
		</div>
		<?php
	...
	protected static function flowbie_shell_wrap_classes( string $extra = '' ): string {
		return trim( 'flowbie-wp-settings flowbie-wp-settings--wide flowbie-wp-settings--shell ' . $extra );
	}
	 ...
	 */
	protected static function flowbie_group_shell_open( string $page_slug, string $wrap_classes = '', string $flash_kind = '' ): void {
		$page_slug = sanitize_key( $page_slug );
		if ( $page_slug === '' ) {
			$page_slug = self::flowbie_current_admin_page_slug();
		}

		$group = Flowbie_Wp_Admin_Menu::get_group_for_page_slug( $page_slug );
		$flash = self::get_and_clear_flash();
		if ( $flash && $flash_kind !== '' && ( ! isset( $flash['kind'] ) || (string) $flash['kind'] !== $flash_kind ) ) {
			$flash = null;
		}

		$wrap_classes = trim( 'flowbie-wp-acf-shell ' . self::flowbie_shell_wrap_classes( $wrap_classes ) );
		?>
		<div class="wrap <?php echo esc_attr( $wrap_classes ); ?>">
			<div class="flowbie-wp-acf-shell-app">
				<?php if ( is_array( $group ) && ! empty( $group['id'] ) ) : ?>
					<header class="flowbie-wp-acf-shell-header">
						<div class="flowbie-wp-acf-shell-header__brand">
							<span class="flowbie-wp-acf-shell-header__icon"><?php echo self::brand_icon_svg( '#22d3ee', 22 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
							<span class="flowbie-wp-acf-shell-header__name"><?php echo esc_html( sprintf( __( 'Flowbie %s', 'flowbie-wp' ), (string) $group['label'] ) ); ?></span>
						</div>
						<nav class="flowbie-wp-acf-shell-nav" aria-label="<?php echo esc_attr( sprintf( __( '%s sections', 'flowbie-wp' ), (string) $group['label'] ) ); ?>">
							<?php
							$active_slug = self::flowbie_shell_active_slug( $page_slug );
							foreach ( Flowbie_Wp_Admin_Menu::get_group_tier_nav_items( (string) $group['id'] ) as $nav_item ) :
								if ( 'flowbie-wp-overseer' === $nav_item['slug'] ) {
									self::render_acf_shell_overseer_nav( 'flowbie-wp-overseer' === $active_slug );
									continue;
								}
								?>
								<a class="flowbie-wp-acf-shell-nav__item<?php echo $nav_item['slug'] === $active_slug ? ' is-active' : ''; ?>" href="<?php echo esc_url( $nav_item['url'] ); ?>"><?php echo esc_html( $nav_item['label'] ); ?></a>
							<?php endforeach; ?>
						</nav>
					</header>
				<?php endif; ?>
				<div class="flowbie-wp-acf-shell-body">
					<?php if ( $flash ) : ?>
						<div class="notice notice-<?php echo ! empty( $flash['success'] ) ? 'success' : 'error'; ?> is-dismissible flowbie-wp-acf-shell-notice">
							<p><?php echo esc_html( isset( $flash['message'] ) ? (string) $flash['message'] : '' ); ?></p>
						</div>
					<?php endif; ?>
		<?php
	...
	protected static function flowbie_group_shell_close(): void {
		?>
				</div>
			</div>
		</div>
	...
	protected static function panel_form_field_input(
		...
		if ( $wrap_cell ) {
			?>
			<div class="flowbie-schema-cell flowbie-schema-cell--<?php echo esc_attr( $span ); ?>">
			<?php
		}
		?>
			<div class="flowbie-field flowbie-field--text flowbie-field--stacked">
				<label class="flowbie-field__label flowbie-field__label--above" for="<?php echo esc_attr( $id ); ?>"><?php echo esc_html( $label ); ?></label>
				<input
	...
	protected static function panel_form_field_textarea(
		...
		?>
			<div class="flowbie-field flowbie-field--textarea flowbie-field--stacked">
				<label class="flowbie-field__label flowbie-field__label--above" for="<?php echo esc_attr( $id ); ?>"><?php echo esc_html( $label ); ?></label>
				<textarea
					name="<?php echo esc_attr( $name ); ?>"
					id="<?php echo esc_attr( $id ); ?>"
		...
		<div class="flowbie-schema-cell flowbie-schema-cell--full">
			<label class="flowbie-wp-panel-toggle">
				<input
					...
					<?php checked( $checked ); ?>
				/>
				<span class="flowbie-wp-panel-toggle__label"><?php echo esc_html( $label ); ?></span>
			</label>
			<?php if ( $note !== '' ) : ?>
				<p class="flowbie-field__note"><?php echo esc_html( $note ); ?></p>
			<?php endif; ?>
