...
trait Flowbie_Wp_Admin_Trait_Handlers_Script_Manager {

	const ACTION_SAVE_SCRIPT           = 'flowbie_wp_save_script';
	const ACTION_DELETE_SCRIPT         = 'flowbie_wp_delete_script';
	const ACTION_BULK_SCRIPTS          = 'flowbie_wp_bulk_scripts';
	const ACTION_IMPORT_SCRIPTS        = 'flowbie_wp_import_scripts';
	const ACTION_IMPORT_SCRIPTS_HFCM   = 'flowbie_wp_import_scripts_hfcm';
	const ACTION_IMPORT_SCRIPTS_HFCM_DB = 'flowbie_wp_import_scripts_hfcm_db';
	const ACTION_EXPORT_SCRIPTS        = 'flowbie_wp_export_scripts';
	const ACTION_EXPORT_SCRIPTS_JSON   = 'flowbie_wp_export_scripts_json';
	const ACTION_SAVE_SCRIPT_SETTINGS  = 'flowbie_wp_save_script_settings';
	const ACTION_SCRIPT_ROW            = 'flowbie_wp_script_row';
	...
	public static function handle_script_row_action(): void {
		...
		if ( $id < 1 || ! in_array( $op, array( 'trash', 'restore', 'activate', 'deactivate', 'delete' ), true ) ) {
			wp_die( esc_html__( 'Invalid script action.', 'flowbie-wp' ) );
		}

		check_admin_referer( 'flowbie_wp_script_row_' . $op . '_' . $id );
		...
		$changed  = Flowbie_Wp_Script_Manager::bulk_action( array( $id ), $op );
		$messages = array(
			'trash'      => __( 'Script moved to trash.', 'flowbie-wp' ),
			'restore'    => __( 'Script restored.', 'flowbie-wp' ),
			'activate'   => __( 'Script activated.', 'flowbie-wp' ),
			'deactivate' => __( 'Script deactivated.', 'flowbie-wp' ),
			'delete'     => __( 'Script deleted permanently.', 'flowbie-wp' ),
		);
		...
		self::set_flash(
			array(
				...
				'success' => $changed > 0,
				'message' => $changed > 0
					? ( isset( $messages[ $op ] ) ? $messages[ $op ] : __( 'Script updated.', 'flowbie-wp' ) )
					: __( 'Script could not be updated.', 'flowbie-wp' ),
			)
		);
	...
	public static function handle_save_script(): void {
		...
		check_admin_referer( self::ACTION_SAVE_SCRIPT, 'flowbie_wp_script_nonce' );
		...
		$rules      = $rules_json !== '' ? json_decode( $rules_json, true ) : Flowbie_Wp_Script_Manager_Rules::defaults();
		...
		$data = array(
			...
			'display_rules' => is_array( $rules ) ? $rules : Flowbie_Wp_Script_Manager_Rules::defaults(),
		);

		$result = Flowbie_Wp_Script_Manager::save( $data );
		if ( empty( $result['ok'] ) ) {
			self::set_flash(
				array(
					'kind'    => 'scripts',
					'success' => false,
					'message' => isset( $result['error'] ) ? (string) $result['error'] : __( 'Could not save script.', 'flowbie-wp' ),
				)
			);
		...
		self::set_flash(
			array(
				'kind'    => 'scripts',
				'success' => true,
				'message' => $id > 0 ? __( 'Script updated.', 'flowbie-wp' ) : __( 'Script created.', 'flowbie-wp' ),
			)
		);
		self::redirect_to_script_manager( 'list' );
	}
	...
	public static function handle_delete_script(): void {
		...
		check_admin_referer( self::ACTION_DELETE_SCRIPT, 'flowbie_wp_delete_script_nonce' );
		...
		self::set_flash(
			array(
				...
				'success' => $deleted,
				'message' => $deleted
					? __( 'Script deleted permanently.', 'flowbie-wp' )
					: __( 'Script could not be deleted.', 'flowbie-wp' ),
			)
		);
		self::redirect_to_script_manager( 'list' );
		return;
	...
	public static function handle_bulk_scripts(): void {
		...
		if ( in_array( $action, array( 'trash', 'restore', 'activate', 'deactivate', 'delete' ), true ) ) {
			$changed = Flowbie_Wp_Script_Manager::bulk_action( $ids, $action );
		}

		$message = $changed > 0
			? sprintf(
				/* translators: %d: number of scripts updated */
				_n( '%d script updated.', '%d scripts updated.', $changed, 'flowbie-wp' ),
				$changed
			)
			: __( 'No scripts were updated.', 'flowbie-wp' );
		if ( $blocked > 0 ) {
			$message .= ' ' . __( 'The built-in Flowbie Page View tag was not removed. Disable tracking in Overseer settings first.', 'flowbie-wp' );
		}
	...
	private static function overseer_builtin_block_message( int $id, string $op ): ?string {
		...
		if ( ! Flowbie_Wp_Overseer::is_builtin_script_id( $id ) ) {
			return null;
		}
		if ( ! Flowbie_Wp_Overseer::is_builtin_protected() ) {
			return null;
		}
		if ( ! in_array( $op, array( 'trash', 'delete' ), true ) ) {
			return null;
		}
		return __( 'The built-in Flowbie Page View tag cannot be removed while Overseer tracking is enabled. Disable tracking in Overseer settings first.', 'flowbie-wp' );
	}

	public static function handle_import_scripts(): void {
		self::handle_script_import_upload(
			self::ACTION_IMPORT_SCRIPTS,
			'flowbie_wp_import_scripts_nonce',
			'script_import_file',
			__( 'Choose a Flowbie JSON or CSV file to import.', 'flowbie-wp' ),
			static function ( string $file_text, string $filename ): array {
				return Flowbie_Wp_Script_Manager_Import::parse_file( $file_text, $filename );
			}
		);
	}
	...
	public static function handle_import_scripts_hfcm(): void {
		self::handle_script_import_upload(
			...
			'flowbie_wp_import_scripts_hfcm_nonce',
			'script_hfcm_import_file',
			__( 'Choose an HFCM export JSON file.', 'flowbie-wp' ),
			static function ( string $file_text, string $filename ): array {
				unset( $filename );
				return Flowbie_Wp_Script_Manager_Import::parse_hfcm_export( $file_text );
			}
		);
	}
	...
	public static function handle_import_scripts_hfcm_db(): void {
		...
		check_admin_referer( self::ACTION_IMPORT_SCRIPTS_HFCM_DB, 'flowbie_wp_import_scripts_hfcm_db_nonce' );

		require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/super-migrate/interface-flowbie-wp-migrate-adapter.php';
		require_once FLOWBIE_WP_PLUGIN_DIR . 'includes/super-migrate/adapters/class-flowbie-wp-migrate-source-hfcm.php';
		$result = Flowbie_Wp_Migrate_Source_Hfcm::import_all_from_database();
		...
		if ( empty( $result['ok'] ) ) {
			self::set_flash(
				array(
					'kind'    => 'scripts',
					'success' => false,
					'message' => isset( $result['error'] ) ? (string) $result['error'] : __( 'HFCM import failed.', 'flowbie-wp' ),
				)
			);
			self::redirect_to_script_manager( 'list' );
		}
	...
	public static function hfcm_database_import_status(): array {
		...
		$plugin_active  = Flowbie_Wp_Migrate_Source_Hfcm::is_hfcm_plugin_active();
		$snippet_count  = Flowbie_Wp_Migrate_Source_Hfcm::hfcm_snippet_count();
		if ( null === $snippet_count && $plugin_active ) {
			$snippet_count = count( Flowbie_Wp_Migrate_Source_Hfcm::hfcm_fetch_db_rows() );
		}
		$hfcm_total     = null !== $snippet_count ? (int) $snippet_count : 0;
		$hfcm_category  = __( 'HFCM', 'flowbie-wp' );
		$imported_count = Flowbie_Wp_Script_Manager::count_by_category( $hfcm_category );
		...
		return array(
			...
			'table'          => Flowbie_Wp_Migrate_Source_Hfcm::hfcm_table_name_for_display(),
		);
	}
	...
	private static function handle_script_import_upload(
		...
		$stats   = Flowbie_Wp_Script_Manager::merge_import( $parsed['rows'] );
		$message = self::format_script_import_message(
			$stats,
			array(
	...
	public static function handle_export_scripts(): void {
		...
		check_admin_referer( self::ACTION_EXPORT_SCRIPTS, 'flowbie_wp_export_scripts_nonce' );

		$rows = Flowbie_Wp_Script_Manager::export_rows();
		$csv  = Flowbie_Wp_Script_Manager_Csv::build( $rows );
		$filename = 'flowbie-scripts-' . gmdate( 'Y-m-d' ) . '.csv';
	...
	public static function handle_export_scripts_json(): void {
		...
		check_admin_referer( self::ACTION_EXPORT_SCRIPTS_JSON, 'flowbie_wp_export_scripts_json_nonce' );

		$rows = Flowbie_Wp_Script_Manager::export_rows();
		$json = Flowbie_Wp_Script_Manager_Import::build_json_export( $rows );
		$filename = 'flowbie-scripts-' . gmdate( 'Y-m-d' ) . '.json';
	...
	public static function handle_save_script_settings(): void {
		...
		check_admin_referer( self::ACTION_SAVE_SCRIPT_SETTINGS, 'flowbie_wp_script_settings_nonce' );

		Flowbie_Wp_Script_Manager::save_settings(
			array(
		...
		self::set_flash(
			array(
				'kind'    => 'scripts',
				'success' => true,
				'message' => __( 'Script Manager settings saved.', 'flowbie-wp' ),
			)
		);
