...
trait Flowbie_Wp_Admin_Trait_Content_Tools_Handlers {

	const ACTION_SWITCH_POST_TYPE = 'flowbie_wp_switch_post_type';
	const ACTION_DUPLICATE_POST   = 'flowbie_wp_duplicate_post';
	...
	public static function save_bulk_edit_content_type( int $post_id, WP_Post $post ): void {
		if ( ! isset( $_REQUEST['flowbie_new_post_type'] ) ) {
			return;
		}
		...
		$new_type = sanitize_key( wp_unslash( (string) $_REQUEST['flowbie_new_post_type'] ) );
		if ( $new_type === '' || $new_type === '-1' ) {
			return;
		}

		Flowbie_Wp_Content_Tools::switch_post_type( $post_id, $new_type );
	}

	public static function handle_switch_post_type(): void {
		check_admin_referer( self::ACTION_SWITCH_POST_TYPE, 'flowbie_content_tools_nonce' );
		...
		$result = $post_id > 0 ? Flowbie_Wp_Content_Tools::switch_post_type( $post_id, $new_type ) : new WP_Error( 'flowbie_missing_post', '' );
	...
	public static function handle_duplicate_post(): void {
		check_admin_referer( self::ACTION_DUPLICATE_POST, 'flowbie_content_tools_nonce' );
		...
		$result = $post_id > 0 ? Flowbie_Wp_Content_Tools::duplicate_post( $post_id ) : new WP_Error( 'flowbie_missing_post', '' );
		if ( is_wp_error( $result ) ) {
