<?php
/**
 * Flo Sheet — master JSON workbook for Flowbie WP settings + third-party crawls.
 *
 * @package Flowbie_Wp
 */
...
class Flowbie_Wp_Flo_Sheet {

	const VERSION     = 1;
	const OPTION_KEY  = 'flowbie_wp_flo_sheet';
	...
	public static function site_meta(): array {
		$meta = array(
			...
			'flowbie_version' => defined( 'FLOWBIE_WP_VERSION' ) ? FLOWBIE_WP_VERSION : '',
		);
	...
	public static function from_json( string $json ): array {
		...
		if ( $json === '' ) {
			return array(
				'ok'    => false,
				'error' => __( 'Flo Sheet JSON is empty.', 'flowbie-wp' ),
			);
		}
		...
		if ( ! is_array( $data ) ) {
			return array(
				...
				'error' => sprintf(
					...
					__( 'Invalid Flo Sheet JSON (%s).', 'flowbie-wp' ),
					json_last_error_msg()
				),
			);
		}
		if ( empty( $data['version'] ) || (int) $data['version'] !== self::VERSION ) {
			return array(
				'ok'    => false,
				'error' => __( 'Unsupported Flo Sheet version.', 'flowbie-wp' ),
			);
		}
		return array(
			'ok'    => true,
	...
	public static function download_filename(): string {
		...
		return 'flowbie-flo-sheet-' . sanitize_file_name( $host ) . '-' . gmdate( 'Y-m-d' ) . '.json';
	}
	...
	public static function redact_secrets( $value ) {
		if ( class_exists( 'Flowbie_Wp_Speed_Export', false ) ) {
			return Flowbie_Wp_Speed_Export::redact_secrets( $value );
		}
		if ( ! is_array( $value ) ) {
			return $value;
		}
