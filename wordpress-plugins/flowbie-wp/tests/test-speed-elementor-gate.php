...
assert_true( Flowbie_Wp_Speed_Gate::html_uses_elementor( $elementor_html ), 'detects elementor-page markup' );
assert_true( ! Flowbie_Wp_Speed_Gate::html_uses_elementor( $plain_html ), 'plain HTML is not elementor' );

$safe = Flowbie_Wp_Speed_Gate::config_for_html( $config, $elementor_html );
assert_true( empty( $safe['aggregate_css'] ) && empty( $safe['aggregate_js'] ) && empty( $safe['defer_js'] ), 'elementor config disables risky flags' );

$unchanged = Flowbie_Wp_Speed_Gate::config_for_html( $config, $plain_html );
assert_true( ! empty( $unchanged['aggregate_css'] ), 'non-elementor config unchanged' );

assert_true(
	Flowbie_Wp_Speed_Excludes::is_defer_excluded( 'https://example.test/wp-content/plugins/elementor/assets/js/frontend.min.js', array() ),
