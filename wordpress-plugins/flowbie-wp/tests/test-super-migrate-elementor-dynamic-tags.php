...
$GLOBALS['flowbie_test_posts']     = array();
...
if ( ! function_exists( 'update_post_meta' ) ) {
	function update_post_meta( $post_id, $key, $value ) {
		$GLOBALS['flowbie_test_post_meta'][ $post_id ][ $key ] = $value;
		return true;
	}
}
...
if ( ! function_exists( 'delete_post_meta' ) ) {
	function delete_post_meta( $post_id, $key ) {
		unset( $GLOBALS['flowbie_test_post_meta'][ $post_id ][ $key ] );
		return true;
	}
}
...
if ( ! function_exists( 'get_post' ) ) {
	function get_post( $post_id ) {
		return $GLOBALS['flowbie_test_posts'][ (int) $post_id ] ?? null;
	}
}
...
if ( ! class_exists( 'Flowbie_Wp_Fields_Storage', false ) ) {
	class Flowbie_Wp_Fields_Storage {
		public static function get_all_groups( bool $active_only = true ): array {
			unset( $active_only );
			return $GLOBALS['flowbie_test_field_groups'] ?? array();
		}
	}
}
...
if ( ! class_exists( 'Flowbie_Wp_Fields_Elementor_Registry', false ) ) {
	class Flowbie_Wp_Fields_Elementor_Registry {
...
$GLOBALS['flowbie_test_field_groups'] = array(
	array(
		'key'      => 'group_contact',
		'location' => array(
			array(
...
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::reset_field_index();
$index = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::build_field_index();
...
$options_key = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::resolve_acf_key_setting( 'options:ci_phone' );
...
$post_key = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::resolve_acf_key_setting( 'field_headline:headline' );
...
$mapped = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::map_acf_tag(
	'acf-text',
	array( 'key' => 'options:ci_phone' )
);
assert_true( ( $mapped['name'] ?? '' ) === 'flowbie-options-field', 'maps acf-text to flowbie-options-field' );
assert_true(
	( $mapped['settings']['field_name'] ?? '' ) === 'contact-information::ci_phone',
	'maps options key to Flowbie composite field_name'
);

$rewrite = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::rewrite_string( $acf_shortcode );
...
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::set_walk_mode( Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::WALK_MODE_MIGRATE_TO_FLOWBIE );
$rewrite_forward = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::rewrite_string( $acf_shortcode );
assert_true( $rewrite_forward['replacements'] === 1, 'rewrite_string replaces one tag in migrate mode' );
assert_true( strpos( $rewrite_forward['value'], 'flowbie-options-field' ) !== false, 'rewritten shortcode uses flowbie tag' );
assert_true( strpos( $rewrite_forward['value'], 'contact-information' ) !== false, 'rewritten shortcode keeps options slug' );

$flowbie_preserved = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::rewrite_string( $rewrite_forward['value'] );
assert_true( (int) $flowbie_preserved['replacements'] === 0, 'migrate mode leaves existing flowbie tags unchanged' );
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::reset_walk_mode();

$flowbie_settings = rawurlencode( wp_json_encode( array( 'field_name' => 'contact-information::ci_phone' ) ) );
$flowbie_shortcode = '[elementor-tag id="abc1234" name="flowbie-options-field" settings="' . $flowbie_settings . '"]';
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::set_walk_mode( Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::WALK_MODE_REVERT_TO_ACF );
$revert_rewrite = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::rewrite_string( $flowbie_shortcode );
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::reset_walk_mode();
...
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::set_walk_mode( Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::WALK_MODE_MIGRATE_TO_FLOWBIE );
$walk = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::walk_elements( $icon_list_elements );
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::reset_walk_mode();
assert_true( (int) $walk['replacements'] === 2, 'walk_elements rewrites icon list dynamic text and link' );
assert_true(
	strpos( (string) $walk['elements'][0]['settings']['icon_list'][0]['__dynamic__text'], 'flowbie-options-field' ) !== false,
	'icon list __dynamic__text updated'
);
assert_true(
	strpos( (string) $walk['elements'][0]['settings']['icon_list'][0]['__dynamic__link.url'], 'flowbie-options-url' ) !== false,
...
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::set_walk_mode( Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::WALK_MODE_MIGRATE_TO_FLOWBIE );
$reordered_rewrite = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::rewrite_string( $reordered_shortcode );
$post_rewrite = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::rewrite_string( $post_shortcode );
$unknown = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::rewrite_string(
...
assert_true( strpos( $reordered_rewrite['value'], 'flowbie-options-url' ) !== false, 'reordered attrs map to flowbie-options-url' );
assert_true( strpos( $post_rewrite['value'], 'flowbie-field' ) !== false, 'post acf tag maps to flowbie-field' );
...
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::set_walk_mode( Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::WALK_MODE_MIGRATE_TO_FLOWBIE );
$contact_walk = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::walk_elements( $static_contact_elements );
Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::reset_walk_mode();
assert_true( (int) $contact_walk['replacements'] >= 3, 'static contact icon list repaired with flowbie tags' );
assert_true(
	strpos( (string) $contact_walk['elements'][0]['settings']['icon_list'][0]['__dynamic__text'], 'flowbie-options-field' ) !== false,
	'static contact row uses flowbie-options-field'
);
assert_true(
	strpos( (string) $contact_walk['elements'][0]['settings']['icon_list'][0]['__dynamic__link.url'], 'flowbie-options-url' ) !== false,
	'static contact row uses flowbie-options-url'
);

$header_json = wp_json_encode( $icon_list_elements );
$GLOBALS['flowbie_test_post_meta'][101]['_elementor_data'] = $header_json;
$GLOBALS['flowbie_test_posts'][101] = new WP_Post(
	array(
		'ID'         => 101,
		'post_title' => 'Header',
...
$apply = Flowbie_Wp_Migrate_Elementor_Dynamic_Tags::apply_documents( array( 101 ), false );
...
assert_true( is_string( $stored ) && strpos( $stored, 'flowbie-options-field' ) !== false, 'stored elementor data uses flowbie tag' );
