...
$row = Flowbie_Wp_Migrate_Source_Hfcm::hfcm_row_to_snippet(
	array(
...
$body_row = Flowbie_Wp_Migrate_Source_Hfcm::hfcm_row_to_snippet(
	array(
		'name'     => 'Body Tag',
...
$body_parsed = Flowbie_Wp_Script_Manager_Import::hfcm_snippets_to_rows( array( $body_row ) );
...
$parsed = Flowbie_Wp_Script_Manager_Import::hfcm_snippets_to_rows( array( $row ) );
...
$second = Flowbie_Wp_Migrate_Source_Hfcm::hfcm_row_to_snippet(
	array(
		'name'         => 'AI Policy',
...
$multi = Flowbie_Wp_Script_Manager_Import::hfcm_snippets_to_rows( array( $row, $second ) );
...
$file_import = Flowbie_Wp_Script_Manager_Import::parse_hfcm_export( is_string( $json ) ? $json : '{}' );
assert_true( count( $file_import['rows'] ) === 2, 'HFCM JSON export path imports all snippets' );

$empty_code = Flowbie_Wp_Migrate_Source_Hfcm::hfcm_row_to_snippet(
	array(
		'name'     => 'Empty Snippet',
		'snippet'  => '   ',
		'location' => 'header',
...
$empty_result = Flowbie_Wp_Script_Manager_Import::hfcm_snippets_to_rows( array( $empty_code ) );
assert_true( ! empty( $empty_result['error'] ), 'empty snippet code returns an error instead of silent skip' );

assert_true(
	Flowbie_Wp_Migrate_Source_Hfcm::is_hfcm_present() || ! Flowbie_Wp_Migrate_Source_Hfcm::is_hfcm_plugin_active(),
