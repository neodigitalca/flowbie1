...
if ( ! function_exists( 'update_option' ) ) {
	function update_option( $key, $value ) {
		...
		$GLOBALS['flowbie_test_updated_options'][ $key ] = $value;
		return true;
	}
}
...
if ( ! class_exists( 'Flowbie_Wp_Fields_Storage', false ) ) {
	class Flowbie_Wp_Fields_Storage {
		const CPT_OPTIONS = 'flowbie-options-page';
...
$GLOBALS['flowbie_test_options']['elementor_active_kit'] = 99;
$GLOBALS['flowbie_test_post_meta'][99]['_elementor_page_settings'] = array(
	'system_colors' => array(
...
$settings = Flowbie_Wp_Migrate_Elementor_Global_Css::read_kit_settings();
$mapped   = Flowbie_Wp_Migrate_Elementor_Global_Css::map_from_elementor( $settings );
...
$merged = Flowbie_Wp_Migrate_Elementor_Global_Css::merge_non_empty(
	array(
...
$merged_overlay = Flowbie_Wp_Migrate_Elementor_Global_Css::merge_non_empty(
	array(
		'gc_color_primary' => '#FFFFFF',
	),
	array(
...
$GLOBALS['flowbie_test_options']['rank-math-options-titles'] = array();
$apply = Flowbie_Wp_Migrate_Elementor_Global_Css::apply( false );
assert_true( ! empty( $apply['ok'] ), 'apply writes global css values' );
assert_true( isset( $GLOBALS['flowbie_test_updated_options']['global-css_gc_color_primary'] ), 'apply updates primary option' );
assert_true( ! empty( $GLOBALS['flowbie_test_updated_options']['global-css_gc_enabled'] ), 'apply enables global css' );

$css = Flowbie_Wp_Global_Css::build_frontend_css();
assert_true( strpos( $css, '--flowbie-color-primary' ) !== false, 'frontend css includes color variables' );
