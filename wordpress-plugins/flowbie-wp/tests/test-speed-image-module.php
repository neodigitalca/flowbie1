...
require_once dirname( __DIR__ ) . '/includes/class-flowbie-wp-speed-image-settings.php';
require_once dirname( __DIR__ ) . '/includes/class-flowbie-wp-speed-image-stats.php';
require_once dirname( __DIR__ ) . '/includes/class-flowbie-wp-speed-image-delivery.php';
...
$config = Flowbie_Wp_Speed_Image_Settings::sanitize_config(
	array(
		'enabled'         => 1,
		'jpeg_quality'    => 200,
		'png_compression' => 99,
...
$mime_ok = Flowbie_Wp_Speed_Image_Settings::sanitize_config(
...
assert_true( Flowbie_Wp_Speed_Image_Delivery::client_accepts_webp(), 'accepts webp when header present' );
$_SERVER['HTTP_ACCEPT'] = 'image/jpeg';
assert_true( ! Flowbie_Wp_Speed_Image_Delivery::client_accepts_webp(), 'rejects webp without header' );

$tmpdir  = sys_get_temp_dir() . '/flowbie-img-test';
...
$url = Flowbie_Wp_Speed_Image_Delivery::path_to_url( $testfile );
