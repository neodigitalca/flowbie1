...
class Flowbie_Wp_Overseer {
	...
	public static function table_name(): string {
		global $wpdb;
		return $wpdb->prefix . 'flowbie_wp_overseer_visits';
	}

	public static function reports_table_name(): string {
		global $wpdb;
		return $wpdb->prefix . 'flowbie_wp_overseer_reports';
	}

	public static function tasks_table_name(): string {
		global $wpdb;
		return $wpdb->prefix . 'flowbie_wp_overseer_tasks';
	}
	...
	public static function init(): void {
		...
		add_filter( 'flowbie_wp_script_manager_render_code', array( __CLASS__, 'filter_script_code' ), 10, 2 );
	...
	public static function should_load_beacon_on_frontend(): bool {
		...
		$rules = Flowbie_Wp_Script_Manager_Rules::decode(
			isset( $row->display_rules ) ? (string) $row->display_rules : ''
		);

		return Flowbie_Wp_Script_Manager_Rules::matches_current_request( $rules );
	}
	...
	public static function ensure_builtin_script(): void {
		...
		$by_name = Flowbie_Wp_Script_Manager::get_by_name( self::BUILTIN_SCRIPT_NAME );
		if ( $by_name ) {
	...
	public static function get_client_ip(): string {
		...
		return apply_filters( 'flowbie_wp_overseer_client_ip', $ip );
	}
	...
	public static function record_event( array $data ) {
		...
		if ( false === $inserted ) {
			return array( 'ok' => false, 'error' => __( 'Could not save visit.', 'flowbie-wp' ) );
		}

		do_action( 'flowbie_wp_overseer_event_recorded', $visit_uid, $event_type, (object) $row );
			...
			array_filter(
				array_map(
					static function ( $uid ) {
						...
						return Flowbie_Wp_Overseer::is_valid_uuid( $uid ) ? $uid : '';
					},
					$visit_uids
				)
			)
