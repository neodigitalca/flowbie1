...
export const META_VISUAL_TOOL_COLOR_STRIPE_CLASS = cn(
  "grid w-full min-w-0 grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(11rem,13rem)] items-center gap-x-4 gap-y-0",
  META_VISUAL_SETTINGS_STRIPE_MIN_H,
);

/** Inner half: label | style slot | degree. */
export const META_VISUAL_TOOL_HALF_GRID_CLASS =
  "grid w-full min-w-0 grid-cols-[minmax(0,1fr)_7rem_5.5rem] items-center gap-x-2 gap-y-0";

export const META_VISUAL_TOOL_STYLE_SELECT_CLASS = "h-8 w-[7rem] max-w-[7rem] shrink-0";

export const META_VISUAL_COLOR_CELL_GRID_CLASS = cn(
  META_VISUAL_CELL_CLASS,
  "grid min-w-0 grid-cols-[6rem_minmax(0,1fr)] items-center gap-x-2",
);
...
export const META_VISUAL_COMPACT_WEIGHT_INPUT_CLASS = cn(
  "h-8 w-full max-w-[5.5rem] border-0 px-1 text-center text-base tabular-nums",
