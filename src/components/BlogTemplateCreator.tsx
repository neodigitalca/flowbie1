...
import { NOTIFY_CHECKLIST_GENERATED_PLEASE_REVIEW_AND_AP, NOTIFY_FAILED_TO_GENERATE_CHECKLIST, NOTIFY_FAILED_TO_GENERATE_TITLE_OPTIONS, NOTIFY_FAILED_TO_REFRESH_TITLE_OPTIONS, NOTIFY_KEYWORD_DATA_AND_H2_SECTIONS_ARE_REQUIRE, NOTIFY_KEYWORD_DATA_IS_REQUIRED, NOTIFY_MISSING_REQUIRED_DATA_TO_GENERATE_BLUEPR, NOTIFY_NO_AGENTS_GENERATED_FROM_TEMPLATE, notifyAddedXAgentsToBlueprint, notifyFailedToGenerateBlueprintX, notifyFailedToGenerateChecklistX, notifyGeneratedXAgents, notifyGeneratedXNewTitleOptions } from "@/lib/notify-messages";
...
export const BlogTemplateCreator: React.FC<BlogTemplateCreatorProps> = ({
    ...
    if (keywordData && keywordData.keyword && keywordData.keyword.trim() && selectedH2Sections.length > 0 && titleOptions.length === 0 && !isGeneratingTitles && showTitleSelector) {
      ...
      generateTitleOptions(
        ...
        .catch((error) => {
          ...
          notify.error(NOTIFY_FAILED_TO_GENERATE_TITLE_OPTIONS);
          setIsGeneratingTitles(false);
        });
    }
  ...
  const generateChecklist = useCallback(async (title: string, userPrompt?: string) => {
    if (!keywordData) {
      notify.error(NOTIFY_KEYWORD_DATA_IS_REQUIRED);
      return;
    }
  ...
  const handleRefreshTitles = useCallback(async () => {
    if (!keywordData || !keywordData.keyword || !keywordData.keyword.trim() || selectedH2Sections.length === 0) {
      notify.error(NOTIFY_KEYWORD_DATA_AND_H2_SECTIONS_ARE_REQUIRE);
      return;
    }
    ...
    try {
      ...
      setTitleOptions(titles);
      notify.success(notifyGeneratedXNewTitleOptions(titles.length));
    } catch (error) {
      console.error("Error refreshing titles:", error);
      notify.error(NOTIFY_FAILED_TO_REFRESH_TITLE_OPTIONS);
    } finally {
      setIsGeneratingTitles(false);
    }
  ...
  const handleApproveChecklist = useCallback(async () => {
    if (!selectedTitle || !keywordData || checklist.length === 0) {
      notify.error(NOTIFY_MISSING_REQUIRED_DATA_TO_GENERATE_BLUEPR);
      return;
    }
    ...
    try {
      ...
      setIsGeneratingBlueprint(false);
      setElapsedTime(0);
      notify.success(notifyGeneratedXAgents(result.agents.length));
    } catch (error) {
  ...
  const handleAcceptAll = useCallback(
    (agents: AgentConfig[]) => {
      ...
      notify.success(notifyAddedXAgentsToBlueprint(agents.length));
      // Reset state
      setChecklist([]);
