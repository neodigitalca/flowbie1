import { NOTIFY_CSV_FILE_DOWNLOADED, NOTIFY_KEYWORD_ANALYSIS_HAD_ISSUES_CONTINUING_W, NOTIFY_NO_BLOG_POSTS_WERE_GENERATED_PLEASE_TRY_, NOTIFY_NO_TEMPLATES_TO_DOWNLOAD, NOTIFY_OPENROUTER_API_KEY_IS_REQUIRED_PLEASE_SE, NOTIFY_SERVICE_AREA_KEYWORD_IS_REQUIRED_WHEN_GE, NOTIFY_SERVICE_AREA_SITEMAP_SCRAPING_HAD_ISSUES, notifyKeywordAnalysisFailedForXContinuing, notifyProcessingXGeneratedBlogPostx, notifyServiceAreaGenerationFailedX, notifyStep1XAnalyzingWordpressBlogPosts, notifyStep1XCompleteAnalyzedXBlogPostP, notifyStep23ProcessingXGscKeywordsFilte, notifyStep2XCompleteSelectedXKeywordsFr, notifyStep2XNoGscKeywordsFoundWillUse, notifyStep2XWarningGscAnalysisFailedXC, notifyStep2bXAnalyzingXKeywordsWithAiF, notifyStep2bXCompleteAnalyzedXKeywordsW, notifyStep3XGeneratingXBlogPostxUsingA, notifyStepXXAnalyzingWordpressServiceAre, notifyStepXXCompleteAnalyzedXServiceAre, notifyStepXXCompleteGeneratedXBlogPost, notifyStepXXCompleteGeneratedXServiceAr, notifyStepXXCompleteScrapedServiceAreaS, notifyStepXXFetchingGscQueryDataForX, notifyStepXXGeneratingXServiceAreaPagex, notifyStepXXScrapingServiceAreaSitemapF } from "@/lib/notify-messages";
/**
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (serviceAreaCount > 0 && !serviceAreaKeyword.trim()) {
        notify.error(NOTIFY_SERVICE_AREA_KEYWORD_IS_REQUIRED_WHEN_GE);
        setIsGenerating(false);
        setProgress(0);
        setCurrentStep('');
        return;
...
// Find service area sitemap URL upfront (if needed)
      ...
      if (hasBlogPosts) {
        ...
        notify.info(notifyStep1XAnalyzingWordpressBlogPosts(totalSteps, sitemapUrl));
        ...
        notify.success(notifyStep1XCompleteAnalyzedXBlogPostP(totalSteps, blogPostAnalysis.examples.length));
      }
...
// Step 1a: Scrape service area sitemap first (like the scrape button does)
        ...
        notify.info(notifyStepXXScrapingServiceAreaSitemapF(hasBlogPosts ? '1a' : '1', totalSteps, serviceAreaSitemapUrl));
        ...
        try {
          ...
          notify.success(notifyStepXXCompleteScrapedServiceAreaS(hasBlogPosts ? '1a' : '1', totalSteps));
        } catch (error) {
          console.warn('[PostPagePackGenerator] Service area sitemap scraping failed, continuing with analysis:', error);
          notify.warning(NOTIFY_SERVICE_AREA_SITEMAP_SCRAPING_HAD_ISSUES);
        }
        ...
        notify.info(notifyStepXXAnalyzingWordpressServiceAre(hasBlogPosts ? '1b' : '1', totalSteps, serviceAreaSitemapUrl));
        ...
        notify.success(notifyStepXXCompleteAnalyzedXServiceAre(hasBlogPosts ? '1b' : '1', totalSteps, serviceAreaAnalysis.examples.length));
      }
...
throw new Error('Service area analysis not available - service area sitemap must be analyzed first');
        ...
        notify.info(notifyStepXXGeneratingXServiceAreaPagex(serviceAreaStepNum, totalSteps, serviceAreaCount, serviceAreaCount !== 1 ? 's' : ''));
...
setProgress(hasBlogPosts ? 45 : 95);
          ...
          notify.success(notifyStepXXCompleteGeneratedXServiceAr(serviceAreaStepNum, totalSteps, serviceRows.length, serviceRows.length !== 1 ? 's' : ''));
        } catch (error) {
console.error('[PostPagePackGenerator] Service area generation failed:', error);
          notify.error(notifyServiceAreaGenerationFailedX(error instanceof Error ? error.message : 'Unknown error'));
      ...
      if (postType === 'post' && blogPostCount > 0) {
        ...
        notify.info(notifyStepXXFetchingGscQueryDataForX(hasServiceAreas ? 3 : 2, totalSteps || 3, site.name));
        ...
        try {
          ...
          if (gscRecommendations && gscRecommendations.recommendedKeywords.length > 0) {
            ...
            notify.info(notifyStep23ProcessingXGscKeywordsFilte(gscRecommendations.recommendedKeywords.length));
            ...
            notify.success(notifyStep2XCompleteSelectedXKeywordsFr(totalSteps || 3, totalSelected, gscRecommendations.recommendedKeywords.length, quickWinsCount, others.length));
          } else {
            setProgress(40);
            setCurrentStep('📝 Using WordPress post titles as keywords...');
            notify.info(notifyStep2XNoGscKeywordsFoundWillUse(totalSteps || 3));
          }
        } catch (error) {
          console.warn('[PostPagePackGenerator] GSC analysis failed, continuing without it:', error);
          const errorMsg = error instanceof Error ? error.message : 'Unknown error';
          notify.warning(notifyStep2XWarningGscAnalysisFailedXC(totalSteps || 3, errorMsg));
        }

        // Step 2b: Perform keyword AI analysis for selected keywords (Content Optimizer module logic)
        ...
        if (keywordsToAnalyze.length > 0 && dataForSEOApiKey && openRouterApiKey) {
          try {
            setProgress(46);
            setCurrentStep(`📊 Analyzing ${keywordsToAnalyze.length} keywords with AI (Content Optimizer module)...`);
            notify.info(notifyStep2bXAnalyzingXKeywordsWithAiF(totalSteps || 3, keywordsToAnalyze.length));
            ...
            // Analyze each keyword with full Content Optimizer module logic
            for (let i = 0; i < keywordsToAnalyze.length; i++) {
              ...
              try {
                ...
                // Step 2: Perform AI analysis (Content Optimizer module)
                ...
                notify.warning(notifyKeywordAnalysisFailedForXContinuing(keyword));
              }
            }
            ...
            notify.success(notifyStep2bXCompleteAnalyzedXKeywordsW(totalSteps || 3, analysisResults.size));
          } catch (error) {
            console.error('[PostPagePackGenerator] Error in keyword analysis step:', error);
            notify.warning(NOTIFY_KEYWORD_ANALYSIS_HAD_ISSUES_CONTINUING_W);
          }
        }
        ...
        notify.info(notifyStep3XGeneratingXBlogPostxUsingA(totalSteps || 3, blogPostCount, blogPostCount !== 1 ? 's' : ''));
        ...
        if (blogGeneratedRows.length > 0) {
          ...
          notify.info(notifyProcessingXGeneratedBlogPostx(blogGeneratedRows.length, blogGeneratedRows.length !== 1 ? 's' : ''));
          ...
          notify.success(notifyStepXXCompleteGeneratedXBlogPost(hasServiceAreas ? 3 : 2, totalSteps, blogRows.length, blogRows.length !== 1 ? 's' : ''));
        } else {
          setProgress(0);
          setCurrentStep('❌ Generation failed');
          notify.warning(NOTIFY_NO_BLOG_POSTS_WERE_GENERATED_PLEASE_TRY_);
        }
      }
...
setGeneratedTemplates(allTemplates);
      ...
      notify.error(error instanceof Error ? error.message : 'Failed to generate pack');
      setProgress(0);
      setCurrentStep('Generation failed');
      setIsGenerating(false);
    } finally {
  ...
  const handleDownloadCSV = () => {
    if (generatedTemplates.length === 0) {
      notify.error(NOTIFY_NO_TEMPLATES_TO_DOWNLOAD);
      return;
    }
    ...
    notify.success(NOTIFY_CSV_FILE_DOWNLOADED);
  };
