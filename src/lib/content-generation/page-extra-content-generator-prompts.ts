...
- The <h2> and <h3> must each include this exact focus keyword phrase: "${primaryKeyword}"
- Use the phrase verbatim inside the heading (case may vary; spelling and word order must match).
- You may add words before or after the phrase (e.g. "${primaryKeyword}: planning your visit").
- Do not write generic headings that omit "${primaryKeyword}".`;
}
...
Focus keyword (required inside <h2> and <h3>): "${ctx.primaryKeyword}"
...
export function buildExtraTextH2UserPrompt(ctx: ExtraTextPromptContext): string {
  return `Write section 1 now: one <h2> that includes "${ctx.primaryKeyword}" exactly, then 1-2 intro <p> tags on the same page topic.
...
export function buildExtraTextH3UserPrompt(
  ...
  return `Write section 2 now: one <h3> that includes "${ctx.primaryKeyword}" exactly, ${linkLine}.
