...
export const FLOWBIE_MARKETING_AD_FILES = {
...
const DEFAULT_FLOWBIE_MARKETING_AD = FLOWBIE_MARKETING_AD_FILES.bofuActionList;

export function resolveFlowbieMarketingAdFilename(referenceAdPattern?: string): string {
  const pattern = referenceAdPattern?.trim();
  if (!pattern) return DEFAULT_FLOWBIE_MARKETING_AD;
  const filenames = Object.values(FLOWBIE_MARKETING_AD_FILES);
  ...
  return DEFAULT_FLOWBIE_MARKETING_AD;
}
...
export async function loadFlowbieMarketingAdReference(
  filename: string = DEFAULT_FLOWBIE_MARKETING_AD,
  ...
  try {
    ...
    return {
      ...
      query: "FlowbieONE Instagram ad (marketing/instagram-ads in codebase)",
      kind: "other",
      layer: "foreground",
      why: "Checked-in FlowbieONE feed ad from marketing/instagram-ads",
      visualDescription:
...
export async function loadFlowbieMarketingAdReferenceFromBrief(
  brief: MetaAdCreativeBrief,
): Promise<ImageReferenceResult | null> {
  return loadFlowbieMarketingAdReference(resolveFlowbieMarketingAdFilename(brief.referenceAdPattern));
}
