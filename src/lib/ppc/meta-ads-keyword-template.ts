...
export const META_KEYWORD_TEMPLATE_CSV =
  "Keyword,Blog Title,FB/Instagram Content,Link/Landing page,Prompt Modifier\nwindow coverings Edmonton,Example blog title,,https://example.com/blog/post,Optional visual note\n";
...
export type MetaKeywordImportRow = {
  focusKeyword: string;
  adName?: string;
  contextSource?: "flowbie_app" | "custom";
  contextUrl?: string;
...
const AD_NAME_HEADER_ALIASES = ["blogtitle", "blog_title", "title"];
...
function recordToImportRow(
  ...
  const adName = pickStringFromRow(record, AD_NAME_HEADER_ALIASES) || undefined;
...
function csvHasCalendarColumns(headers: string[]): boolean {
  ...
  return (
    normalized.some((key) => AD_NAME_HEADER_ALIASES.includes(key)) ||
