...
const KEY_INSIGHTS_DIVISION =
  "**SECTION DIVISION (NO REDUNDANCY):** Prefer **prose-only** interpretation (risks, opportunities, what changed for the business). **Forbidden:** opening by restating total clicks, impressions, CTR, or average position **and** their MoM **%** (that is **only** in **Search Performance Compared Month Over Month**). **Forbidden:** paraphrasing the Executive Summary lead paragraph. If you include **one** theme table, **≤6 data rows**, **THEME ROW INTEGRITY** applies; the table must **add** something (e.g. different slice or framing), **not** a second copy of the same top-query grid with identical figures.";
...
export function buildUserMessageForSection(args: {
  ...
  if (
    ...
    parts.push(
      "Report context (NO REDUNDANCY): Sections above this in the final report already include Executive Summary and Search Performance Compared Month Over Month with the **only** site-wide KPI table. Do **not** repeat that table, do **not** restate the same four site totals with MoM %, and do **not** paste a second query/theme MoM grid that duplicates the same rows and figures as an earlier section.",
    );
  }
...
export function getGscReportingSectionSystemPrompt(kind: GscReportingSectionPlan["kind"]): string {
  ...
  switch (kind) {
    ...
    case "search_performance_period":
      return `${base} ${POSITION_LEXICON} SECTION: Search performance MoM - **This section owns the canonical KPI table** (totals / current vs prior period). **Lead with** that one summary table, then at most **2–3** short sentences including the **average position** gloss above. **Do not** put month or date ranges in the H2 - the **Target H2** is fixed. **Table:** mirror CSV values and order; **header row uses abbreviations** (**Clk**, **Imp**, **CTR**, **Pos**, **Δ%** - not long month labels in headers). ${TABLE_RULE} Do not repeat this table elsewhere.`;
