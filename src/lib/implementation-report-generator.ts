...
export interface ContentComparison {
  ...
  /** Prior ACF `seo_extra_text` / `extra_text` (markdown or plain). */
  originalExtraText?: string;
  /** New ACF extra block written this run (markdown). */
  newExtraText?: string;
...
export async function generateImplementationReport(
  ...
  options?: { skipMetaDescriptionSection?: boolean }
): Promise<string> {
  const skipMetaDescriptionSection = Boolean(options?.skipMetaDescriptionSection);
  const {
    originalTitle,
    newTitle,
    originalExcerpt,
  ...
  const origExtraTrim = (originalExtraText || '').trim();
  const newExtraTrim = (newExtraText || '').trim();
  if (origExtraTrim || newExtraTrim) {
    report.push('## Extra text (ACF: seo_extra_text)');
    report.push('');
    report.push(
      'This block is the complementary section written to the `seo_extra_text` field (or `extra_text` when that is what the site uses).',
    );
    report.push('');
    if (origExtraTrim !== newExtraTrim) {
      report.push('### Before');
      report.push(origExtraTrim ? origExtraTrim : '_(empty)_');
      report.push('');
      report.push('### After');
      report.push(newExtraTrim ? newExtraTrim : '_(empty)_');
      report.push('');
    } else {
      report.push('**No textual change recorded** (values match or both empty in this comparison).');
      report.push('');
    }
  }
  ...
  if (origExtraTrim !== newExtraTrim && (origExtraTrim || newExtraTrim)) {
    changes.push('- **Extra text (seo_extra_text):** Updated complementary ACF block');
  }
